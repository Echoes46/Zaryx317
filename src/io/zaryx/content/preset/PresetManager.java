package io.zaryx.content.preset;
...
import static io.zaryx.content.CompletionistCape.hasRequirements;
...
public class PresetManager {
	...
	public void open(Player player) {
		...
		if (Boundary.isIn(player, Boundary.OUTLAST_AREA)
				...
				|| Boundary.isIn(player, ArbograveConstants.ALL_BOUNDARIES)) {
			return;
		}
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("demonx") || itemName.contains("strongman") || itemName.contains("tainted")) {
			return false;
		}
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				|| itemName.contains("karil") ||  itemName.contains("verac") || itemName.contains("keeper")
				...
				itemName.contains("hunting knife") || itemName.contains("elder maul") || itemName.contains("bulwark") || itemName.contains("claws")) {
			return true;
		}
		switch (itemId) {
			case 28688:
	...
	private void loadPreset(Player player, boolean loadPrevious) {
		...
		try {
			...
			if (!defaultPreset) {
				...
				if (player.getItems().freeEquipmentSlots() != 15) {
					player.sendMessage("You need to remove your equipment before doing this.");
					player.sendMessage("You have: [" + player.getItems().freeEquipmentSlots() + "] equip slots free.");
					return;
				}
