...
public class QuickPrayers {
	...
	private static void activateNormal(Player player, int prayer) {
		 ...
		 * Checks for level
		 */
		if (player.getLevelForXP(player.playerXP[5]) <  CombatPrayer.PRAYER_LEVEL_REQUIRED[prayer]) {
			player.sendMessage("You don't have the required Prayer level to activate " +  CombatPrayer.PRAYER_NAME[prayer] + ".");
			return;
		}
		/**
		 * Switches
