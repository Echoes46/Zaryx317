...
import static io.zaryx.content.combat.weapon.CombatStyle.SLASH;
...
public class MeleeData {
    ...
    public static boolean usingSytheOfVitur(Player player) {
        if (player.attacking.getCombatType() == CombatType.MELEE && player.getItems().isWearingItem(25739)) {
            ...
            return true;
        } else if (player.attacking.getCombatType() == CombatType.MELEE && player.getItems().isWearingItem(39001)) {
            return true;
    ...
    public static void setWeaponAnimations(Player c) {
        ...
        if (weaponName.contains("ale of the gods") || weaponName.contains("crystal grail")) {
    ...
    private static int getStyleBasedAnimation(Player c, int stabAnim, int slashAnim, int crushAnim) {
        if (c.getCombatConfigs().getWeaponMode() == null || c.getCombatConfigs().getWeaponMode().getCombatStyle() == null) {
            return slashAnim;
        }
    ...
    private static int getSwordAnimation(Player c) {
        return getStyleBasedAnimation(c, 412, 390, 401);
    }

    private static int getScimitarAnimation(Player c) {
        return getStyleBasedAnimation(c, 412, 395, 401);
    }

    private static int getDaggerAnimation(Player c) {
        return getStyleBasedAnimation(c, 412, 390, 401);
    }

    private static int getSpearAnimation(Player c) {
        return getStyleBasedAnimation(c, 400, 390, 401);
    }

    private static int getMaceAnimation(Player c) {
        return getStyleBasedAnimation(c, 412, 390, 401);
    }

    private static int getAxeAnimation(Player c) {
        return getStyleBasedAnimation(c, 390, 395, 401);
    }

    private static int getTwoHandedSwordAnimation(Player c) {
        return getStyleBasedAnimation(c, 7045, 7045, 7054);
    }

    public static int getWepAnim(Player c) {
        int weaponId = c.getEquipmentToShow(Player.playerWeapon);
        String weaponName = ItemAssistant.getItemName(weaponId).toLowerCase();
        ...
        /*
         * Ranged / magic / custom weapons with known dedicated animations.
         */
        ...
        /*
         * Special/custom melee weapons with unique animations.
         */
        if ((weaponName.contains("scythe") && !weaponName.contains("demonx")) || weaponName.contains("candy cane scythe")) {
            return 8056;
        }
        if (weaponName.contains("demonx scythe")) {
            return 8290;
        }
        if (weaponName.contains("demonx sword")) {
            return getSwordAnimation(c);
        }
        if (weaponName.contains("osmumten")) {
            return getStyleBasedAnimation(c, 9471, 390, 9471);
        }
        if (weaponName.contains("ghrazi rapier")) {
            return getStyleBasedAnimation(c, 8145, 390, 401);
        }
        if (weaponName.contains("hunting knife")) {
            return 7328;
        }
        ...
        if (weaponName.contains("lance")) {
            switch (c.getCombatConfigs().getWeaponMode().getCombatStyle()) {
                case STAB:
                    return 8288;
                case CRUSH:
                case SLASH: // 8289 is slash but it is broken.
                default:
                    return 8290;
            }
        }
        if (weaponName.contains("dharok")) {
            return getStyleBasedAnimation(c, 2067, 2067, 2066);
        }
        if (weaponName.contains("soulreaper")) {
            return 7054;
        }
        if (weaponName.contains("arclight")) {
            return getStyleBasedAnimation(c, 451, 2067, 2067);
        }
        ...
        /*
         * Normal weapon families. These use the actual selected CombatStyle so
         * Lunge/Stab styles no longer reuse Slash animations, and Slash styles no
         * longer force lunge animations.
         */
        if (weaponName.contains("halberd") || weaponName.contains("partisan")) {
            return getStyleBasedAnimation(c, 440, 440, 401);
        }
        if (weaponName.contains("hasta") || weaponName.contains("spear") || weaponName.contains("zamorakian")) {
            return getSpearAnimation(c);
        }
        if (weaponName.contains("2h sword") || weaponName.contains("godsword")
                || weaponName.contains("saradomin sword") || weaponName.contains("blessed sword")
                || weaponName.contains("large spade")) {
            return getTwoHandedSwordAnimation(c);
        }
        if (weaponName.contains("dagger")) {
            return getDaggerAnimation(c);
        }
        if (weaponName.contains("scim")) {
            return getScimitarAnimation(c);
        }
        if (weaponName.contains("warhammer") || weaponName.contains("maul") || weaponName.contains("bludgeon") || weaponName.contains("club")) {
            return 401;
        }
        if (weaponName.contains("mace") || weaponName.contains("flail") || weaponName.contains("anchor") || weaponName.contains("cudgel") || weaponName.contains("chainmace")) {
            return getMaceAnimation(c);
        }
        if (weaponName.contains("battleaxe") || weaponName.contains(" axe") || weaponName.endsWith("axe") || weaponName.contains("pickaxe")) {
            return getAxeAnimation(c);
        }
        ...
        if (weaponName.contains("staff") || weaponName.contains("wand") || weaponName.contains("sceptre") || weaponName.contains("trident")) {
            return getStyleBasedAnimation(c, 412, 390, 406);
        }

        switch (weaponId) { // if you don't want to use strings
