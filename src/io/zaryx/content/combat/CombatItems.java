...
public class CombatItems {
	...
	public boolean elyProc() {
		...
		if (c.playerEquipment[Player.playerShield] == 12817 && Misc.isLucky(45)) {
			c.startGraphic(new Graphic(333));
			return true;
		}
	...
	public boolean wearingThammaronsSceptre() {
		return c.getItems().getWeapon() == Items.THAMMARONS_SCEPTRE || c.getItems().getWeapon() == 27679;
	...
	public boolean usingAccumulator() {
		return c.getItems().isWearingItem(Items.AVAS_ACCUMULATOR)
				...
				|| c.getItems().isWearingItem(33183);
	}
	...
	public void checkVenomousItems(Entity target) {
		...
		if (c.getItems().isWearingItem(28585, Player.playerWeapon)) {
			c.setToxicStaffOfTheDeadCharge(c.getToxicStaffOfTheDeadCharge() - 10);
			if (c.getToxicStaffOfTheDeadCharge() <= 0) {
				c.setToxicStaffOfTheDeadCharge(0);
				c.sendMessage("Your toxic staff of the dead has lost all charge.");
				c.getItems().equipItem(-1, 0, Player.playerWeapon);
				c.getItems().addItemUnderAnyCircumstance(28583, 1);
			}
		}

		if (c.getItems().isWearingItem(22323, Player.playerWeapon)) {
			c.setSangStaffCharge(c.getSangStaffCharge() - 1);
			if (c.getSangStaffCharge() <= 0) {
				c.setSangStaffCharge(0);
				c.sendMessage("Your sang staff of the dead has lost all charge.");
				c.getItems().equipItem(-1, 0, Player.playerWeapon);
