...
public class Killstreak {
	...
	private void giveKillstreakReward(Type type, int killstreak) {
		if (type == Type.ROGUE) {
			player.getItems().addItemUnderAnyCircumstance((player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? 13307 : 2996), killstreak);
			player.sendMessage("You are on a " + killstreak + " rogue killstreak, you have been given " + killstreak + (player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? "Blood Money." :" pk tickets."));
		} else if (type == Type.HUNTER) {
			player.getItems().addItemUnderAnyCircumstance((player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? 13307 : 2996), 5);
			player.sendMessage("You are on a 3 hunter killstreak, you have been given 5 "+ (player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? "Blood Money." :" pk tickets."));
		}
	}
