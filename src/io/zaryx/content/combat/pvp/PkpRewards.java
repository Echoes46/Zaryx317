package io.zaryx.content.combat.pvp;

import io.zaryx.Configuration;
import io.zaryx.Server;
import io.zaryx.content.minigames.bounty_hunter.TargetState;
...
//                    && killer.getBH().hasTarget() && killer.getBH().getTarget().getName().equalsIgnoreCase(dying.playerName)) {
        ...
        if (Boundary.isIn(dying, Boundary.WILDERNESS_PARAMETERS)) {
            if (personDieingKillstreak > 10) {
                killer.getItems().addItemUnderAnyCircumstance((killer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? 13307 : 2996), 30);
                killer.sendMessage("[@red@PKP@bla@] You are rewarded an extra@red@ 10" + " "+(killer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? "blood money" : "pkp")+" @bla@for ending a kill streak.");
            ...
            if (canReceiveRewards) {
                didReceiveRewards = true;
                int random = 30;
                killer.getItems().addItemUnderAnyCircumstance((killer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? 13307 : 2996), random * (killer.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33074) ? 2 : 1));
                if (dying.amDonated >= 20 && dying.amDonated < 50) { //regular donator
                    int bonuspkp = 3;
                    killer.getItems().addItemUnderAnyCircumstance((killer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? 13307 : 2996), bonuspkp);
                    killer.sendMessage("[@red@PKP@bla@] You are rewarded an extra@red@ " + bonuspkp + " "+(killer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) ? "blood money" : "pkp")+" @bla@for your donator rank.");
                ...
                if (killer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) || killer.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
        ...
        PlayerHandler.executeGlobalMessage(":killfeed:"+killer.getLoginName()+":"+dying.getLoginName()+":"+killer.getStyle());
        killer.ismeleeing = killer.ismaging = killer.isranging = false;
