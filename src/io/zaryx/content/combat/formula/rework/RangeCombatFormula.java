...
public class RangeCombatFormula implements CombatFormula {
    ...
    private double getEffectiveRangeAttackLevel(Player player) {
        ...
        if (player.fullEliteVoidRange()) {
            ...
            effectiveLevel = Math.floor(effectiveLevel);
        } else if (player.fullEliteVoidRange() && !player.getPosition().inWild()) {
        ...
        if (player.armourofozrange()) {
            effectiveLevel += effectiveLevel * 1.5;
            effectiveLevel = Math.floor(effectiveLevel);
        }
    ...
    public int getMaxHit(Entity attacker, Entity defender, double specialAttackMultiplier,
        ...
        if (maxhit != -1) {
            ...
            if (attacker.isPlayer()) {
                ...
                if (player.getItems().isWearingItem(Items.TOXIC_BLOWPIPE, Player.playerWeapon) || player.getItems().isWearingItem(33177, Player.playerWeapon)|| player.getItems().isWearingItem(28688, Player.playerWeapon)) {
