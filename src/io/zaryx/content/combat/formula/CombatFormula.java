...
import static io.zaryx.content.combat.melee.CombatPrayer.*;
...
public class CombatFormula {
    ...
    public static double getPrayerStrengthBonus(Player c) {
        if (c.prayerActive[BURST_OF_STRENGTH])
            ...
            return 1.23;
        else if (c.prayerActive[LEECH_STRENGTH])
            return 1.00 + (c.leechStrength*0.1);
        else if (c.prayerActive[TURMOIL])
            return 1.23;
        return 1;
    }
    ...
    public static double getPrayerRangedAccuracyBonus(Player c) {
        if (c.prayerActive[SHARP_EYE]) {
            ...
            return 1.20;
        } else if (c.prayerActive[LEECH_RANGED]) {
            return 1.00 + (c.leechRanged * 0.1);
        }
