...
import io.zaryx.content.combat.effects.damageeffect.impl.amuletofthedamned.impl.KarilEffect;
...
import io.zaryx.content.combat.weapon.WeaponInterface;
    ...
    public void beforeDamageCalculated(CombatType type) {
        ...
        if (type == CombatType.MELEE) {
            ...
            if (attacker.getSpellId() > -1) {
                ...
                if (PvpWeapons.activateEffect(attacker, Items.THAMMARONS_SCEPTRE) || PvpWeapons.activateEffect(attacker, 27679))
...
/*        if (npcId == 655 && !Boundary.isIn(attacker, Boundary.AFK_ZONE_BOSS)) {
        ...
        if (npcId == Hunllef.RANGED_PROTECT && type == CombatType.RANGE) {
            npc.forceChat("I CAN GET YOU FROM HERE!!");
            maximumDamage = 0;
            maximumAccuracy = 0;
        }
    }
