...
// Now check against each NPC ID individually and whether the player has Trickster equipped
        if ((npcId == 4923 && hasTricksterEquipped) ||
                ...
                (npcId == 11278 && hasTricksterEquipped) ||
                (npcId == 11246 && hasTricksterEquipped)) {

            c.sendMessage("@red@You must take off trickster to attack this npc");
            return false;
        }
        ...
        if (Boundary.isIn(c, Boundary.NEX) && c.usingInfAgro) {
            c.sendMessage("@red@Aggro pot is disabled in nex.");
            return false;
        }

//        if (c.getPosition().inMulti() &&  c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33216)) {
//            c.sendMessage("Trickster does not work in multi combat zones");
//        }
        ...
        if (!npc.getPosition().inMulti()) {
            ...
            if (!Boundary.isIn(c, Boundary.OLM) && !Boundary.isIn(c, Boundary.RAIDS) && !Boundary.isIn(c, Boundary.DUNGROOMS)) {
...
/*        if (npc.getNpcId() == 4922 || npc.getNpcId() == 5129 || npc.getNpcId() == 8918 || npc.getNpcId() == 7860) {
        ...
        if (npc.getNpcId() == 6611 || npc.getNpcId() == 6612) {
            ...
            if (minion.stream().filter(Objects::nonNull).anyMatch(n -> n.getNpcId() == Npcs.SKELETON_HELLHOUND && !n.isDead() && n.getHealth().getCurrentHealth() > 0 && n.getPosition().inWild())) {
