...
public class HitExecutorNpc extends HitExecutor {
    ...
    public static void applySoulSplitOnNpc(Player c, int damage, Entity targetEntity) {
...
if (damage.getAmount() > 0) {
    if (CombatPrayer.isPrayerOn(attacker, CombatPrayer.SOUL_SPLIT)) {
        applySoulSplitOnNpc(attacker, damage.getAmount(), defender.asNPC());
        attacker.gfx100(1184);
    }
}
...
//            }
        ...
        } else if (npc.getNpcId() == 8888) {
            if (Seldaeh.damageCount.containsKey(attacker)) {
                Seldaeh.damageCount.put(attacker, Seldaeh.damageCount.get(attacker) + damage.getAmount());
            } else {
                Seldaeh.damageCount.put(attacker, damage.getAmount());
            }
        ...
        switch (npc.getNpcId()) {
            ...
            case 1612:
                ...
                break;
            case 5126:
               /* if (!Boundary.isIn(attacker, new Boundary(2618, 3971, 2690, 3901))) {
                    attacker.attacking.reset();
                    attacker.sendMessage("You cannot do this from here.");
                    return;
                }*/
            ...
            case 11246:
                attacker.setMaledictusDamageCounter(attacker.getMaledictusDamageCounter() + damage.getAmount());
                NPCHandler.glodAttack = "MAGIC";
                break;
