...
import io.zaryx.content.bosses.hydra.HydraStage;
import io.zaryx.content.bosses.nightmare.attack.Spores;
import io.zaryx.content.combat.magic.*;
import io.zaryx.content.combat.melee.MeleeData;
import io.zaryx.content.combat.range.Bow;
...
import io.zaryx.content.items.aoeweapons.AOESystem;
import io.zaryx.content.items.aoeweapons.AoeManager;
...
import io.zaryx.model.entity.npc.NPCClipping;
...
public class AttackEntity {
    ...
    public void attackEntity(Entity targetEntity) {
        ...
        if (targetEntity.isNPC() && !attacker.usingClickCast) {
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == Items.THAMMARONS_SCEPTRE || attacker.playerEquipment[Player.playerWeapon] == 27679) {
                attacker.usingMagic = true;
                attacker.autocasting = true;
                attacker.setSpellId(104);
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == 84) {
                attacker.usingMagic = true;
                attacker.autocasting = true;
                attacker.setSpellId(Armadyl.COMBAT_SPELL_INDEX);
            }
        ...
        if (targetEntity.isPlayer()) {
            ...
            if (!Boundary.isIn(attacker, Boundary.DUEL_ARENA) && !TourneyManager.getSingleton().isInArena(attacker) && !WGManager.getSingleton().isInArena(attacker)) {
                if (!attacker.attackedPlayers.contains(attacker.playerAttackingIndex) && !PlayerHandler.players[attacker.playerAttackingIndex].attackedPlayers.contains(attacker.getIndex()) && !attacker.isSkulled) {
                    if (!targetPlayer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && !targetPlayer.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN))) {
            ...
            if (getCombatType() == CombatType.MELEE && (targetNpc.getNpcId() == 3162 || targetNpc.getNpcId() == 3169
                    ...
                    || targetNpc.getNpcId() == 3718 || targetNpc.getNpcId() == 3713 || targetNpc.getNpcId() == 3714
                    || targetNpc.getNpcId() == 3715 || targetNpc.getNpcId() == 3716 || targetNpc.getNpcId() == 3719
                    || targetNpc.getNpcId() == 3720 || targetNpc.getNpcId() == 3725 || targetNpc.getNpcId() == 3726
                    || targetNpc.getNpcId() == 3727 || targetNpc.getNpcId() == 3728 || targetNpc.getNpcId() == 3729)) {
                attacker.sendMessage("You Must be using Ranged to attack this creature!");
                return;
            }
        ...
        if (attacker.usingSpecial
            ...
            if (getCombatType() == CombatType.MAGE) {
                ...
                if (getCombatType() == CombatType.RANGE) {
                    ...
                    if (attacker.playerEquipment[Player.playerWeapon] == 29000) {
                        attacker.gfx100(1910);
                    }
                    ...
                    if (attacker.playerEquipment[Player.playerWeapon] >= 4212 && attacker.playerEquipment[Player.playerWeapon] <= 4223
                            ...
                            || attacker.playerEquipment[Player.playerWeapon] == 33005
                            ...
                            || attacker.playerEquipment[Player.playerWeapon] == 20484
                            || attacker.playerEquipment[Player.playerWeapon] == 39000
    ...
    private void handleItemChangesOnAttack(Entity targetEntity) {
        ...
        if (!attacker.usingMagic) {
            ...
            if (targetEntity.isNPC()) {
                ...
                } else if (attacker.playerEquipment[Player.playerWeapon] == Items.THAMMARONS_SCEPTRE) {
                    attacker.setThammaronCharge(attacker.getThammaronCharge() - 1);
                } else if (attacker.playerEquipment[Player.playerWeapon] == 27679) {
                    attacker.setThammaronCharge(attacker.getThammaronCharge() - 1);
                } else if (attacker.playerEquipment[Player.playerWeapon] == 84) {
                    attacker.setArmadylStaffCharge(attacker.getArmadylStaffCharge() - 1);
                }
            }
    ...
    public void fireRangeProjectile(Entity entity) {
        ...
        if (attacker.weaponUsedOnAttack == 12926) {
            ...
            attacker.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(attacker),
                    RangeData.getRangeProjectileGFX(attacker), 35, 35,
                    Projectile.getLockon(entity), 45);
        } else if (attacker.weaponUsedOnAttack == 29000) {
            attacker.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(attacker),
                    1910, 35, 35,
    ...
    private boolean determineWeaponStyle() {
        ...
        switch (attacker.playerEquipment[Player.playerWeapon]) {
            ...
            case 84:
                if (attacker.getArmadylStaffCharge() <= 0/* && attacker.playerEquipment[Player.playerWeapon] != 25731*/) {
                    attacker.sendMessage("Your Armadyl staff has no more charges.");
                    attacker.attacking.reset();
                    return false;
                }
                if (!attacker.usingClickCast) {
                    ...
                    attacker.autocasting = true;
                    attacker.autocastId = Armadyl.COMBAT_SPELL_INDEX;
                }
                return true;
            case 33205:
                if (!attacker.usingClickCast) {
            ...
            case 39002:
                if (!attacker.usingClickCast) {
                    attacker.usingMagic = true;
                    attacker.autocasting = true; //Demonx Staff
                    attacker.autocastId = 109;
                }
                return true;
            case 33149:
                if (!attacker.usingClickCast) {
                    attacker.usingMagic = true;
                    attacker.autocasting = true; //nox Staff
            ...
            case 20486:
                if (!attacker.usingClickCast) {
                    attacker.usingMagic = true;
                    attacker.autocasting = true; //nox Staff
                    attacker.autocastId = 108;
                }
                return true;
            case 28585:
                if (attacker.getToxicStaffOfTheDeadCharge() <= 0) {
                    attacker.sendMessage("Your sceptre is out of charges");
                    attacker.attacking.reset();
                    return false;
                }
                if (!attacker.usingClickCast) {
                    attacker.usingMagic = true;
                    attacker.autocasting = true; //warped sceptre
              ...
              case 22335:
                  if (!attacker.usingClickCast) {
                      attacker.usingMagic = true;
                      attacker.autocasting = true; //starter staff
            ...
            case 22555:
            case 27679:
                if (attacker.getPvpWeapons().thammaronSceptreCharges <= 0) {
                    attacker.sendMessage("Your sceptre is out of charges");
                    attacker.attacking.reset();
                    return false;
                }
                if (!attacker.usingClickCast) {
                    attacker.usingMagic = true;
                    attacker.autocasting = true; //warped
        ...
        if (!attacker.usingMagic) {
            ...
            attacker.usingCross =
                    attacker.playerEquipment[Player.playerWeapon] == 4734
                            ...
                            || attacker.playerEquipment[Player.playerWeapon] == 28869
            ...
            if (!Bow.canUseArrow(attacker) && Configuration.CORRECT_ARROWS && attacker.usingBow && !attacker.getCombatItems().usingCrystalBow() && !(attacker.playerEquipment[Player.playerWeapon] == 22550)
                    ...
                    && attacker.playerEquipment[Player.playerWeapon] != 26374 && attacker.playerEquipment[Player.playerWeapon] != 33206 && attacker.playerEquipment[Player.playerWeapon] != 26269 && attacker.playerEquipment[Player.playerWeapon] != 25916 && attacker.playerEquipment[Player.playerWeapon] != 8880 && attacker.playerEquipment[Player.playerWeapon] != 25918
                    && (attacker.playerEquipment[Player.playerWeapon] != 25896)
                    && (attacker.playerEquipment[Player.playerWeapon] != 25886)
                    && (attacker.playerEquipment[Player.playerWeapon] != 28869)
                    && (attacker.playerEquipment[Player.playerWeapon] != 25884)
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == 9185 && !attacker.getCombatItems().properBolts()
                    ...
                    || attacker.playerEquipment[Player.playerWeapon] == 28869 && !attacker.getCombatItems().properBolts()
            ...
            // Disable attack if pvp weapon out of charge
            if (weapon == Items.THAMMARONS_SCEPTRE || weapon == Items.THAMMARONS_SCEPTRE_U || weapon == 27679) {
                if (attacker.getPvpWeapons().getCharges(weapon) <= 0) {
                    attacker.stopMovement();
                    attacker.sendMessage("Your weapon needs charges to function properly.");
                    attacker.attacking.reset();
        ...
        if (attacker.getItems().isWearingItem(84)) {
            if (attacker.getArmadylStaffCharge() <= 0) {
                attacker.sendMessage("Your Armadyl staff has no charges remaining, you must recharge it.");
                return false;
            }
        }
    ...
    public static int getRangeDistance(Player player) {
        ...
        if (name.contains("twisted bow"))
            ...
            return 10;
        else if (name.contains("webweaver bow"))
            return isLongRange ? 12 : 10;
