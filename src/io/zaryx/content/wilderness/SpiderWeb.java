...
public class SpiderWeb {
    ...
    public static void slash(Player player, GlobalObject object) {
        ...
        /**
         * Check added for no weapon & for a knife.
         */

        Optional<WorldObject> obj2 = player.getRegionProvider().get(objectX, objectY).getWorldObject(object.getObjectId(), objectX, objectY, object.getHeight());

        if (player.getItems().playerHasItem(946)) {
            player.sendMessage("You slash the web apart.");
            player.startAnimation(390);
            Server.getGlobalObjects().add(new GlobalObject(734, objectX, objectY, obj2.get().getHeight(), obj2.get().getFace(), obj2.get().getType(), 20, 733));
            return;
        } else if (weaponId == -1) {
        ...
        if (player.getItems().hasItemOnOrInventory(Items.KNIFE)) {
            player.sendMessage("You slash the web apart.");
            player.startAnimation(390);
            Server.getGlobalObjects().add(new GlobalObject(734, objectX, objectY, obj2.get().getHeight(), obj2.get().getFace(), obj2.get().getType(), 20, 733));
            return;
