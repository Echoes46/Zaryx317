...
public class TaskMaster {
    ...
    public void showInterface() {
        for (TaskMasterKills taskMasterKills : player.getTaskMaster().taskMasterKillsList) {
            if (taskMasterKills.getWeekly()) {
                player.getPA().sendString(38008, "Complete @whi@" + taskMasterKills.getDesc() + " @or1@"
                        + (taskMasterKills.getAmountToKill() - taskMasterKills.getAmountKilled()) + " times.");
                player.getPA().sendString(38009, getTaskTime(taskMasterKills));
                player.getPA().sendString(38010,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled()
                                ? "@red@Incomplete"
                                : "@gre@Complete");
                player.getPA().sendConfig(5002,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled() ? 0 : 1);
            }

            if (taskMasterKills.getTaskType() == TaskType.COMBAT && !taskMasterKills.getWeekly()) {
                player.getPA().sendString(38002, "Kill @whi@" + taskMasterKills.getDesc() + " @or1@"
                        + (taskMasterKills.getAmountToKill() - taskMasterKills.getAmountKilled()) + " times.");
                player.getPA().sendString(38003, getTaskTime(taskMasterKills));
                player.getPA().sendString(38004,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled()
                                ? "@red@Incomplete"
                                : "@gre@Complete");
                player.getPA().sendConfig(5000,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled() ? 0 : 1);
            }

            if (taskMasterKills.getTaskType() == TaskType.SKILLING) {
                player.getPA().sendString(38005, taskMasterKills.getDesc() + " @or1@"
                        + (taskMasterKills.getAmountToKill() - taskMasterKills.getAmountKilled()) + " times.");
                player.getPA().sendString(38006, getTaskTime(taskMasterKills));
                player.getPA().sendString(38007,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled()
                                ? "@red@Incomplete"
                                : "@gre@Complete");
                player.getPA().sendConfig(5001,
                        taskMasterKills.getAmountToKill() > taskMasterKills.getAmountKilled() ? 0 : 1);
            }
        }
    ...
    public void generateTasks(Player player, boolean resetScroll) {
        if (player.getTaskMaster().taskMasterKillsList == null) {
            player.getTaskMaster().taskMasterKillsList = new ArrayList<>();
        }

        if (player.getTaskMaster().taskMasterKillsList.isEmpty()
                || player.getTaskMaster().taskMasterKillsList.stream()
                .anyMatch(t -> LocalDateTime.now().isAfter(t.getLocalDateTime()))) {

            player.getTaskMaster().setMoneyMakingTime(LocalDateTime.now().plus(1, ChronoUnit.HOURS));

            player.getTaskMaster().taskMasterKillsList
                    .removeIf(killz -> LocalDateTime.now().isAfter(killz.getLocalDateTime()));

            while (player.getTaskMaster().taskMasterKillsList.size() < 3) {
                Tasks tasks = Tasks.values()[Misc.random(Tasks.values().length - 1)];

                if (player.getMode().equals(Mode.forType(ModeType.WILDYMAN))
                        || player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN))) {

                    if (tasks.taskType == TaskType.COMBAT
                            && !tasks.daily
                            && tasks.wildy
                            && player.getTaskMaster().taskMasterKillsList.stream()
                            .noneMatch(t -> t.getTaskType() == TaskType.COMBAT && !t.getWeekly())) {

                        TaskMasterKills killz = new TaskMasterKills(
                                tasks.max,
                                0,
                                new GameItem[]{new GameItem(13307, 750)},
                                tasks.difficultyType,
                                tasks.taskType,
                                false,
                        ...
                        player.sendMessage("[@pur@TaskMaster@bla@] You've been assigned " + killz.getDesc()
                                + " x " + killz.getAmountToKill() + ", "
                                + killz.getTaskDifficulty().name().toLowerCase() + " difficulty");
                    }

                    if (tasks.taskType == TaskType.SKILLING
                            && player.getTaskMaster().taskMasterKillsList.stream()
                            .noneMatch(t -> t.getTaskType() == TaskType.SKILLING)) {
                        ...
                        player.getTaskMaster().taskMasterKillsList.add(killz);
                        player.sendMessage("[@pur@TaskMaster@bla@] You've been assigned "
                                + killz.getDesc().replace("@whi@", "")
                                + " x " + killz.getAmountToKill() + ", "
                                + killz.getTaskDifficulty().name().toLowerCase() + " difficulty");
                    }
                    ...
                    if (player.getTaskMaster().taskMasterKillsList.stream().noneMatch(TaskMasterKills::getWeekly)) {
                        if (tasks.daily && tasks.wildy) {
                            TaskMasterKills killz = new TaskMasterKills(
                                    tasks.max,
                                    0,
                                    new GameItem[]{new GameItem(13307, 1500)},
                                    tasks.difficultyType,
                                    tasks.taskType,
                                    true,
                    ...
                    if (tasks.taskType == TaskType.SKILLING
                        ...
                        TaskMasterKills killz = new TaskMasterKills(
                                tasks.max,
                                0,
                                new GameItem[]{new GameItem(13307, 2500)},
                                tasks.difficultyType,
                                tasks.taskType,
                                false,
                                LocalDateTime.now().plus(1, ChronoUnit.HOURS),
    ...
    public void finishTask(Player player, TaskMasterKills kills) {
        ...
        player.sendMessage("<col=4B0082>Congratulations you finished "
                + kills.getDesc() + " x " + kills.getAmountToKill() + ".");
    }
    ...
    public void loadAllMoneyMaking(Player player) {
        ...
        try (FileReader fileReader = new FileReader(file)) {
            ...
            if (rootEl.isJsonArray()) {
                ...
                System.out.println("[TaskMaster] " + player.getLoginName()
                        + ": unexpected JSON type " + rootEl.getClass().getSimpleName());
                return;
            }

            Type listType = new TypeToken<List<TaskMasterKills>>() {}.getType();
            List<TaskMasterKills> kills = gson.fromJson(asArray, listType);

            if (kills == null) {
                kills = Collections.emptyList();
            }
    ...
    public void saveAllMoneyMaking(Player player) {
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            System.out.println("Saved " + player.getTaskMaster().taskMasterKillsList.size()
                    + " tasks to " + player.getLoginName() + ".json");
