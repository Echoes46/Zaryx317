package io.zaryx.content.wildwarning;
...
import static io.zaryx.content.combat.formula.rework.MeleeCombatFormula.WEAPONWILD;
...
public class WildWarning {
    ...
    public static void sendWildWarning(Player player, Consumer<Player> accept) {
        ...
        for (int i : new int[] {}) {
            if (player.getItems().hasItemOnOrInventory(ARMORWILD)) {
                player.sendMessage("[@red@WILD@bla@] @blu@You can not take " + ItemDef.forId(i).getName() + " into the wild!");
                return;
            }
        }
        ...
        for (int i : new int[] {}) {
            if (player.getItems().hasItemOnOrInventory(WEAPONWILD)) {
                player.sendMessage("[@red@WILD@bla@] @blu@You can not take " + ItemDef.forId(i).getName() + " into the wild!");
                return;
            }
        }
