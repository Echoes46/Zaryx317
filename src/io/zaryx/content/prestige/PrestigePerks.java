package io.zaryx.content.prestige;
...
public enum PrestigePerks {
    ...
    // 5x mode = 7500
    // standard mode = 1500
    DAMAGE_BONUS1(1, 3, 18934, 18935, 2406, 500, 5_000_000),//1.5% damage boost
    EXPERIENCE_BONUS1(1, 3, 18916, 18917, 2400, 500, 5_000_000),//1.5% xp Boost

    DOUBLE_PC_POINTS(1, 100, 18952, 18953, 2412, 500, 5_000_000),//2x Chest rewards

    BLOODY_MINIGAME(2, 100, 18919, 18920, 2401, 500, 7_500_000), // Double Bloodyminigame points!
    CANNON_EXTENDER(2, 100, 18937, 18938, 2407, 500, 7_500_000),// 150 more Cannonballs! 200 if 1x & 5x xp mode
    TRIPLE_HESPORI_KEYS(2, 10, 18955, 18956, 2413, 500, 7_500_000),//3x all keys 10 chance

    DAMAGE_BONUS2(3, 3, 18940, 18941, 2408, 500, 12_500_000),//1.5% damage boost
    EXPERIENCE_BONUS2(3, 3, 18922, 18923, 2402, 500, 12_500_000),//1.5% xp Boost
    RESTORE_FULL_PRAYER(3, 100, 18958, 18959, 2414, 500, 12_500_000),// 10% chance to restore prayer to full

    ATTUNE_PERKS(4, 3, 18943, 18944, 2409, 500, 15_000_000),//10 % chance to save item / Nomad points when upgrading
    ZERK_RANGE_MAGE(4, 1, 18925, 18926, 2403, 500, 15_000_000),//ZERK, Range & Mage perk's combined into one for bonus damage!
    HEAL_TO_MAX(4, 100, 18961, 18962, 2415, 500, 15_000_000),//10% chance to heal to max

    DAMAGE_BONUS3(5, 3, 18946, 18947, 2410, 500, 20_000_000),//1.5% damage boost
    EXPERIENCE_BONUS3(5, 3, 18928, 18929, 2404, 500, 20_000_000),//1.5% xp Boost
    NOMAD_PLUS_15(5, 15, 18964, 18965, 2416, 500, 20_000_000),//15% more foundry when melting items!

    EXPERIENCE_DAMAGE_BONUS1(6, 6, 18949, 18950, 2411, 500, 30_000_000),//3% damage boost & xp boost
    RAGE(6, 0, 18967, 18968, 2417, 500, 30_000_000),//An item that gives you "Rage mode" -
    ONE_TIME_15_UMBS(6, 15, 18931, 18932, 2405, 500, 30_000_000)//One time purchase of 10 umb's for normal or 20 for 5x and 1x xp modes.
    ;
    ...
    public static void HandleButtons(Player player, PrestigePerks value) {
        ...
        player.start(new DialogueBuilder(player).option("This relic costs " + value.madCost + " UpgradePoints & " + value.cost + " prestige pooints!",
                new DialogueOption("Buy relic", p -> {
                    if (value.madCost > player.foundryPoints) {
                        p.sendMessage("You need " + Misc.formatCoins(value.madCost) + " UpgradePoints to purchase this perk!");
                    ...
                    switch (value.button) {
                        case 18932:
                            ...
                            p.getItems().addItemUnderAnyCircumstance(6679, 10);
                            break;
                        case 18968:
