package io.zaryx.content.dwarfleaguecannon;
...
import io.zaryx.model.Projectile;
...
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.ALLOWED_REV_AREAS;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.CANNON_PIECES;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.CANNON_SIZE;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.COMPLETE_CANNON_OBJECT_ID;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.MAX_DAMAGE;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.MAX_GRANITE_DAMAGE;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.PLACING_ANIMATION;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.PROHIBITED_CANNON_AREAS;
import static io.zaryx.content.dwarfleaguecannon.CannonConstants.PROJECTILE_ID;
...
public class Cannon {
    ...
    public static void attemptPlace(Player player) {
        if (Arrays.stream(PROHIBITED_CANNON_AREAS).anyMatch(boundary -> boundary.in(player)) &&
        ...
        } else if (!io.zaryx.content.dwarfleaguecannon.CannonRepository.hasDistanceFromOtherCannons(player.getPosition())) {
            ...
            player.setLeagueCannon(new io.zaryx.content.dwarfleaguecannon.Cannon(player.getPosition()));
    ...
    public static boolean clickObject(Player player, int objectId, Position position, int option) {
        if (objectId == COMPLETE_CANNON_OBJECT_ID) {
            if (!io.zaryx.content.dwarfleaguecannon.CannonRepository.exists(position)) {
    ...
    public static boolean clickItem(Player player, int itemId) {
        if (itemId == 26520) {
            io.zaryx.content.dwarfleaguecannon.Cannon.attemptPlace(player);
            return true;
        }
     ...
     */
    private io.zaryx.content.dwarfleaguecannon.CannonRotationState rotationState = io.zaryx.content.dwarfleaguecannon.CannonRotationState.NORTH;
    ...
    public boolean equals(Object o) {
        ...
        io.zaryx.content.dwarfleaguecannon.Cannon cannon = (io.zaryx.content.dwarfleaguecannon.Cannon) o;
    ...
    private void place(Player player) {
        if (io.zaryx.content.dwarfleaguecannon.CannonRepository.add(this)) {
...
/*        if (!operating && player.distance(getPosition()) <= 18 && tickCheck <= 0) {
    ...
    private void rotate() {
        int next = (rotationState.ordinal() + 1) % io.zaryx.content.dwarfleaguecannon.CannonRotationState.values().length;
        rotationState = io.zaryx.content.dwarfleaguecannon.CannonRotationState.values()[next];
    ...
    public List<NPC> getShootableNpcs(Player player) {
        ...
        int rotationStateIndex = (rotationState.ordinal() + 1) % io.zaryx.content.dwarfleaguecannon.CannonRotationState.values().length;
        ...
        for (NPC local : possibleTargets) {
            ...
            switch (io.zaryx.content.dwarfleaguecannon.CannonRotationState.values()[rotationStateIndex]) {
                case NORTH:
