package io.zaryx.content.ytmanager;
...
public class YTManager {
   ...
   /* public static Map<String, ArrayList<String>> USERNAMES_VOTED = new HashMap<>();
    ...
    public static void postVideo(Player player, String videoUrl) {
        new Thread(() -> {
            try {
                ...
                addVideo(video);
                PlayerHandler.executeGlobalMessage("<img=15><shad=ff0000> A new youtube video has been posted by " + channelTitle + ".");
                PlayerHandler.executeGlobalMessage("<img=15><shad=ff0000> Type ::yt and comment your name on the video to get a free reward!");
                open(player);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private static void handleReward(Player player, boolean doubleReward) {
        player.getItems().addItemUnderAnyCircumstance(6199, (doubleReward ? 2 : 1));
        player.sendMessage("You receive " + ItemDef.forId(6199).getName() + " as a reward for commenting on a video!");
    }
    ...
    public static void load() {
        ...
        // Load YouTube videos
        loadVideos(videoQuery);

        // Check votes for loaded videos
        checkVotes(voteCheckQuery);
    }

    private static void loadVideos(String query) {
        try (Connection connection = getConnection();
             PreparedStatement statement = connection.prepareStatement(query);
            ...
            while (results.next()) {
                ...
                if (timeDiff >= 1000 * 60 * 60 * 24 * 3) { // Ignore videos older than 3 days
                    continue;
                }
    ...
    private static void checkVotes(String query) {
        try (Connection connection = getConnection();
             PreparedStatement statement = connection.prepareStatement(query);
             ResultSet results = statement.executeQuery()) {
