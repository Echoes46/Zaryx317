package io.zaryx.content.battlepass;
...
public class Pass {
    ...
    public static void tick() {
        if (System.currentTimeMillis() > daysUntilEnd && !seasonEnded) {
            // End the season and schedule the next one for the 1st of the next month
            seasonEnded = true;
            ...
            daysUntilStart = nextMonth.getTimeInMillis();
        }
    ...
    public static void openInterface(Player player) {
        ...
        int xpNeeded = 25 + ((player.getTier() / 2) * 2);
...
//        if (c.getPosition().inWild() && (npc.getDefinition().getCombatLevel() > 150)
...
//                || Boundary.isIn(c, Boundary.VORKATH)
//                || Boundary.isIn(c, Boundary.KRAKEN_BOSS_ROOM)
//                || Boundary.isIn(c, Boundary.GODWARS_BOSSROOMS) && (npc.getDefinition().getCombatLevel() > 170)
//                || Boundary.isIn(c, Boundary.ZULRAH) && (npc.getDefinition().getCombatLevel() > 100)
...
//                || Boundary.isIn(c, new Boundary(1024, 2496, 3967, 4159))
...
//        }
        ...
        if (c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33217)) {
            exp *= 2;
        }

        c.xp += exp;

        int xpNeeded = 25 + ((c.tier / 2) * 2);

        while (c.xp >= xpNeeded) {
            c.xp -= xpNeeded;
            levelUp(c);
            xpNeeded = 25 + ((c.tier / 2) * 2); // Recalculate xpNeeded for the next tier
        }
    }
