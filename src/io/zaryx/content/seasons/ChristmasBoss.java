...
import static io.zaryx.content.seasons.ChristmasTsnumaiPaths.*;
...
public class ChristmasBoss {
    ...
    public static void initBoss() {
        ...
        PlayerHandler.executeGlobalMessage("<img=28>[@red@C@gre@H@red@R@gre@I@red@S@gre@T@red@M@gre@A@red@S@bla@]<img=28> @pur@ The Evil Snowman Has spawned, ::xmas!");
        ...
        evilSnowman.setNpcAutoAttacks(Lists.newArrayList(
                ...
                new NPCAutoAttackBuilder()
                        ...
                        .setOnHit(attack -> {
                            ...
                            PlayerHandler.getPlayers().stream().filter(plr -> plr != null && Boundary.isIn(plr, new Boundary(2905, 3928, 2928, 3953))).forEach(plr -> {
                                int dam;
                                ...
                                plr.appendDamage(dam, (dam > 0 ? Hitmark.HIT : Hitmark.MISS));
                ...
                new NPCAutoAttackBuilder()
                        ...
                        .setOnHit(attack -> {
                            // Apply damage
                            PlayerHandler.getPlayers().stream().filter(plr ->
...
//                        .setProjectile(new ProjectileBaseBuilder().setProjectileId(-1).setCurve(16).setSpeed(50).setSendDelay(3).createProjectileBase())
    ...
    public static void handleDeath(NPC npc) {
        if (npc.getNpcId() == 2317) {
            spawned = false;
            alive = false;
            PlayerHandler.executeGlobalMessage("<img=28>[@red@C@gre@H@red@R@gre@I@red@S@gre@T@red@M@gre@A@red@S@bla@]<img=28> @pur@ The Evil Snowman Has died!");
