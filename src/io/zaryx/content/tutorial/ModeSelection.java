...
public class ModeSelection {
    ...
    public boolean onClickButton(Player player, int buttonId) {
        switch (buttonId) {
            case 24311:
                ...
                if (player.getRights().isOrInherits(Right.GROUP_WILDYMAN)){
                    player.getRights().remove(Right.GROUP_WILDYMAN);
                }
                player.getRights().reset();
                refreshKit(player, KitData.NORMAL);
                modeType = ModeType.STANDARD;
            ...
            case 24317:
                player.getRights().reset();
                refreshKit(player, KitData.GROUP_WILDYMAN);
                modeType = ModeType.GROUP_WILDYMAN;
                player.setMode(Mode.forType(ModeType.GROUP_WILDYMAN));
                player.getRights().setPrimary(Right.GROUP_WILDYMAN);
                return true;
            case 24318:
            case 24319:
            case 24320:
    ...
    public enum KitData {
        NORMAL(Right.PLAYER, new String[]{
                "Play Zaryx as a casual player.",
                "This game mode has no restrictions at all.",
                "1x, 5x, 10x, 25x experience rates available."
        }, new GameItem[]{new GameItem(11280, 1), new GameItem(544, 1), new GameItem(542, 1), new GameItem(9005, 1), new GameItem(7461, 1), new GameItem(6215, 1), new GameItem(6568, 1), new GameItem(11964, 1), new GameItem(22331, 1)},

                new GameItem(22335, 1), new GameItem(22333, 1), new GameItem(2572, 1), new GameItem(24364, 1),
        ...
        WILDYMAN(Right.WILDYMAN, new String[]{
                ...
                new GameItem(995, 300000), new GameItem(2841, 1), new GameItem(380, 100),
                new GameItem(555, 100), new GameItem(556, 100), new GameItem(558, 100),
                new GameItem(554, 100), new GameItem(557, 100), new GameItem(1265, 1), new GameItem(1351, 1), new GameItem(19564, 1)
        ),

        GROUP_WILDYMAN(Right.GROUP_WILDYMAN, new String[]{
                "25% extra DropRate, Locked to Wilderness as a group.",
                "constant red skull.",
                "(Changeable stats when maxed in specific skill).",
                "Downgrade on death to normal wildyman."
        }, new GameItem[]{new GameItem(11280, 1), new GameItem(544, 1), new GameItem(542, 1), new GameItem(9005, 1), new GameItem(7461, 1), new GameItem(6215, 1), new GameItem(6568, 1), new GameItem(11964, 1), new GameItem(22331, 1)},
