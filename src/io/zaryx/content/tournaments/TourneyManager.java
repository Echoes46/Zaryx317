package io.zaryx.content.tournaments;
...
import io.zaryx.model.entity.player.Coordinate;
...
public class TourneyManager {
    ...
    private static final int FOG_DURATION_TICKS = Misc.toCycles(2, TimeUnit.MINUTES) + Misc.toCycles(30, TimeUnit.SECONDS);
    ...
    private final ArrayList<String> toLeavePlayers = new ArrayList<>();
    private List<Player> toLeaveTargets = Lists.newCopyOnWriteArrayList();
    ...
    public static final String[] tourneyOrder = {"Dharok", "Rune Melee", "Pure", "Pure", "Range AGS", "Monk Robes", "Dharok",
    ...
    public static boolean handleObjects(Player player, WorldObject object, int optionId) {
        if (object == null || object.getId() != 31622 && object.getId() != 39637  && object.getId() != 26645)
            return false;
    ...
    public void handleDeath(String playerName, boolean logout) {
        ...
        if (player != null) {
            ...
            if (player.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
                ...
                player.getPA().forceMove(2096, 6002, 0, logout);
            }
        }
