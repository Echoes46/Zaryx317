...
final class HolidayLayouts {
    ...
    static final int[][] GHOST_ROOMS={{3099,3370},{3106,3364},{3111,3361}};
    static HolidayEvents.Layout round(HolidayEvents.Layout base,int seed) {
        ...
        List<int[]> rooms=new ArrayList<>(Arrays.asList(GHOST_ROOMS));
        int[][] permutations={{0,1,2},{0,2,1},{1,0,2},{1,2,0},{2,0,1},{2,1,0}};
        int[] permutation=permutations[Math.floorMod(seed,6)];
        for(var n:result.npcs)if(!n.home&&n.role>=0){int[] tile=rooms.get(permutation[n.role]);n.x=tile[0];n.y=tile[1];}
        ...
        int shift=Math.floorMod(seed/6,3);
