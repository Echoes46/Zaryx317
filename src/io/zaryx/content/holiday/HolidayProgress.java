...
public final class HolidayProgress {
    ...
    public void useEdition(int value) {
        ...
        edition=value;stage=runs=tokens=gathered=puzzle=delivered=order=0;
    }
    ...
    public String encode() {
        return edition+","+stage+","+runs+","+tokens+","+gathered+","+puzzle+","+delivered+","+order;
    }
    public static HolidayProgress decode(String text) {
        HolidayProgress p=new HolidayProgress();
        String[] values=text.split(",");
        if(values.length!=8)throw new IllegalArgumentException("Invalid holiday progress");
