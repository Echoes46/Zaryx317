...
public class equip extends Command {
    ...
    private static final int[] range_weapons = {28869, 19481, 26712, 19478, 12788, 9185, 11785, 21012, 26374, 33206, 26269, 839, 845, 847, 851, 855, 859, 841, 843, 849,
            853, 857, 12424, 22333, 861, 4212, 4214, 4215, 12765, 12766, 12767, 12768, 11235, 4216, 4217, 4218, 4219, 4220,
            4221, 4222, 4223, 4734, 6724, 33005, 29000, 20997, 33005, 21902, 22550, 25916, 25918, 26486, 25865, 25867, 25884, 25888, 33058, 33207, 20484, 25890, 25892, 25894, 25896,27655, 39000,
            //corrupted bow
...
//        c.getPA().itemOnInterface(773,1, 65022, 0);
        ...
        /*if (c.saradominFaction) {
            c.getPA().itemOnInterface(12637, 1, 27664, 0);
            c.wearSlot = 0;
            return;
        }

        if (c.zamorakFaction) {
            c.getPA().itemOnInterface(12638, 1, 27664, 0);
            return;
        }

        if (c.guthixFaction) {
            c.getPA().itemOnInterface(12639, 1, 27664, 0);
            return;
    ...
    public static void getGear(Player c) {
        ...
        if (c.playerEquipment[Player.playerFeet] > 0) {
            c.getPA().itemOnInterface(c.playerEquipment[Player.playerFeet], 1, 65028,0);
        } else {
