package io.zaryx.content.minigames.barrows.brothers;

import io.zaryx.content.minigames.barrows.RewardItem;
...
public abstract class Brother {
	...
	public void spawnBrother() {
		if (defeated) {
			if (finalBrother) {
				player.sendMessage("Something went wrong with the final Barrows brother. Please report this issue on the discord.");
				return;
			} else {
		...
		active = true;
		if (finalBrother) {
			npc = NPCSpawning.spawnNpcOld(player, getId(), 3551, 9694, 0, 1, getHP(), getMaxHit(), getAttack(), getDefense(), true, true);
		} else {
	...
	public abstract Position getStairsId();
