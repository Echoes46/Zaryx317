package io.zaryx.content.minigames.raids;

import io.zaryx.content.party.PartyFormAreaController;
import io.zaryx.content.party.PlayerParty;
import io.zaryx.model.entity.player.Boundary;

import java.util.Set;

public class WildPartyCoxController extends PartyFormAreaController {

    @Override
    public String getKey() {
        return WildPartyCox.TYPE;
    }

    @Override
    public Set<Boundary> getBoundaries() {
        return Set.of(Boundary.WILD_COXPARTY);
    }

    @Override
    public PlayerParty createParty() {
        return new WildPartyCox();
    }
}
