...
import static io.zaryx.content.minigames.raids.RaidConstants.checkInstances;
...
//
    ...
    public void leaveGameWild(Player player) {
        if (System.currentTimeMillis() - player.infernoLeaveTimer < 15000) {
            player.sendMessage("You cannot leave yet, wait a couple of seconds and try again.");
            return;
        }
    ...
    public void spawnRaidsNpc(int npcType, int x, int y, int heightLevel, int WalkingType, int HP, int maxHit, int attack, int defence, boolean attackPlayer) {
        ...
        // Scale the base HP by the number of players
        int scaledHP = HP * playerCount;

        // Add 20% extra health
        scaledHP += (int) (scaledHP * 0.1);

        int[] stats = getScaledStats(scaledHP, attack, defence, playerCount);
        attack = stats[0];
        defence = stats[1];
        scaledHP = stats[2];

        NPC npc = NPCSpawning.spawn(npcType, x, y, heightLevel, WalkingType, maxHit, attackPlayer,
                NpcStats.builder().setAttackLevel(attack).setHitpoints(scaledHP).setDefenceLevel(defence).createNpcStats());
    ...
    public void spawnNpcs(int currentRoom) {
        ...
        switch(roomNames.get(currentRoom)) {
            ...
            case "vespula":
                ...
                spawnRaidsNpc(7538, 3306, 5483, height, 1, 250, 20, 350, 220, true);
                spawnRaidsNpc(7538, 3306, 5489, height, 1, 250, 20, 350, 220, true);
                spawnRaidsNpc(7531, 3305, 5486, height, 1, 800, 40, 450, 220, true);
                mobAmount += 3;
                vespula = true;
                break;
            case "olm":
                    ...
                    if (!player.getRaidRewards().isEmpty()) {
                        if (player.getItems().getInventoryCount(21046) > 0) {
                            ...
                            if (Misc.isLucky(5)) {
                                ...
                                player.sendErrorMessage("WINNER...Kinda?");
                            }
                        }
                    }
                }
