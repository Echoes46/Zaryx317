package io.zaryx.content.minigames.inferno;
...
public class Inferno extends Tzkalzuk {
    ...
    public static void gamble(Player player) {
        ...
        if (Misc.random(0,40) == 1) {
            if (!player.getItems().hasAnywhere(25910, 25912) && !player.getItems().hasInBank(33335) && !player.getItems().hasInBank(25912)) {
                player.getItems().addItemUnderAnyCircumstance(25910, 1);
                PlayerHandler.executeGlobalMessage("[@red@Slayer@bla@] @cr20@<col=255> " + player.getDisplayName() + "</col> received Zuk's Tooth from <col=255>TzKal-Zuk</col>.");
            }
        }
