package io.zaryx.content.minigames.tob.instance;
...
import io.zaryx.util.logging.player.DiedAtTobLog;
...
public class TobInstance extends InstancedArea {
    ...
    public void moveToNextRoom(Player player) {
        if (getCurrentRoom().isRoomComplete(this) || getPlayerRoomIndex(player) < roomIndex) {
            ...
            int height_2 = playerSpawnPosition.getHeight();

            if (player.debugMessage) {
                player.sendErrorMessage("Current Height Level: " + height + " / " + height_2 + " / " + nextRoomIndex + " / " + player.getInstance().getHeight());
            }

            if (nextRoomIndex == 4) {
                if (player.getParty().getOwner().isPresent() && player.getParty().getOwner().get() == player) {
                    player.moveTo(new Position(playerSpawnPosition.getX(), playerSpawnPosition.getY(), playerSpawnPosition.getHeight()));
                } else if (player.getParty().getOwner().isPresent()) {
                    player.moveTo(player.getParty().getOwner().get().getPosition());
                }
            } else {
                player.moveTo(new Position(playerSpawnPosition.getX(), playerSpawnPosition.getY(), playerSpawnPosition.getHeight()));
            }
