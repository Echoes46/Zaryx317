...
public class BasicBot {
    ...
    private static void miniCowBot(Player player) {
        ...
        CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            @Override
            public void execute(CycleEventContainer container) {
                List<NPC> npcList = Arrays.stream(NPCHandler.npcs)
                        .filter(npc1 -> npc1 != null && npc1.underAttackBy <= 0 &&
    ...
    private static void miniRockBot(Player player) {
        ...
        CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            @Override
            public void execute(CycleEventContainer container) {
                List<NPC> npcList = Arrays.stream(NPCHandler.npcs).filter(Objects::nonNull).filter(npc1 -> npc1.underAttackBy <= 0).filter(npc1 -> npc1.getPosition().withinDistance(player.getPosition(), 10)).collect(Collectors.toList());
    ...
    private static void handleRandomDonation() {
        ...
        Player p = PlayerHandler.getPlayers().stream().filter(Player::isBot)
                .collect(Collectors.toList())
                .get(Misc.random((int) PlayerHandler.getPlayers().stream().filter(Player::isBot).count()));
