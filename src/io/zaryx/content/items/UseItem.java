...
import io.zaryx.content.PetCollector;
import io.zaryx.content.PlatinumTokens;
...
import io.zaryx.content.bosses.Tempoross;
...
import io.zaryx.content.combat.magic.Armadyl;
...
import io.zaryx.content.item.lootable.impl.AOEChest;
import io.zaryx.content.item.lootable.impl.CrystalChest;
import io.zaryx.content.item.lootable.impl.DZchest;
import io.zaryx.content.item.lootable.impl.FoeMysteryBox;
import io.zaryx.content.items.item_combinations.Godswords;
...
import io.zaryx.content.skills.Cooking;
import io.zaryx.content.skills.Skill;
import io.zaryx.content.skills.crafting.BattlestaveMaking;
import io.zaryx.content.skills.crafting.BraceletMaking;
import io.zaryx.content.skills.crafting.BryophytaStaff;
import io.zaryx.content.skills.crafting.GemCutting;
import io.zaryx.content.skills.crafting.GlassBlowing;
import io.zaryx.content.skills.crafting.JewelryMaking;
import io.zaryx.content.skills.crafting.LeatherMaking;
import io.zaryx.content.skills.crafting.SpinMaterial;
import io.zaryx.content.skills.firemake.Firemaking;
import io.zaryx.content.skills.fishing.ArielFishing;
import io.zaryx.content.skills.herblore.Crushable;
import io.zaryx.content.skills.herblore.HerbloreDecantCleanUnfNpc;
import io.zaryx.content.skills.herblore.PoisonedWeapon;
import io.zaryx.content.skills.herblore.PotionDecanting;
...
import io.zaryx.util.logging.player.ItemOnItemLog;
...
public class UseItem {
    ...
    public static void resetLampState(Player player) {
        if (player == null) {
            return;
        }
        ...
        player.normalLamp = false;
        player.antiqueLamp = false;
        player.lampItemUsed = -1;
    }

    public static boolean consumeLampItem(Player player) {
        if (player == null) {
            return false;
        }

        int itemId = player.lampItemUsed;
        if (itemId <= 0) {
            player.sendMessage("@red@No lamp item was selected.");
            resetLampState(player);
            player.getPA().closeAllWindows();
            return false;
        }

        if (!player.getItems().playerHasItem(itemId, 1)) {
            player.sendMessage("@red@You no longer have the required item.");
            resetLampState(player);
            player.getPA().closeAllWindows();
            return false;
        }
                ...
                switch (itemId) {
                    case 12924:
                        if (c.getItems().playerHasItem(12924)
                                && c.getItems().playerHasItem(29237)
                                && c.getItems().playerHasItem(2949)) {
                            c.startAnimation(898);
                            c.getItems().deleteItem2(12924, 1);
                            c.getItems().deleteItem2(29237, 1);
                            c.getItems().deleteItem2(2949, 1);
                            //if (Misc.isLucky(5)) {
                            c.getItems().addItemUnderAnyCircumstance(28688, 1);
                            PlayerHandler.executeGlobalMessage("[<col=FFD700>FORGE@bla@] @red@" + c.getDisplayName() + " has unlocked the Blazing blowpipe!");
                        } else c.sendMessage("You need a Seeping Wing and a Golden Hammer to make this");
                        break;

                    case 10887:
                        if (c.getItems().playerHasItem(10887)
                                && c.getItems().playerHasItem(28357)
                                && c.getItems().playerHasItem(2949)) {
                            c.startAnimation(898);
                            c.getItems().deleteItem2(10887, 1);
                            c.getItems().deleteItem2(28357, 1);
                            c.getItems().deleteItem2(2949, 1);
                            //if (Misc.isLucky(5)) {
                            c.getItems().addItemUnderAnyCircumstance(27855, 1);
                            PlayerHandler.executeGlobalMessage("[<col=FFD700>FORGE@bla@] @red@" + c.getDisplayName() + " has unlocked the Strongman's Anchor!");
                        } else c.sendMessage("You need Black quartz and a Golden Hammer to make this");
                        break;
                    case 11824:
                        if (c.getItems().playerHasItem(11824)
                                && c.getItems().playerHasItem(28864)
                                && c.getItems().playerHasItem(2949)) {
                            c.startAnimation(898);
                            c.getItems().deleteItem2(11824, 1);
                            c.getItems().deleteItem2(28864, 1);
                            c.getItems().deleteItem2(2949, 1);
                            //if (Misc.isLucky(5)) {
                            c.getItems().addItemUnderAnyCircumstance(28988, 1);
                            PlayerHandler.executeGlobalMessage("[<col=FFD700>FORGE@bla@] @red@" + c.getDisplayName() + " has unlocked the Tainted Spear!");
                        } else c.sendMessage("You need Subdued Tar and a Golden Hammer to make this");
                        break;
    ...
    public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
        ...
        if (GodBooks.mix(c, itemUsed, useWith))
            return;
        ...
        if (itemUsed == 5066 && useWith == 23073|| useWith == 24370 || itemUsed == 25898 && useWith == 25910 || useWith == 25904) {
            if (c.getItems().playerHasItem(5066) && c.getItems().playerHasItem(23073) && c.getItems().playerHasItem(25898) && c.getItems().playerHasItem(25910) && c.getItems().playerHasItem(25904) &&
                    c.getItems().playerHasItem(24370)) {
                c.getItems().deleteItem(5066, 1);
                c.getItems().deleteItem(23073, 1);
                c.getItems().deleteItem(24370, 1);
                c.getItems().deleteItem(25898, 1);
                c.getItems().deleteItem(25910, 1);
                c.getItems().deleteItem(25904, 1);
                c.getItems().addItem(33333, 1);

                c.getDH().sendStatement("@blu@You have created the Helm of all realms.");
            } else {
                c.sendMessage("You need all 5 superior slayer helmets to create the helm of all realms.");
            }
            return;
        }


        if (itemUsed == 22088 && useWith == 22089|| useWith == 22090 || itemUsed == 22091) {
            if (c.getItems().playerHasItem(22088) && c.getItems().playerHasItem(22089) && c.getItems().playerHasItem(22090) && c.getItems().playerHasItem(22091)) {
                c.getItems().deleteItem(22088, 1);
                c.getItems().deleteItem(22089, 1);
                c.getItems().deleteItem(22090, 1);
                c.getItems().deleteItem(22091, 1);
                c.getItems().addItem(22087, 1);

                c.getDH().sendStatement("@blu@You have crafted the demon's token");
            } else {
                c.sendMessage("You need all 4 pieces to create the token.");
            }
            return;
        }
        ...
        if (itemUsed == 24466 && useWith == 11864) { //superslayerhelms
            c.getItems().deleteItem(useWith, 1);
            c.getItems().deleteItem(itemUsed, 1);
            c.getItems().addItem(24370, 1);
        }

        if (itemUsed == 8125 && useWith == 11864) { //superslayerhelms
            c.getItems().deleteItem(useWith, 1);
            c.getItems().deleteItem(itemUsed, 1);
            c.getItems().addItem(25904, 1);
        }
        if (itemUsed == 23077 && useWith == 11864) { //superslayerhelms
            c.getItems().deleteItem(useWith, 1);
            c.getItems().deleteItem(itemUsed, 1);
            c.getItems().addItem(23073, 1);
        }
        if (itemUsed == 33334 && useWith == 11864) { //superslayerhelms
            c.getItems().deleteItem(useWith, 1);
            c.getItems().deleteItem(itemUsed, 1);
            c.getItems().addItem(25898, 1);
        }
        if (itemUsed == 33335 && useWith == 11864) { //superslayerhelms
            c.getItems().deleteItem(useWith, 1);
            c.getItems().deleteItem(itemUsed, 1);
            c.getItems().addItem(25910, 1);
        }
        ...
        if (itemUsed == 1755 && ArmorSets.isSet(useWith) || ArmorSets.isPackSet(itemUsed) && useWith == 1755) {
...
//		if (Thammaron.useItem(c, itemUsed, useWith)) {
//			return;
//		}
        if (Armadyl.useItem(c, itemUsed, useWith)) {
            return;
        }
        ...
        //corrupted
        if (itemUsed == 23837 && useWith == 27690) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(27690)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(27690, 1);
                c.getItems().addItem(28531, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 13652) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(13652)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(13652, 1);
                c.getItems().addItem(28534, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 11802) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(11802)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(11802, 1);
                c.getItems().addItem(28537, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 20997) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(20997)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(20997, 1);
                c.getItems().addItem(28540, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 22325) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(22325)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(22325, 1);
                c.getItems().addItem(28543, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 23971) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(23971)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(23971, 1);
                c.getItems().addItem(23842, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 23975) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(23975)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(23975, 1);
                c.getItems().addItem(23845, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 23979) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(23979)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(23979, 1);
                c.getItems().addItem(23848, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 25865) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(25865)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(25865, 1);
                c.getItems().addItem(25867, 1);
            }
        }
        if (itemUsed == 23837 && useWith == 22555) {
            if (c.getItems().playerHasItem(23837) && c.getItems().playerHasItem(22555)) {
                c.getItems().deleteItem(23837, 1);
                c.getItems().deleteItem(22555, 1);
                c.getItems().addItem(23854, 1);
            }
        }
        ...
        if (itemUsed == 28583 && useWith == 28593 || itemUsed == 28593 && useWith == 28583) {
            if (!c.getItems().playerHasItem(28583)) {
                c.sendMessage("You need the uncharged toxic staff of the dead to do this.");
                return;
            }
            if (c.getToxicStaffOfTheDeadCharge() > 0) {
            ...
            int amount = c.getItems().getItemAmount(28593);
            if (amount > 20000) {
                amount = 20000;
                c.sendMessage("The staff only required 20000 extracts to fully charge.");
            }
            c.getItems().deleteItem2(28593, amount);
            c.getItems().deleteItem2(28583, 1);
            c.getItems().addItem(28585, 1);
            c.setToxicStaffOfTheDeadCharge(amount);
            c.sendMessage("You charge the warped sceptre for " + amount + " extract");
            return;
        }
...
//		UseItem.ItemonItem(c, itemUsed, useWith, itemUsedSlot, usedWithSlot);
    ...
    public static void handleItemOnNPC(Player player, NPC npc, int itemId, int slot) {
        ...
        if (npcId == 5816) {
            if (NPCHandler.npcs[player.npcClickIndex].spawnedBy == player.getIndex() && player.hasPetSpawned) {
                boolean hasstoragepet = PetHandler.hasstoragepetout(player);
                if(hasstoragepet){
                    player.getPakYak().deposit(itemId, 1,true);
        ...
        if (npcId == 1504) {//lamp on hunter for hunter xp
            if (itemId == 2528) {
                if (player.getItems().playerHasItem(2528, 1)) {
                    ...
                    player.getPA().addSkillXPMultiplied(5000 * amt, Skill.HUNTER.getId(), true);
