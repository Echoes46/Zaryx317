...
public class Firemaking {
    ...
    public static void lightFire(final Player player, final int logUsed, final String usage) {
        ...
        if (usingTinderbox && Boundary.isIn(player, Boundary.EDGE_TRADING_AREA)) {
        ...
        if (System.currentTimeMillis() - player.lastFire < 1800 && usingTinderbox) {
            return;
        }

        if (player.playerIsFiremaking && usingTinderbox) {
            return;
        }
        ...
        if (usingTinderbox && !player.getItems().playerHasItem(logUsed)) {
        ...
        double osrsExperience = log.getExperience() + log.getExperience() / 10 * pieces;
