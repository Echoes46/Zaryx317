package io.zaryx.content.skills.runecrafting.ouriana;
...
public class OurianaAltar {
    ...
    public static boolean clickObject(Player player, WorldObject worldObject) {
        if (worldObject.getId() == 29_631) {
            ...
            if (worldObject.getId() == 34_837) {
                if (!CycleEventHandler.getSingleton().getEvents().isEmpty()) {
                    CycleEventHandler.getSingleton().stopEvents(player);
                }
                CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.GENEROUSRC, player, new CycleEvent() {
                    @Override
                    public void execute(CycleEventContainer container) {
                        ...
                        if (player.getInventory().containsAll(new ImmutableItem(Items.PURE_ESSENCE_NOTED, 1))) {
                            int essence = player.getItems().getItemAmount(Items.PURE_ESSENCE_NOTED);
                            if (essence >= 20) {
                                player.getEventCalendar().progress(EventChallenge.COMPLETE_X_RUNS_AT_THE_ZMI_ALTAR, 1);
                            }
                            for (TaskMasterKills taskMasterKills : player.getTaskMaster().taskMasterKillsList) {
                            ...
                            player.getItems().deleteItem2(Items.PURE_ESSENCE_NOTED, essence);
                            player.gfx100(186);
                            player.startAnimation(791);
                            List<ImmutableItem> runes = Lists.newArrayList();
                            int xpGained = 0;
                            for (int count = 0; count < essence; count++) {
                                ...
                                int runeId = runePool[Misc.trueRand(runePool.length)];
                                Runecrafting.RunecraftingData runecraftData = Objects.requireNonNull(Runecrafting.RunecraftingData.forId(runeId));
                                xpGained += (runecraftData.getExperience() * 1.5d);
                                runes.add(new ImmutableItem(runeId, (1 + Misc.trueRand(2)) * runeMultiplier));
                            }
