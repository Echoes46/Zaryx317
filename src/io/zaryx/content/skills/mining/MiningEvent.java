package io.zaryx.content.skills.mining;
...
import io.zaryx.content.achievement_diary.impl.DesertDiaryEntry;
...
public class MiningEvent extends Event<Player> {
	...
	public void update() {
		...
		if (super.getElapsedTicks() - lastAnimation > ANIMATION_CYCLE_DELAY) {
			...
			attachment.getPA().sendSound(432, 1, 0, 0);
		}
	}
	...
	public void execute() {
		...
		/*if (mineral == Mineral.CRYSTAL_TREE) {
		...
		if (Misc.random(mineral.getPetChance()) / dropRate == 7) {
		...
		if (Misc.random(mineral.getPetChance()) / dropRate == 3) {
			attachment.getItems().addItemUnderAnyCircumstance(20362, clueAmount);
			attachment.sendMessage("@blu@You appear to see a clue geode fall within the rock, and pick it up.");

		}

		if (Misc.random(mineral.getPetChance()) / dropRate == 5) {
		...
		int outfitRate = attachment.skillingPetRateScroll ? (int) (mineral.getPetChance() * .75) : mineral.getPetChance();
		if (Misc.random(outfitRate) == 2) {
			attachment.sendMessage("@blu@You appear to see a prospector helmet in the rock and proceed to pick it up");
			attachment.getItems().addItemUnderAnyCircumstance(12013, 1);
		}
		if (Misc.random(outfitRate) == 2) {
			attachment.sendMessage("@blu@You appear to see a prospector jacket in the rock and proceed to pick it up");
			attachment.getItems().addItemUnderAnyCircumstance(12014, 1);
		}
		if (Misc.random(outfitRate) == 2) {
			attachment.sendMessage("@blu@You appear to see a prospector legs in the rock and proceed to pick it up");
			attachment.getItems().addItemUnderAnyCircumstance(12015, 1);
		}
		if (Misc.random(outfitRate) == 2) {
			attachment.sendMessage("@blu@You appear to see a prospector boots in the rock and proceed to pick it up");
			attachment.getItems().addItemUnderAnyCircumstance(12016, 1);
		}
	}
