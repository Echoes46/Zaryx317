...
public class Farming {
	...
	public void doConfig() {
		FarmingPatches[] patches = {
				...
				FarmingPatches.DZ250,
				FarmingPatches.DZ_HERB_NEW,
				FarmingPatches.WILDYHOME1,
				FarmingPatches.WILDYCAVE1,
				FarmingPatches.WILDYCAVE2,
				FarmingPatches.WILDYCAVE3
		};
		...
		switch (closest) {
			case DZ_HERB:
				...
				break;
			case WILDYHOME1:
				config = (config(FarmingPatches.WILDYHOME1) << 24);
				break;
			case DZ250:
				config = (config(FarmingPatches.DZ250) << 24)
						+ (config(FarmingPatches.DZ250N) << 8)
						+ (config(FarmingPatches.DZ250S));
				break;
			case WILDYCAVE1:
				config = (config(FarmingPatches.WILDYCAVE1) << 24);
				break;
			case WILDYCAVE2:
				config = (config(FarmingPatches.WILDYCAVE2) << 24);
				break;
			case WILDYCAVE3:
				config = (config(FarmingPatches.WILDYCAVE3) << 24);
				break;
			case DZ_HERB_NEW:
					...
					if (plant.seed == seed) {
						if (player.getPA().getLevelForXP(player.playerXP[Skill.FARMING.getId()]) >= plant.level) {
							...
							if (player.getItems().playerHasItem(patch.planter)) {
								...
								doConfig();
								player.getPA().addSkillXPFromAction((int)(plant.plantExperience * 10), Skill.FARMING.getId(), true);
							} else {
