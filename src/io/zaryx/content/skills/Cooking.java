...
import io.zaryx.content.achievement_diary.impl.WesternDiaryEntry;
...
public class Cooking extends SkillHandler {
	...
	public enum cook {
		...
		SHARKNOTED(384, 386),
		SHRIMPNOTED(318, 316),
		ANCHOVIESNOTED(322, 320),
		LOBSTERNOTED(378, 380),
		TROUTNOTED(336, 334),
		SALMONNOTED(332, 330),
		KARAMBWANNOTED(3143, 3145),
		SWORDFISHNOTED(372, 374),
		TUNANOTED(360, 362),
		SARDINENOTED(328, 326),
		BASSNOTED(364, 366),
		LAVAEELNOTED(2149, 2150),
		MONKFISHNOTED(7945, 7947),
		MANTARAYNOTED(390, 392),
		ANGLERFISHNOTED(13440, 13442),
		DARKCRABNOTED(11935, 11937);
	...
	public static boolean cookThisFood(Player p, int i, int object) {
		switch (i) {
			...
			case 384:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 160, 81, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 318:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 20, 1, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 322:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 22, 1, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 378:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 70, 40, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 336:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 30, 20, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 332:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 35, 30, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 3143:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 80, 55, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 372:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 70, 50, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 360:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 60, 35, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 328:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 25, 5, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 364:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 50, 55, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 2149:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 200, 89, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 7945:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 130, 61, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }

				case 390:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 210, 90, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 13440:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 220, 91, 370, i + 2, object);
				} else { p.sendMessage("You need to have donated $2500 to use this feature"); }
			case 11935:
				if (p.amDonated > 2499) {
					return cookFish(p, i, 200, 89, 370, i + 2, object);
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
			...
			public void execute() {
				...
				if (attachment.playerSkillProp[7][6] >= fishStopsBurning(attachment.playerSkillProp[7][0]) || cookFish(c)) {
					...
					attachment.getPA().addSkillXPFromAction(attachment.playerSkillProp[7][1]*xpBonus(attachment), Player.playerCooking, true);
					...
					attachment.getPA().sendSound(1039,0,0,0);
				} else {
					attachment.getPA().sendSound(240,0,0,0);
