 ...
 */
	...
	public void steal(Pickpocket pickpocket, NPC npc) {
		...
		foeArtefact(player);
		if (Boundary.isIn(player, Boundary.WILDYMAN_CAVE )) {
			if (Misc.random(20) == 5) {
				int randomAmount = 3;
				player.sendMessage("You received " + randomAmount + " pkp while Thieving!");
				player.getItems().addItem(2996, randomAmount);
			}
		}
		...
		if (Boundary.isIn(player, Boundary.WILDYMAN_CAVE )) {
			if (Misc.random(20) == 5) {
				int randomAmount = 3;
				player.sendMessage("You received " + randomAmount + " Blood money while Thieving!");
	...
	public enum Pickpocket {
		...
		DRUNKEN_DWARF(35, 90, 2500, new HashMap<Rarity, List<GameItem>>() {
			{
