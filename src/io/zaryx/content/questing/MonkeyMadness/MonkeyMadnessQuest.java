...
public class MonkeyMadnessQuest extends Quest {
    ...
    public List<String> getJournalText(int stage) {
        ...
        switch (stage) {
            case 0:
                lines.add("I must find King Narnode Shareen");
                lines.add("Pretty big tree for a small guy they said?");
                lines.add("");
                lines.add("Rewards:");
                lines.add("Access to dragon scimitar shop.");
                lines.add("Experience lamp x 2.");
                lines.add("1.5m UpgradePoints.");
                lines.addAll(getStartRequirementLines());
                break;
            case 1:
                lines.add("i should check in with his spy at Ape Atoll.");
                lines.add("use the Gnome Glider at the top of the tree.");
                break;
            case 2:
            ...
            case 3:
                ...
                break;
            case 4:
                lines.add("The demon's dead! I should report back to the king.");
                break;
            case 5:
    ...
    public List<String> getCompletedRewardsList() {
        return Lists.newArrayList("Access to the King's shop.", "2 Experience Lamps.", "500k UpgradePoints");
    }
                ...
                if (getStage() == 0) {
                    if (!canStartQuest()) {
                        player.start(getKingNarnodeDialogue()
                                .npc(DialogueExpression.ANGER_1, "You need at least 2 quest points to start this quest.")
                        );
                        return false;
                    }
                    player.start(getKingNarnodeDialogue()
