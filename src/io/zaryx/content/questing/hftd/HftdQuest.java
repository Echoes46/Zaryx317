...
public class HftdQuest extends Quest {
    ...
    public List<String> getJournalText(int stage) {
        ...
        switch (stage) {
            case 0:
                lines.add("You need to get to the lighthouse!");
                lines.add("Speak to Jossik to start the quest");
                lines.add("");
                lines.add("Rewards:");
            ...
            case 1:
                ...
                break;
            case 2:
                lines.add("I should talk to Jossik again.");
                break;
            case 3:
    ...
    public List<String> getCompletedRewardsList() {
        return Lists.newArrayList("Access to God Books.", "Ability to kill the Dagannoth mother.", "2 Experience Lamps.", "1 Quest Point");
    }
    ...
    public boolean handleNpcClick(NPC npc, int option) {
        if (npc.getNpcId() == Npcs.JOSSIK) {
            if (option == 1) {
                if (getStage() == 0) {
                    if (!canStartQuest()) {
                        player.start(getJossikDialogue().npc(DialogueExpression.ANGER_1, "You need at least " + getRequiredQuestPoints() + " quest points to start this quest."));
                        return false;
                    }
