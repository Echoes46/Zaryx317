...
public abstract class MysteryBoxLootable implements Lootable {
    ...
    public void setMysteryPrize() {
        double[] rarityProbabilities = {0.02, 0.12, 0.39, 0.90}; // Very Rare, Rare, Uncommon, Common
        double random = Math.random();
        List<GameItem> itemList = null;

        if (getItemId() == 19897 || getItemId() == 22087) {
            ...
            if (itemList == null || itemList.isEmpty()) {
                itemList = getLoot().get(MysteryBoxRarity.COMMON.getLootRarity()); // Fallback to Common
            }
    ...
    public void quickOpen() {
        ...
        if (player.getUltraInterface().isActive() ||
                ...
                player.getDragonToken().isActive() ||
        ...
        if (!(player.getSuperMysteryBox().canMysteryBox) || !(player.getNormalMysteryBox().canMysteryBox) ||
                ...
                !(player.getDragonToken().canMysteryBox)||
