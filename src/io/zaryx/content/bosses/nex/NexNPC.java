package io.zaryx.content.bosses.nex;

import io.zaryx.content.bosses.hydra.CombatProjectile;
import io.zaryx.content.bosses.nex.attacks.*;
...
public class NexNPC {
    ...
    public static int attackSpeed = 4;
    ...
    public static void updateTargets() {
        targets = PlayerHandler.getPlayers().stream().filter(plr -> !plr.isDead && Boundary.isIn(plr, Boundary.NEX)).collect(Collectors.toList());
    }
...
//        setAttacks();
        switch (newPhase) {
            ...
            case ZAROS:
                ...
                npc.appendHeal(500, Hitmark.HEAL_PURPLE);
                break;
        }
    }
...
//                    this.startAnimation(9181); // 9175 = walk / 9176 = speed walk / 9178 = Fly?
    ...
    static Player getRandomTarget() {
        ...
        return Misc.random(PlayerHandler.getPlayers().stream().filter(plr -> !plr.isDead && Boundary.isIn(plr, Boundary.NEX)).collect(Collectors.toList()));
    }
