package io.zaryx.content.bosses.dukesucellus;
...
public class DukeSucellus extends LegacySoloPlayerInstance {
    ...
    public static void enter(Player player, DukeSucellus instance) {
        try {
            ...
            NPC duke = NPCSpawning.spawnNpc(player, 12166, 3036, 6452, instance.getHeight(), 0, 30, false, false);
    ...
    public static void changeVats(GlobalObject object, Player p) {
        if (object.getObjectId() == 47536) {
            object.setId(47539);
        } else if (object.getObjectId() == 47539) {
            object.setId(47536);
        }
    ...
    private static int wakeupPotion = 1;

    public static void wakeDuke(NPC duke, Player c) {
        c.getItems().deleteItem2(28351, 1);
        wakeupPotion--;
        if (wakeupPotion <= 0) {
            duke.requestTransform(12191);
            NPC bigDuke = NPCSpawning.spawnNpc(c, 12191, duke.getX(), duke.getY(), duke.getInstance().getHeight(), 0, 35, true, false);
            bigDuke.facePosition(3039,6450);
            CycleEventHandler.getSingleton().stopEvents(duke);
            duke.getInstance().add(bigDuke);
            duke.unregisterInstant();
            wakeupPotion = 1;
        }
    }
    ...
    public static void handleDukeAttacks(NPC duke) {
        ...
        if (rng <= 5) {
            slapFloorAttack(duke);
        } else if (rng > 5 && rng < 8) {
            duke.startAnimation(10178);
            gasFlare(duke);
        } else {
            basicAttackHandler(duke);
        }
    }
        ...
        for (Player player : duke.getInstance().getPlayers()) {
            if (player != null) {
                ...
                if (player.protectingMagic()) {
                    player.appendDamage(duke, Misc.random(1, 25), Hitmark.HIT);
                } else {
                    player.appendDamage(duke, Misc.random(1, 50), Hitmark.HIT);
                }
            }
        }
    }
    ...
    private static void dealDamageToPlayers(Position position, NPC duke) {
        if (duke.getInstance() == null) {
            return;
        }
        if (duke.getInstance().getPlayers().isEmpty()) {
            return;
        }
        for (Player player : duke.getInstance().getPlayers()) {
