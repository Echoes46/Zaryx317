...
public class Hespori {
	...
	private static final List<HesporiBonus> bonuses = Lists.newArrayList(new AttasBonus(), new IasorBonus(), new KronosBonus(),
			new BuchuBonus(), new CelastrusBonus(), new GolparBonus(), new KeldaBonus(), new NoxiferBonus(), new ConsecrationBonus(), new AchieveBonus());
	...
	public static final int Y = 4380;
	...
	public static void rewardPlayers(boolean eventCompleted) {
		...
		.forEach(p -> {
					if (eventCompleted && (p.getMode().getType() == ModeType.WILDYMAN || p.getMode().getType() == ModeType.GROUP_WILDYMAN)) {
						p.getPA().startTeleport2(3440, 3836, 0);
					}
					if (!eventCompleted) {
						p.sendMessage("@blu@Hespori event was ended before she was killed!");
						p.canLeaveHespori = true;
						if (p.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || p.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN))) {
							p.getPA().startTeleport2(3448, 3838, 0);

						} else {
							p.getPA().startTeleport2(2095, 6002, 0);
...
/*		if (c.getLevelForXP(c.playerXP[20]) < 50 || c.getLevelForXP(c.playerXP[13]) < 50) {
	...
	public static long ATTAS_TIMER, KRONOS_TIMER, IASOR_TIMER, GOLPAR_TIMER, BUCHU_TIMER, NOXIFER_TIMER, KELDA_TIMER, CELASTRUS_TIMER, CONSECRATION_TIMER, ACHIEVE_TIMER;
			...
			while(sc.hasNextLine()){
				...
				switch (i) {
					...
					case 9:
						...
						break;
					case 10:
						ACHIEVE_TIMER = (int) Long.parseLong(amount);
						break;
				}
			}
