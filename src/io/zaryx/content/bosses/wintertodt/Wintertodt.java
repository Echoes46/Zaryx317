package io.zaryx.content.bosses.wintertodt;
...
public class Wintertodt {
    ...
    private static final int SNOW_EFFECT = 1311;
    ...
    private static boolean delayRequired = false;
    ...
    private static void extinguishBraziers() {
        ...
        if (Server.getGlobalObjects().get(BURNING_BRAZIER,1620, 3997, 0) != null && Server.getGlobalObjects().get(BURNING_BRAZIER,1620, 3997, 0).getObjectId() == BURNING_BRAZIER && Misc.isLucky(5)) {
        ...
        } if (Server.getGlobalObjects().get(BURNING_BRAZIER,1620, 4015, 0) != null && Server.getGlobalObjects().get(BURNING_BRAZIER,1620, 4015, 0).getObjectId() == BURNING_BRAZIER && Misc.isLucky(5)) {
            globalObject = Server.getGlobalObjects().get(BURNING_BRAZIER,1620, 4015, 0);
            breakBrazier(globalObject);
            Server.playerHandler.sendStillGfx(new StillGraphic(502, globalObject.getPosition()), globalObject.getPosition());
        } if (Server.getGlobalObjects().get(BURNING_BRAZIER,1638, 4015, 0) != null && Server.getGlobalObjects().get(BURNING_BRAZIER,1638, 4015, 0).getObjectId() == BURNING_BRAZIER && Misc.isLucky(5)) {
            globalObject = Server.getGlobalObjects().get(BURNING_BRAZIER,1638, 4015, 0);
            breakBrazier(globalObject);
            Server.playerHandler.sendStillGfx(new StillGraphic(502, globalObject.getPosition()), globalObject.getPosition());
        } if (Server.getGlobalObjects().get(BURNING_BRAZIER,1638, 3997, 0) != null && Server.getGlobalObjects().get(BURNING_BRAZIER,1638, 3997, 0).getObjectId() == BURNING_BRAZIER && Misc.isLucky(5)) {
    ...
    private static void breakBrazier(GlobalObject b) {
        ...
        /*new GlobalObject(SNOW_EFFECT, b.getX() + 1, b.getY(), 0, 0, 10, 30);
        new GlobalObject(SNOW_EFFECT, b.getX(), b.getY() + 1, 0, 0, 10, 30);
        new GlobalObject(SNOW_EFFECT, b.getX() + 1, b.getY() + 1, 0, 0, 10, 30);
        new GlobalObject(SNOW_EFFECT, b.getX() + 2, b.getY() + 1, 0, 0, 10, 30);
        new GlobalObject(SNOW_EFFECT, b.getX() + 1, b.getY() + 2, 0, 0, 10, 30);*/
        new StillGraphic(SNOW_EFFECT, b.getPosition());
        new StillGraphic(SNOW_EFFECT, b.getPosition());
        new StillGraphic(SNOW_EFFECT, b.getPosition());
        new StillGraphic(SNOW_EFFECT, b.getPosition());
        new StillGraphic(SNOW_EFFECT, b.getPosition());
    ...
    private static void doMagicAttack() {
        if (Misc.isLucky(25)) {
            ...
            for (Player player : PlayerHandler.players) {
                if (player != null && Boundary.isIn(player, Boundary.WINTERTODT)) {
                    players.add(player.getIndex());
                }
            }
            int rnd = Misc.random(players.size() - 1);
            Player player = PlayerHandler.getPlayers().get(rnd);
            if (player.getY() <= 3988)
                return;
            ...
            if (TileControl.generate(baseX, baseY, 0).getTile() != null && !Server.getGlobalObjects().anyExists(baseX, baseY,0))
                ...
                player.startGraphic(new Graphic(SNOW_EFFECT));
            ...
            for (Player p : PlayerHandler.getPlayers()) {
                if (p.getPosition().withinDistance(new Position(baseX, baseY), 1) && Boundary.isIn(p, Boundary.WINTERTODT)) {
                    System.out.println(fallingSnow.size() + " " + p.getDisplayNameLong() + " " + p.getPosition().getX() + " " + p.getPosition().getY());
                    p.sendMessage("The freezing cold attack of the Wintertodt's magic hits you.");
                    int dmg = Misc.random(0, 8);
                    p.appendDamage(dmg, (dmg > 0 ? Hitmark.HIT : Hitmark.MISS));
                }
            }
        }
    }
    ...
    private static void attackPyromancers() {
        if (Misc.isLucky(5)) {
            if (Server.getGlobalObjects().get(BROKEN_BRAZIER,1620, 3997, 0) != null) {
                new StillGraphic(860, pyro1.getPosition());
            ...
            if (Server.getGlobalObjects().get(BROKEN_BRAZIER,1620, 4015, 0) != null) {
                new StillGraphic(860, pyro2.getPosition());
            ...
            if (Server.getGlobalObjects().get(BROKEN_BRAZIER,1638, 4015, 0) != null) {
                new StillGraphic(860, pyro3.getPosition());
            ...
            if (Server.getGlobalObjects().get(BROKEN_BRAZIER,1638, 3997, 0) != null) {
                new StillGraphic(860, pyro4.getPosition());
    ...
    private static void death() {
        ...
        for (Player c : PlayerHandler.getPlayers()) {
