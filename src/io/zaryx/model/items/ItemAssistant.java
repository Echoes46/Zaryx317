...
import io.zaryx.content.Sounds;
...
import io.zaryx.content.combat.effects.damageeffect.impl.amuletofthedamned.impl.VeracsEffect;
...
import io.zaryx.content.commands.owner.equip;
...
public class ItemAssistant {
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				|| itemName.contains("karil") || itemName.contains("verac") || itemName.contains("beckoning")
				...
				|| itemName.contains("hunting knife") || itemName.contains("eeper's") || itemName.contains("tumeken") || itemName.contains("elder maul") || itemName.contains("bulwark")
	...
	public boolean canEquip(int wearID) {
		...
		switch (wearID) {
			...
			case 21003:
				...
				break;
			case 33175:
				if(player.playerLevel[0] > 1) {
					player.sendMessage("@red@You must have a attack level of only 1 to wield the Axe Of Araphel");
					return false;
				}
				if(player.playerLevel[2] < 80) {
					player.sendMessage("@red@You must have a strength level of at least 80 to wield the Axe Of Araphel");
					return false;
				}
				break;
			...
			case 20134:
			case 20140:
					if (!player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || !player.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
					player.sendMessage("@red@You must be a Wildyman to equip this!");
					return false;
				}
			break;
			...
			case 33814:
			case 33806:
				if(player.playerLevel[0] < 87) {
					player.sendMessage("You must have a attack level of at least 87 to wield this weapon");
					return false;
				}
				break;
			case 13199:
...
//			case 20005:
	...
	public boolean removeFromAnyTabWithoutAdding(int itemId, int itemAmount, boolean updateView) {
		...
		if (item.getAmount() < 0)
			item.setAmount(0);
		tab.remove(item, player.placeHolders);
		if (tab.size() == 0) {
	...
	public void moveItems(int from, int to, int moveWindow, boolean insertMode) {
		if (GroupIronmanBank.swap(player, moveWindow, insertMode, from, to))
			return;
		if (GroupWildyManBank.swap(player, moveWindow, insertMode, from, to))
			return;
	...
	public void deleteArrow() {
		...
		if (player.getItems().isWearingItem(10499, player.playerCape)  && !player.getItems().isWearingItem(4734)  && !player.getItems().isWearingItem(28951)  && !player.getItems().isWearingItem(28136) &&
	...
	public void deleteEquipment() {
		...
		if ( (Arrays.stream(WeaponDataConstants.THROWN).anyMatch(id -> id == thrownweapons) || player.usingOtherRangeWeapons) && !player.getItems().isWearingItem(Items.TOXIC_BLOWPIPE)|| player.usingOtherRangeWeapons && !player.getItems().isWearingItem(28688)) {
	...
	public static String getItemName1(int[] itemId) {
		return ItemDef.forId1(itemId).getName();
	}
	...
	public void openUpBank() {
		if (Boundary.isIn(player, Boundary.BOUNTY_HUNTER_OUTLAST)) {
			return;
		}
		...
		if (player.getPosition().inWild() && !(player.getRights().isOrInherits(Right.GROUP_WILDYMAN) || player.getPosition().inWild() && !(player.getRights().isOrInherits(Right.WILDYMAN)))) {
			player.sendMessage("");
		}
		else if (player.getPosition().inWild() && !(player.getRights().isOrInherits(Right.GAME_DEVELOPER))) {
	...
	public void doGuthixBoost() {
		int[] toIncrease = { 0, 1, 2, 4, 6 };
		int boost;
		for (int i = 0; i < toIncrease.length; i++) {
			boost = (int) (getGuthixBoost(toIncrease[i]));
			player.playerLevel[toIncrease[i]] += boost;
			if (player.playerLevel[toIncrease[i]] > (player.getLevelForXP(player.playerXP[toIncrease[i]]) + boost))
				player.playerLevel[toIncrease[i]] = (player.getLevelForXP(player.playerXP[toIncrease[i]]) + boost);
			player.getPA().refreshSkill(toIncrease[i]);
		}
	}
	public double getGuthixBoost(int skill) {
		double boost = 1;
		switch (skill) {
			case 0:
			case 1:
			case 2:
				boost = 5 + (player.getLevelForXP(player.playerXP[skill]) * .30);
				break;
			case 4:
			case 6:
				boost = (player.getLevelForXP(player.playerXP[skill]) * .30);
				break;
		}
		return boost;
	}
