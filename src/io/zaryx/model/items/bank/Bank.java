...
public class Bank {
	...
	public void sendValue() {
		...
		for (int i = 0; i < bankTabs.length; i++) {
			for (BankItem bankItem : getBankTab(bankTabs[i].getTabId()).getItems()) {
		...
		player.getPA().sendString(59117, "@or1@ Bank value: <col=FF5500>" + Misc.formatCoins(coins) + " Coins " + "@or1@ Upgrade Points: <col=FF5500>" + Misc.formatCoins(foundry));
	}
	...
	public void RemoveBankTabItem(int itemId) {
		for (int i = 0; i < getBankTab().length; i++) {
			if (bankTabs[i].contains(new BankItem(itemId))) {
				bankTabs[i].remove(new BankItem(itemId, getItemCountInTabs(itemId)), player.placeHolders);
				break;
			}
		}
	}
