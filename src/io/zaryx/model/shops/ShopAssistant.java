...
import io.zaryx.content.questing.hftd.HftdQuest;
import io.zaryx.content.upgrade.UpgradeMaterials;
import io.zaryx.content.wogw.Wogwitems;
...
import io.zaryx.util.logging.player.ShopBuyLog;
import io.zaryx.util.logging.player.ShopSellLog;
...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 310) {
			c.sendMessage(itemName + ": currently costs " + getSpecialItemValue(removeId) + " Ecto Tokens.");
			return;
		}
		if (c.myShopId == 118) {
		...
		if (c.myShopId == 171) {
			if (removeId == 691 || removeId == 692 || removeId == 693 || removeId == 696) {
				c.sendMessage("@pur@[UPGRADER]@bla@ You pay an extra 10% when buying back Upgrade Points as certificates.");
			}
		...
		if (c.myShopId == 193) {
			c.sendMessage(itemName + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Raid Of The Damned Points.");
			return;
		}
		if (c.myShopId == 119) {
		...
		if (c.myShopId == 389) {
			c.sendMessage(itemName + ": currently costs " + getSpecialItemValue(removeId) + " Trivia points");
			return;
		}
		if (c.myShopId == 196) {
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 147
				|| c.myShopId == 18 || c.myShopId == 198 || c.myShopId == 199) {
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (c.myShopId == 389) {
			c.sendErrorMessage("You can't sell items back to the Trivia Shop!");
			return false;
		}
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 197) {
			...
			if (c.getBloody_points() < itemValue) {
				c.sendMessage("You do not have enough Blood points to buy this from the shop.");
				return;
			}
		...
		if (c.myShopId == 310) {
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.getItems().getInventoryCount(4278) < itemValue) {
				c.sendMessage("You do not have enough Ecto Tokens to buy this from the shop.");
				return;
			}
			c.getItems().deleteItem2(4278, itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 193) {
			...
			if (c.arboPoints < itemValue) {
				c.sendMessage("You do not have enough Raid Of The Damned Points to buy this from the shop.");
				return;
			}
		...
		if (c.myShopId == 389) {
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.getTriviaPoints() < itemValue || itemValue < 0) {
				c.sendMessage("You do not have enough Trivia points to buy this from the shop.");
				return;
			}
			c.setTriviaPoints(c.getTriviaPoints() - itemValue);
			c.getQuestTab().updateInformationTab();
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
...
//			c.getItems().deleteItem(itemID, itemValue); // Deduct the required value of items
		...
		} else if (c.myShopId == 9 || c.myShopId == 112 || c.myShopId == 199) {
		...
		} else if (c.myShopId == 389) {
			if (c.triviaPoints >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					c.triviaPoints -= getSpecialItemValue(itemID) * amount;
					c.getQuestTab().updateInformationTab();
					c.getItems().addItem(itemID, amount);
					c.getItems().sendInventoryInterface(3823);
					logShop("bought", itemID, amount);
				...
				c.sendMessage("You do not have enough Trivia Points to buy this item.");
			}
