...
public class ItemHandler {
	...
	public void createGroundItem(Player player, int itemId, int itemX, int itemY, int height, int itemAmount, int playerId, boolean globalise, int hideTicks) {
		if (!ItemDef.forId(itemId).isNoted() && player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33111) && !Boundary.isIn(player, Island.boundary)) {
			itemId = ItemDef.forId(itemId).getNotedItemIfAvailable();
		}

		if (!Boundary.isIn(player, Boundary.WILDERNESS_PARAMETERS) &&
				!ItemDef.forId(itemId).isNoted() && player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33085) && Misc.random(0, 100) < 10  && !Boundary.isIn(player, Boundary.OUTLAST) && !Boundary.isIn(player, Island.boundary)) {
			itemId = ItemDef.forId(itemId).getNotedItemIfAvailable();
		}
		...
		if (player.getItems().playerHasItem(33159) && itemId == 10501 || (player.hasFollower && (player.petSummonId == 33159)) && itemId == 10501 && !Boundary.isIn(player, Island.boundary)) {
			player.getItems().addItemUnderAnyCircumstance(itemId,itemAmount);
			logPickup(player, new GroundItem(itemId, itemX, itemY, height, itemAmount, hideTicks, player.getDisplayNameLower()));
			return;
		}

		if (!Boundary.isIn(player, Boundary.WILDERNESS_PARAMETERS) && player.getItems().isWearingItem(26914) && !Boundary.isIn(player, Boundary.OUTLAST) && !Boundary.isIn(player, Island.boundary)
		...
		if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33083) && !Boundary.isIn(player, Boundary.WILDERNESS_PARAMETERS) && Misc.random(0,100) > 90 && !Boundary.isIn(player, Boundary.OUTLAST)
				&& !Boundary.isIn(player, Boundary.FALLY_OUTLAST) && !Boundary.isIn(player, Boundary.LUMBRIDGE_OUTLAST) && !Boundary.isIn(player, Boundary.SWAMP_OUTLAST) && !Boundary.isIn(player, Island.boundary)
				&& !Boundary.isIn(player, Boundary.FOREST_OUTLAST)&& !Boundary.isIn(player, Boundary.SNOW_OUTLAST)&& !Boundary.isIn(player, Boundary.ROCK_OUTLAST)&& !Boundary.isIn(player, Boundary.LUMBRIDGE_OUTLAST)&& !Boundary.isIn(player, Boundary.WG_Boundary)) {
			if (player.getMode().isUltimateIronman()) {
				player.getInventory().addOrDrop(new ImmutableItem(itemId,itemAmount));
			} else {
		...
		if (itemId > 0 && itemAmount > 0) {
			...
			if (crusher && boneOptional.isPresent()) {
				...
				if (itemId == bone.getItemId()) {
					...
					player.getPA().addSkillXPMultiplied((int) (bone.getExperience() * itemAmount),
