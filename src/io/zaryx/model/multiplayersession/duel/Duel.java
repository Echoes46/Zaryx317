...
import io.zaryx.model.multiplayersession.Multiplayer;
...
public class Duel extends Multiplayer {
    ...
    public boolean requestable(Player requested) {
        ...
        if (player.getLootingBag().isWithdrawInterfaceOpen()
                || player.getLootingBag().isDepositInterfaceOpen()
                || requested.getLootingBag().isWithdrawInterfaceOpen()
                || requested.getLootingBag().isDepositInterfaceOpen()) {
            return false;
        }
        ...
        if (player.connectedFrom.equalsIgnoreCase(requested.connectedFrom)
                && !player.getRights().isOrInherits(Right.MODERATOR)) {
        ...
        /*
         * Updated by Khaos:
         * Duel requests should only happen in the lobby.
         * The last Boundary.DUEL_ARENA entry is the lobby/challenge area.
         */
        if (!isInDuelLobby(player)) {
            player.sendMessage("You must be in the duel arena lobby to do this.");
            return false;
        }

        if (!isInDuelLobby(requested)) {
            player.sendMessage("The other player must be in the duel arena lobby to do this.");
            return false;
        }
    ...
    public void request(Player requested) {
        ...
        DuelSession session = (DuelSession) Server.getMultiplayerSessionListener()
                .requestAvailable(player, requested, MultiplayerSessionType.DUEL);
