...
public class Food {
    ...
    private boolean canEat(Player player, int foodId) {
        ...
        if (player.getHealth().getCurrentHealth() >= player.getHealth().getMaximumHealth() && foodId != 13441) {
            player.sendMessage("Current HP: " + player.getHealth().getCurrentHealth() + "Max HP" + player.getHealth().getMaximumHealth());
            player.sendMessage("You have max or greater than max hp already");
            return false;
        }
    ...
    public void eat(int id, int slot) {
        ...
        if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33231)) {
            player.getHealth().increase(foodToEat.getHeal() + 5);
            return;
        }
