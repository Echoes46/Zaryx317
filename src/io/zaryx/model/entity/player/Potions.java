...
public class Potions {
	...
	private void applyPotion(int itemId, int slot) {
		...
		switch (itemId) {
			...
			case 4417:
				if (!c.guthixFaction) {
					c.sendMessage("@red@You must be apart of the Guthix faction to drink this!");
					return;
				}
				doGuthixRest(4419, slot);
				break;
			case 4419:
				if (!c.guthixFaction) {
					c.sendMessage("@red@You must be apart of the Guthix faction to drink this!");
	...
	public void drinkInfAgroPot() {
		...
		c.startAnimation(829);
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.InfAgroTimer = System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(30);
	...
	public void doGuthixRest(int replaceItem, int slot) {
		c.getPA().sendGameTimer(ClientGameTimer.OVERLOAD, TimeUnit.MINUTES, 5);
		c.hasOverloadBoost = false;
		c.startAnimation(829);
		if (!Boundary.isIn(c, Boundary.WG_Boundary)) {
		...
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.hasGuthixRestBoost = true;
		doGuthixBoost();
		handleGuthixRestTimer();
		c.getPA().refreshSkill(0);
		c.getPA().refreshSkill(1);
		c.getPA().refreshSkill(2);
		c.getPA().refreshSkill(3);
	...
	private void createOverloadDamageEvent() {
		...
		CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.OVERLOAD_HITMARK_ID, c, new CycleEvent() {
			...
			public void execute(CycleEventContainer b) {
				...
				if (c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33119) && !c.getArboContainer().inArbo() || (c.armourofozmage() && !c.getArboContainer().inArbo())) {
                    b.stop();
                    return;
                }
				if (c.teleTimer > 0) {
...
/*		if (c.getHealth().getCurrentHealth() < c.getHealth().getMaximumHealth()) {
	...
	public void handleGuthixRestTimer() {
		CycleEventHandler.getSingleton().stopEvents(c, CycleEventHandler.Event.GUTHIX_BOOST_ID);
		CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.GUTHIX_BOOST_ID, c, new CycleEvent() {
	...
	public double getGuthixBoost(int skill) {
		...
		switch (skill) {
			...
			case 1:
			case 2:
				boost = 5 + (c.getLevelForXP(c.playerXP[skill]) * .30);
				break;
			case 4:
			case 6:
				boost = (c.getLevelForXP(c.playerXP[skill]) * .30);
				break;
		}
		return boost;
	}
