...
import io.zaryx.model.items.inventory.InventoryListener;

import java.util.List;
import java.util.stream.Stream;

public class GroupWildyManBank {
    ...
    private static GroupWildyManBank getBank(Player player) {
    ...
    public static final int OBJECT_ID = 7811;
    ...
    private final GroupWildyManGroup groupWildyman;

    public GroupWildyManBank(GroupWildyManGroup groupWildyman) {
        this.groupWildyman = groupWildyman;
        inventory.addListener(new groupWildymanBankListener());
    }
    ...
    public void withdraw(Player player, int itemId, int itemSlot, int amount) {
        ...
        inventory.shift();
        updateInventoryContainer(player);
        groupWildyman.addWithdrawItemLog(player, item);
    }
    ...
    private Stream<Player> getGroupMembersWithOpenBank() {
        return groupWildyman.getOnline().stream().filter(it -> it.isInterfaceOpen(INTERFACE_ID));
    }

    private class groupWildymanBankListener implements InventoryListener {
        @Override
        public void capacityExceeded(Inventory inventory) { }

        @Override
        public void itemsUpdated(Inventory inventory) {
            getGroupMembersWithOpenBank().forEach(GroupWildyManBank.this::updateContainer);
        }

        @Override
        public void itemUpdated(Inventory inventory, int slot, GameItem item) {
            getGroupMembersWithOpenBank().forEach(it -> GroupWildyManBank.this.updateContainerSlot(it, slot, item));
        }
    }
}
