...
public class GroupWildyManGroup {
    ...
    private final GroupWildyManGroupStats stats = new GroupWildyManGroupStats(this);
    private final GroupWildyManBank bank = new GroupWildyManBank(this);
    private final List<String> mergedCollectionLogs = Lists.newArrayList();
    private CollectionLog collectionLog;
    private final Deque<GwmWithdrawItemLog> withdrawItemLog = new ArrayDeque<GwmWithdrawItemLog>();
    private final Deque<GwmDropItemLog> dropItemLog = new ArrayDeque<GwmDropItemLog>();
    ...
    public GroupWildyManGroup(String name, List<String> members) {
        this.name = name;
    ...
    public boolean canJoin(Player checking, Player joining) {
        ...
        if (!joining.getRights().contains(Right.GROUP_WILDYMAN)) {
            checking.sendStatement("Not a Group Wildyman player.");
            return false;
        }

        if (GroupWildyRepository.getGroupForOnline(joining).isPresent()) {
    ...
    public void addToGroup(Player player) {
        ...
        online.add(player);
        player.sendMessage("Joined Wildyman Group '" + name + "'.");
    }
    ...
    public void addWithdrawItemLog(Player player, GameItem gameItem) {
        if (!log(gameItem))
            return;
        withdrawItemLog.offerFirst(new GwmWithdrawItemLog(player.getDisplayName(), gameItem));
    ...
    public void addDropItemLog(Player player, GameItem gameItem) {
        if (!log(gameItem))
            return;
        dropItemLog.offerFirst(new GwmDropItemLog(player.getDisplayName(), gameItem, player.getPosition()));
    ...
    public GroupWildyManGroupStats getStatistics() {
        return stats;
    }

    public GroupWildyManBank getBank() {
        return bank;
    }
    ...
    public Deque<GwmWithdrawItemLog> getWithdrawItemLog() {
        return withdrawItemLog;
    }

    public Deque<GwmDropItemLog> getDropItemLog() {
        return dropItemLog;
    }

    @Override
    public String toString() {
        return "GroupWildymanGroup{" +
                "name='" + name + '\'' +
                ", members=" + members +
                ", finalized=" + finalized +
                ", online=" + online +
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        GroupWildyManGroup that = (GroupWildyManGroup) o;
