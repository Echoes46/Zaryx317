...
import io.zaryx.model.entity.player.mode.ModeRevertType;
import io.zaryx.model.items.ContainerUpdate;
import io.zaryx.model.items.GameItem;
import io.zaryx.model.items.ItemInterface;
...
public class GroupWildymanDialogue extends DialogueBuilder {

    private static final int NPC_ID = GroupWildyMan.GROUP_FORM_NPC;

    public GroupWildymanDialogue(Player player) {
        super(player);
        if (!player.getMode().isGroupWildyman()) {
            npc(NPC_ID, "Pfft.. I only speak to Group Wildyman members..");
            return;
        }

        setNpcId(NPC_ID);
        GroupWildyManGroup group = currentGroup();
        ...
        if (group == null) {
            ...
            if (isTutIsland()) {
                npc("Greetings, " + player.getDisplayName() + ".", "Would you like to form an Wildyman Group?");
            } else {
                npc("Greetings, " + player.getDisplayName() + ".", "Would you like to form an Wildyman Group?", "@red@You can only join/form one group per account!");
            }
        }

        DialogueOption createGroup = new DialogueOption("Create Group", this::createWildymanGroup);
        ...
        DialogueOption leaveWithoutForming = new DialogueOption("Leave now, form/join group later", plr -> GroupWildyMan.moveAfterJoin(plr, null));
        DialogueOption viewLogs = new DialogueOption("View logs", GroupWildymanLogsInterface::open);
    ...
    private boolean isTutIsland(Player player) {
        return player.getController().getKey().equals(new GroupWildyManFormingController().getKey());
    }
    ...
    private GroupWildyManGroup currentGroup() {
        return GroupWildyRepository.getGroupForOnline(getPlayer()).orElse(null);
    }
    ...
    private void kickPlayerFromGroup(Player player) {
        Optional<GroupWildyManGroup> gr = GroupWildyRepository.getGroupForOnline(player);

        if (gr.isEmpty()) {
            npcn("You need to be in a Group Wildyman team", "and be the leader in order to kick", "someone from your group");
            return;
        }

        GroupWildyManGroup group = gr.get();

        if (group.isFinalized()) // Disabled kick for formed groups
            return;

        if (!group.isLeader(player)) {
            npcn("Only your group leader can kick players", "from your group Wildyman team.");
            return;
        }
        ...
        exit(plr -> player.getPA().sendEnterString("Enter the players username you wish to kick from your team.", (p, otherName) -> {
            ...
            GroupWildyRepository.removeFromGroup(other, group);
            other.sendStatement("You were kicked from the '" + group.getName() + "' group.");
            npcn("Perfect, I have kicked '"+otherName+"'", "from your group Wildyman team.");
        }));
    }

    public void invitePlayerToGroup(Player player) {
        String header = "Enter username";
        GroupWildyManGroup group1 = currentGroup();
    ...
    private static void join(Player player, GroupWildyManGroup group) {
        ...
        player.getPA().closeAllWindows();
        GroupWildyRepository.addToGroup(player, group);
        if (group.isFinalized() && Boundary.isIn(player, Boundary.GROUP_IRONMAN_FORMING)) {
            GroupWildyMan.moveAfterJoin(player, group);
        }
        if (!group.isFinalized()) {
    ...
    private void createWildymanGroup(Player player) {
        Consumer<Player> createGroup = plr -> {
            player.getPA().closeAllWindows();
            Optional<GroupWildyManGroup> gr = GroupWildyRepository.getFromGroupList(player);
            ...
            player.getPA().sendEnterString("Enter a name you wish to register as your group..", (p, groupName) -> {
                ...
                if (GroupWildyRepository.groupExistsWithName(groupName)) {
                ...
                npcn("Perfect, I have created your Wildyman group '"+groupName+"'", "To manage your team, speak to me again.");
                GroupWildyManGroup group = GroupWildyRepository.addFormingGroup(player, groupName);
    ...
    private void disbandGroup(Player player) {
        ...
        group.getOnline().forEach(it -> {
            it.sendMessage("Your Wildyman Group was disbanded by the leader.");
            GroupWildyRepository.removeFromGroup(it, group);
        });
    ...
    private void leaveGroup(Player player) {
        ...
        if (isFormed()) {
            ...
            GroupWildyRepository.removeFromGroup(player, group);
            npcn("You've been removed from the group.");
        }
    }

    private void leaveFormedGroup(Player player, GroupWildyManGroup group) {
    ...
    private void formGroup(Player player) {
        if (!isGroupLeader())
            return;
        GroupWildyMan.formGroup(player);
    }
