...
public class PlayerHandler {
	...
	public void createProjectile(Position start, Position end, int offset, int projectileId, int startHeight, int endHeight, int speed, int delay, int curve, int slope, int targetIndex, int lockon) {
		for (Player player : players) {
			if (player != null && player.getPosition().withinDistance(start, 64)) { // Check if the player is within range to see the projectile
				player.getOutStream().createFrame(85);
				player.getOutStream().writeByteC(start.getY() - (player.getMapRegionY() * 8) - 2);
				player.getOutStream().writeByteC(start.getX() - (player.getMapRegionX() * 8) - 3);

				player.getOutStream().createFrame(117);
				player.getOutStream().writeByte(offset); // Offset
				player.getOutStream().writeByte(end.getX() - start.getX()); // Distance X
				player.getOutStream().writeByte(end.getY() - start.getY()); // Distance Y
				player.getOutStream().writeWordA(lockon); // Lockon index
				player.getOutStream().writeWordA(projectileId); // GFX ID for projectile
				player.getOutStream().writeByte(startHeight); // Start height
				player.getOutStream().writeByte(endHeight); // End height
				player.getOutStream().writeWordA(targetIndex); // Target player index
				player.getOutStream().writeWordA(speed); // Projectile speed
				player.getOutStream().writeByte(delay); // Initial delay before projectile appears
				player.getOutStream().writeByte(curve); // Curve angle
				player.getOutStream().writeByte(slope); // Slope
			}
		}
	}
	...
	public static java.util.stream.Stream<Player> getPlayerStream() {
		return StreamSupport.stream(Arrays.stream(players).spliterator(), false)
	...
	public void sendObjAni(GlobalObject object, int animation) {
		for (Player player : players) {
			if (player != null && player.getPosition().withinDistance(object.getPosition(), 20)) {
				player.getPA().sendPlayerObjectAnimation(object.getPosition().getX(), object.getPosition().getY(), animation, object.getType(), object.getFace());
			}
		}
	}
	...
	public void sendProjectile(Projectile projectile, InstancedArea instancedArea) {
		for (Player player : players) {
			if (player != null && player.distanceToPoint(projectile.getStart().getX(), projectile.getStart().getY()) < 24
					&& player.getInstance() == instancedArea && player.getHeightLevel() == projectile.getStart().getHeight()) {
	...
	public void sendSound(int id, Position position, InstancedArea instancedArea) {
		for (Player player : players) {
			if (player != null && player.distance(position) < 24
				...
				player.getPA().sendSound(id, SoundType.AREA_SOUND);
			}
		}
	}
	...
	public void sendSound(int id, Entity source) {
		...
		for (Player player : players) {
			if (player != null && player.distance(source.getPosition()) < 24
				...
				player.getPA().sendSound(id, source == player || source.isNPC() && player.npcAttackingIndex == source.getIndex() ? SoundType.SOUND.ordinal()
						: SoundType.AREA_SOUND.ordinal(), 0,0);
			}
		}
	}
	...
	public void updatePlayer(Player plr, Stream str) {
		...
		if (plr.mapRegionDidChange) {
			...
			str.writeUnsignedWord(plr.mapRegionY + 6);
