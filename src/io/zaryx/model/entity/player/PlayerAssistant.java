...
import io.zaryx.content.achievement_diary.impl.WildernessDiaryEntry;
import io.zaryx.content.boosts.BoostType;
import io.zaryx.content.boosts.Booster;
import io.zaryx.content.boosts.Boosts;
import io.zaryx.content.bosses.BossHealthHud;
import io.zaryx.content.combat.Damage;
import io.zaryx.content.combat.Hitmark;
import io.zaryx.content.combat.effects.damageeffect.impl.DragonfireShieldEffect;
import io.zaryx.content.combat.magic.CombatSpellData;
import io.zaryx.content.combat.magic.MagicRequirements;
import io.zaryx.content.combat.magic.NonCombatSpellData;
...
import io.zaryx.content.questing.cursePrayers.CursePrayersQuest;
...
import io.zaryx.content.skills.crafting.CraftingData;
import io.zaryx.content.skills.crafting.Enchantment;
...
import io.zaryx.model.cycleevent.impl.WheatPortalEvent;
...
import io.zaryx.model.entity.player.broadcasts.BroadcastManager;
import io.zaryx.model.entity.player.broadcasts.BroadcastType;
...
import io.zaryx.net.outgoing.messages.ComponentVisibility;
...
//		sendSound(soundId, soundType.ordinal(), 0, 0);
    ...
    public void sendSound(int id, int type, int delay, int volume) {
		/*if(c.getOutStream() != null && id != -1) {
			...
			c.getOutStream().writeUnsignedWord(delay);
//			c.getOutStream().writeUnsignedWord(volume);
			c.flushOutStream();
		}*/
    }
    ...
    public void sendNpcOnInterface(int id, int npc, int zoom) {
        ...
        stream.writeUnsignedWord(zoom);
        stream.writeUnsignedWord(npc);
        c.flushOutStream();
    }
    ...
    public void sendProgressBar(int id, int progress) {
        ...
        stream.createFrame(246);
        stream.writeWordBigEndian(id);
        stream.writeUnsignedWord(progress);
        stream.writeUnsignedWord(0);
        c.flushOutStream();
    }
...
//
    ...
    public static void sendItem(Player player, int componentId, int item, int capacity) {
        ...
        player.getOutStream().writeUnsignedWord(1);
        if (capacity > 254) {
...
//	updateConfig(c, spell);
    ...
    public void sendFrame126Pray(String s, int id) {
        sendFrame126(s, id, true);
    }
    ...
    public void sendQuestJournal(CursePrayersQuest cursePrayersQuest) { //TODO
    }
...
/*	public void initWheelOfFortune(int wheelInterfaceId, int winningIndex, int[] items) {
    ...
    public void initWheelOfFortune(int interfaceId, int index, int[] items) {
        if (c.getOutStream() != null) {
            ...
            IntStream.of(items).forEach(i -> c.getOutStream().writeUnsignedWord(i));
            c.flushOutStream();
        }
    }
    ...
    public void removeAllWindows() {
        if (c.getOutStream() != null && c != null) {
            ...
            // Reset sit animation when closing/walking from blackjack
            if (c.playerStandIndex == 4086) {
...
//		System.out.println("id = " + id + ", slot = " + slot + ", column = " + column + ", amount = " + amount );
    ...
    public void alchemy(int itemId, String alch) {
        ...
        if (c.getPosition().inHomeAreaSafe()) {
            c.sendMessage("@red@You must use the barrier to leave the safe pk area!");
            return;
        }
        ...
        switch (alch) {
            ...
            case "high":
                if (System.currentTimeMillis() - c.alchDelay > 500) {
                    ...
                    c.getItems().deleteItem(itemId, 1);
                    int amount = (int) (ShopAssistant.getItemShopValue(itemId) / 4);
...
/*			if (Boundary.isIn(c, Boundary.ICE_PATH) || Boundary.isIn(c, Boundary.ICE_PATH_TOP) && c.heightLevel > 0) {
            ...
            if (Boundary.isIn(c, Boundary.DUNGROOMS)) {
                c.getPotions().resetOverload();
            }
            if (c.getSlayer().superiorSpawned) {
                c.getSlayer().superiorSpawned = false;
...
/*		if (Boundary.isIn(c, Boundary.OUTLAST_HUT)) {
    ...
    public boolean canTeleport(String teleportType) {
		...
		/*f (c.wildLevel > 20  && !c.isInterfaceOpen(31000)) {
			c.sendMessage("you cannot teleport above 20 wild");
			return false;
		}*/
        ...
        if (c.getPosition().inClanWars() || c.getPosition().inClanWarsSafe() || c.getPosition().inHomeAreaSafe()) {
            c.sendMessage("@cr10@You can not teleport from here, speak to the doomsayer to leave.");
            return false;
        }
        ...
        if (Boundary.isIn(c, Boundary.AFK_ZONE)) {
            c.sendMessage("Please use the AFK door");
            return false;
        }
        if (Boundary.isIn(c, Boundary.DUEL_ARENA)) {
            c.sendMessage("Please exit the duel arena to teleport");
            return false;
        }

        if (Boundary.isIn(c, Boundary.DUNGROOMS)) {
            if (c.getDungInstance() != null) {
                c.sendMessage("Please use the stairs or type @red@::leavedung!");
                return false;
            }
        }
        if (Boundary.isIn(c, Boundary.RFD)) {
...
/*		if (Boundary.isIn(c, Boundary.ICE_PATH) || Boundary.isIn(c, Boundary.ICE_PATH_TOP) && c.heightLevel > 0) {
        ...
        if (Boundary.isIn(c, Boundary.AFK_ZONE)) {
            if (Boundary.isIn(c, Boundary.DUEL_ARENA)) {
                if (!teleportType.equals("glory") && !teleportType.equals("obelisk") && c.petSummonId != 10533) {
                    if (c.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && !c.getRights().contains(Right.GROUP_WILDYMAN)) {
                        c.sendMessage("You must use the gate to exit the AFK zone.");
                        c.getPA().closeAllWindows();
                        return false;
                    }
                } else if (!teleportType.equals("obelisk") && c.petSummonId != 10533) {
                    if (c.wildLevel > 30 || !c.getRights().contains(Right.GROUP_WILDYMAN)) {
                        c.sendMessage("You must use the gate to exit the AFK zone.");
                        c.getPA().closeAllWindows();
                        return false;
...
/*		if (Boundary.isIn(c, Boundary.HESPORI) && !c.canLeaveHespori) {
        ...
        if (Boundary.isIn(c, Boundary.DUNGROOMS)) {
            if(c.getDungInstance() != null) {
                c.sendMessage("You cannot teleport out of the dungeon. Please use the stairs!");
                return ;
            }
        }
        if (c.isTeleblocked()) {
...
/*		if (Boundary.isIn(c, Boundary.ICE_PATH) || Boundary.isIn(c, Boundary.ICE_PATH_TOP) && c.heightLevel > 0) {
    ...
    public void refreshSkill(int i) {
        ...
        c.getPA().sendString(99501,"Current poll: @red@Inactive");
        c.getPA().sendFrame126Pray(c.playerLevel[5]+"/"+getLevelForXP(c.playerXP[5]), 99501);
    }
    ...
    public boolean addSkillXPPrayer(double amount, int skill, boolean dropExperience) {
        return addSkillXP((int) (c.getExpMode().getType().getExperienceRate(Skill.forId(skill)) * 15), skill, dropExperience);
    }
    ...
    public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
        ...
        if (c.getItems().isWearingItem(29999)) {
            amount *= 1.25;
        }

        if (c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33226)) {
            amount *= 3;
...
/*		if (c.getMode().getType().isStandardRate(Skill.forId(skill))) {
        ...
        if (c.getOutStream() != null) {
            ...
            for (Object param : params) {
                if (param instanceof Integer) {
                    c.getOutStream().writeDWord((int) param);//nvm thats int right? dword = int
                }else if(param instanceof Boolean) {
                    c.getOutStream().writeDWord(Boolean.TRUE.equals(param) ? 1 : 0);
                } else {
    ...
    public static void wildySlayerTeleport(final Player player) {
        ...
        if (!player.getRights().isOrInherits(Right.WILDYMAN) && !player.getRights().isOrInherits(Right.GROUP_WILDYMAN)) {
            player.sendMessage("Only Wildyman or Group Wildyman can use this teleport.");
            return;
        }
        ...
        if (task.isInWilderness()) {
            player.sendMessage("You are teleporting to your task of " + task.getPrimaryName() + " in the wilderness.");
            player.getPA().startTeleport(x, y, z, "modern", false);
        } else {
            player.sendMessage("@red@Your task is not int he wilderness!");
        }
    }
    ...
    public void useOperate(int itemId) {
        ...
        switch (itemId) {
            ...
            case 13136:
                ...
                if (c.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && c.petSummonId != 10533 || !c.getRights().contains(Right.GROUP_WILDYMAN) || !c.getRights().contains(Right.WILDYMAN)) {
                    c.sendMessage(
                ...
                if (c.guthixFaction && c.wildLevel < 50) {
                    c.getPA().spellTeleport(3426, 2927, 0, false);
                    return;
                }
                break;
    ...
    public static void TorvaHealth (Player c) {
        c.getHealth().setMaximumHealth(106);
        c.getPA().refreshSkill(3);
    }


    public static void SangHealth (Player c) {
        c.getHealth().setMaximumHealth(110);
        c.getPA().refreshSkill(3);
    }

    public static void AhriHealth (Player c) {
        c.getHealth().setMaximumHealth(131);
        c.getPA().refreshSkill(3);
    }
...
/*		if (c == null || c.isDisconnected()) {
    ...
    public void sendSlayerCount(int id) {
        ...
        if (c.getSlayer().getTaskAmount() > 0 && c.getSlayer().getTask().isPresent()) {
            ...
            stream.writeWordBigEndian(id);
            stream.writeDWord(c.getSlayer().getTaskAmount());
            c.flushOutStream();
        }
    }
