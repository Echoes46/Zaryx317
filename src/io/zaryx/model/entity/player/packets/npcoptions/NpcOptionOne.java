package io.zaryx.model.entity.player.packets.npcoptions;
...
import io.zaryx.content.bosses.nightmare.NightmareActionHandler;
import io.zaryx.content.dailyrewards.DailyRewardsDialogue;
...
import io.zaryx.content.skills.farming.FarmingTeleport;
...
import io.zaryx.content.teamBounties.BountyDialogue;
...
import io.zaryx.model.entity.player.mode.group.GroupIronmanDialogue;
...
import io.zaryx.model.entity.player.mode.wildygroup.GroupWildymanDialogue;
...
/*				if (PetHandler.pickupPet(player, npcType, true))
		...
		case DailyRewardsDialogue.DAILY_REWARDS_NPC:
			...
			case 10529:
				//player.sendMessage("Dude im disabled");
			...
			case 3779:
				player.start(new DialogueBuilder(player).npc(3779, "I am Stan, I have access to the latest AOE Materials!","I sometimes offer some amazing deals.").option("Have any Volatile minerals to exchange?",
						new DialogueOption("Yes", player1 -> {
							...
							if (amt > 0) {
								player1.getItems().deleteItem2(13186, amt);
								player1.getItems().addItem(13307, (amt * 3));
							}
							player1.getPA().closeAllWindows();
						}), new DialogueOption("No", player1 -> player1.getPA().closeAllWindows())));
				break;
...
//				BloodExchangeBurnPrice.openExchangeRateShop(player);
			...
			case 5527:
				...
				break; 
				case 2459:
				player.getShops().openShop(389);
				player.sendMessage("You currently have @red@" + player.triviaPoints + " @bla@Trivia Points");
				break;
		case 2111:
		...
		case 9120:
			...
			case 3005:
				player.start(new DialogueBuilder(player).option("Welcome adventurer, what would you like to do?",
					new DialogueOption("Teleport to EctoFuntus", p -> p.getPA().startTeleport(3659, 3526, 0, "modern", false)),
					new DialogueOption("Nothing", p -> p.getPA().closeAllWindows())));
				break;
		...
		case 7716:
			player.start(new DialogueBuilder(player).setNpcId(7716).option("How can I help you today?",
				...
				if (player1.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player1.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN))) {
					player1.getPA().closeAllWindows();
...
/*		case 5520:
		...
		case Npcs.MAC:
			...
			case 6599:
				player.start(new BountyDialogue(player));
			break;

			case 9656:
				player.start(new GodFactions(player, true));
				break;
		...
		case GroupIronman.GROUP_FORM_NPC:
			...
			break;
			case GroupWildyMan.GROUP_FORM_NPC:
			player.start(new GroupWildymanDialogue(player));
			break;
		case Npcs.ADAM:
		...
		case 7913:
			if (player.getMode().isIronmanType()
					|| player.getRights().isOrInherits(Right.GAME_DEVELOPER) || player.getMode().isWildyman() || player.getMode().isGroupWildyman()) {
				player.getShops().openShop(41);
			} else {
				player.sendMessage("You must be an Iron Man, or a Wildyman or Group Wildyman to access this shop.");
			}
			break;
		case 7769:
