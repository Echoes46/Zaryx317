...
import io.zaryx.content.PlayerEmotes;
import io.zaryx.content.Q2;
import io.zaryx.content.UimStorageChest;
import io.zaryx.content.achievement.inter.AchieveV2;
import io.zaryx.content.bosses.hespori.Hespori;
import io.zaryx.content.cheatprevention.CheatEngineBlock;
...
import io.zaryx.content.deals.Deals;
import io.zaryx.content.deals.TimeOffers;
import io.zaryx.content.dialogue.DialogueAction;
import io.zaryx.content.dialogue.DialogueActionButton;
import io.zaryx.content.dialogue.DialogueConstants;
...
import io.zaryx.content.skills.crafting.CraftingData.tanningData;
import io.zaryx.content.skills.crafting.GlassBlowing;
import io.zaryx.content.skills.crafting.LeatherMaking;
import io.zaryx.content.skills.crafting.Tanning;
...
import io.zaryx.model.entity.player.packets.dialogueoptions.FiveOptions;
import io.zaryx.model.entity.player.packets.dialogueoptions.FourOptions;
import io.zaryx.model.entity.player.packets.dialogueoptions.OptionHandler;
import io.zaryx.model.entity.player.packets.dialogueoptions.ThreeOptions;
import io.zaryx.model.entity.player.packets.dialogueoptions.TwoOptions;
import io.zaryx.model.items.ContainerUpdate;
		...
		switch (actionButtonId) {
			...
			case 121040:
				...
				if (Boundary.isIn(c, Boundary.DUEL_ARENA)) {
					c.teleporting = false;
					c.sendMessage("You must exit the duel arena to teleport");
					return;
				}
			...
			case 183156:
				...
				if (!(c.getSuperMysteryBox().canMysteryBox) || !(c.getNormalMysteryBox().canMysteryBox) ||
						...
						!(c.getDragonToken().canMysteryBox) ||
						...
						!(c.getMiniTobBox().canMysteryBox) || !(c.getMiniUltraBox().canMysteryBox)
				) {
			...
			case 51039:
				if (c.amDonated < 25) {
					c.sendMessage("@red@You must be a donator to use this!");
					return;
				} else {
			...
			/*case 130131:
				...
				break;*/
			case 130131:
				//c.sendMessage("Temporarily Disabled");
				c.getPA().removeAllWindows();
...
//			}
			...
			case 226162:
				...
				if (!c.inBank) {
					GroupWildyManBank.bankAll(c, false);
					return;
				}
			...
			case 226170:
				...
				if (!c.inBank) {
					GroupWildyManBank.bankAll(c, true);
					return;
				}
...
//			}
				...
				if (c.usingLamp) {
					if (c.getItems().playerHasItem(2528) && c.normalLamp && !c.antiqueLamp) {
						...
						if (playerExpMode == ExpModeType.FiveTimes) {
							xpMultiplier = 5;
						} else if (playerExpMode == ExpModeType.TwentyFiveTimes) {
							xpMultiplier = 25;
						} else if (playerExpMode == ExpModeType.OneTimes) {
							xpMultiplier = 1;
						} else if (playerExpMode == ExpModeType.TenTimes) {
							xpMultiplier = 10;
						}

						int totalXP = xpPerLamp * amt * xpMultiplier;
			...
			case 33206:
				if (c.getPosition().inClanWarsSafe()) {
					...
					if (c.wildLevel > 0 || c.getRights().contains(Right.GROUP_WILDYMAN) || c.getRights().contains(Right.WILDYMAN)) {
						c.sendMessage("@red@I dont think so.");
						return;
					}

					c.getDH().sendOption3("Rock crabs", "Information Tab", "-- Cancel --");
					c.dialogueAction = 9800;
			...
			case 33218:
				if (c.getPosition().inClanWarsSafe()) {
					...
					//c.getSI().selected = 4;
					c.getDH().sendOption3("Home altar", "Information Tab", "-- Cancel --");
					c.dialogueAction = 9804;
				}
				break;
			case 33221:
				if (c.getPosition().inClanWarsSafe()) {
			...
			case 33224: // runecrafting
				...
				c.getDH().sendOption3("Abyss", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9806;
				break;
			case 33210: // agility
				...
				c.getDH().sendOption3("Teleport to Grace", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9807;
				break;
			case 33213: // herblore
				...
				c.getDH().sendOption3("Herblore Shop", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9808;
				break;
			case 33216: // theiving
				...
				c.getDH().sendOption3("Home Stalls", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9809;
				break;
			case 33219: // crafting
				...
				c.getDH().sendOption3("Crafting Shop", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9810;
				break;
			case 33222: // fletching
				...
				c.getDH().sendOption3("Woodcutting Guild", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9811;
				break;
			case 47130:// slayer
				...
				c.getDH().sendOption3("Slayer Masters", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9812;
				break;
			case 33208:// mining
				...
				c.getDH().sendOption3("Mining Area", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9813;
				break;
			case 33211: // smithing
			...
			case 33214: // fishing
				...
				c.getDH().sendOption3("Home Pond", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9815;
				break;
			case 33217: // cooking
				...
				c.getDH().sendOption3("Home Range", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9816;
				break;
			case 33220: // firemaking
				...
				c.getDH().sendOption3("Wintertodt", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9817;
				break;
			case 33223: // woodcut
			...
			case 54104: // farming
				...
				c.getDH().sendOption3("Home Farming", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9819;
				break;
			case 137121: // hunter
				...
				c.getDH().sendOption3("Hunter Area", "Information Tab", "-- Cancel --");
				c.dialogueAction = 9820;
				break;
			case 34142: // tab 1
...
/*		case 73245:
			...
			case 75010:
				...
				if (c.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && !c.guthixFaction) {
...
//
//			if (c.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && c.petSummonId == 10533 && c.gwdAltarTimer > 0) {
//				c.sendMessage("Your pet is on cooldown, you'll need to run lower!");
//
//				return;
//			}
		...
		case 153236: // rigour
			...
			case 132213: // curses - turmoil
				//c.sendMessage("Disabled temporarily!");
