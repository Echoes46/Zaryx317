...
import io.zaryx.content.bosses.ForestGuardian;
import io.zaryx.content.bosses.bryophyta.Bryophyta;
import io.zaryx.content.bosses.hespori.Hespori;
import io.zaryx.content.bosses.hespori.HesporiSpawner;
import io.zaryx.content.bosses.mimic.StrangeCasketDialogue;
import io.zaryx.content.bosses.obor.OborInstance;
import io.zaryx.content.bosses.relicguardian.RelicGuardian;
...
import io.zaryx.content.item.lootable.impl.SerenChest;
import io.zaryx.content.item.lootable.impl.UnbearableChest;
...
import io.zaryx.content.skills.agility.impl.rooftop.RooftopPollnivneach;
import io.zaryx.content.skills.hunter.impling.PuroPuro;
import io.zaryx.content.skills.runecrafting.ouriana.OurianaAltar;
import io.zaryx.content.skills.runecrafting.ouriana.OurianaBanker;
...
import io.zaryx.util.logging.player.ClickObjectLog;
...
//        if (SquidGames.handleObjects(c, worldObject, option)) {
//            return;
//        }
        ...
        switch (option) {
            case 1:
                ...
                if (io.zaryx.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 1)) {
                    return;
                }
                ...
                switch (c.objectId) {
                    ...
                    case 7353: {
                        c.getPA().movePlayer(1978, 5980, 0);
                        return;
                    }
                    case 7352: {
                        c.getPA().movePlayer(2021, 5991, 0);
                        return;
                    }
                    ...
                    case 38829: {
                        ...
                        c.start(new DialogueBuilder(c).statement("Warning! You're about to enter an instanced area, anything left on",
                                ...
                                option("Are you sure you wish to open it?",
                                        new DialogueOption("Yes, let's go!", player -> {
                                            if (!player.getItems().playerHasItem(RelicGuardian.KEY, 1)) {
                                                /**
                                                 * Requires relic key
                                            ...
                                            new RelicGuardian().enter(player);
                                            player.getPA().closeAllWindows();
                    ...
                    case 410: {
                        /**
                         * relic guardian gate wildyman
                         */
                        c.start(new DialogueBuilder(c).statement("Warning! You're about to enter an instanced area, anything left on",
                                        "the ground when you leave will be lost. Would you like to continue?").

                                option("Are you sure you wish to enter?",
                                        new DialogueOption("Yes, let's go!", player -> {
                                            if (!player.getItems().playerHasItem(RelicGuardian.KEY, 1)) {
                                                /**
                                                 * Requires relic key
                    ...
                    case 28686: {
                        ...
                        c.start(new DialogueBuilder(c).statement("Warning! You're about to enter an instanced area, anything left on",
                                ...
                                option("Are you sure you wish to enter?",
                                        new DialogueOption("Yes, let's go!", player -> {
                                            if (!player.getItems().playerHasItem(ForestGuardian.KEY, 1) && !player.getItems().playerHasItem(995, 5000000)) {
                                                /**
                                                 * Requires forest key
                                                 */
                                                player.sendMessage("You need a forest key and 5m to enter!");
                                                player.getPA().closeAllWindows();
                                                return;
                                            }
                                            new ForestGuardian().enter(player);
                                            player.getPA().closeAllWindows();
                    ...
                    case 33222:
                        ...
                        if (c.getItems().getInventoryCount(9698) == 105) {//TODO FINISH BOTH ROCKS FOR HESPORI
                            c.sendMessage("You dont need anymore runes, BURN HIM!");
                            return;
                        }
                        if (c.getItems().getInventoryCount(9699) == 105) {//TODO FINISH BOTH ROCKS FOR HESPORI
                            c.sendMessage("You dont need anymore burning runes, BURN HIM!");
                            return;
                        }
                    ...
                    case 33223:
                        ...
                        if (c.getItems().getInventoryCount(23778) == 105) {//TODO FINISH BOTH ROCKS FOR HESPORI
                            c.sendMessage("You dont need anymore gems, BURN HIM!");
                            return;
                        }
                        if (c.getItems().getInventoryCount(23783) == 105) {//TODO FINISH BOTH ROCKS FOR HESPORI
                            c.sendMessage("You dont need anymore toxic gems, BURN HIM!");
                            return;
                        }
...
/*                    case 29735:// Basic training
            ...
            case 2:
                ...
                if (io.zaryx.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 2)) {
                    return;
                }
                if (OurianaBanker.clickObject(c, c.objectId, option)) {
                    return;
            ...
            case 3:
                ...
                if (io.zaryx.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 3)) {
                    return;
                }
