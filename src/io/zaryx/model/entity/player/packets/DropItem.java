...
import io.zaryx.content.items.ItemCombinations;
...
//		if (itemId == Items.THAMMARONS_SCEPTRE) {
//			Thammaron.clickItem(c, itemId, 5);
//			return;
//		}
//		if (itemId == 27679) {
//			Thammaron.clickItem(c, itemId, 5);
...
//		}
		...
		if (!Boundary.isIn(c, Boundary.OUTLAST_AREA)
			...
			ItemDef def = itemDef;
//			Discord.writeDropHandler("```[Drop-Log]" + c.getDisplayName() + " dropped " + def.getName() + " x " + Misc.insertCommas(amount) + " at " + c.absX + ", " + c.absY + "```");
		}
		...
		if (c.getBankPin().requiresUnlock()) {
			...
			for (int item : Configuration.TOURNAMENT_ITEMS_DROPPABLE) {
				if ((Boundary.isIn(c, Boundary.OUTLAST_AREA)  || Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST_AREA)
						...
						|| Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST)
						|| Boundary.isIn(c, Boundary.SWAMP_OUTLAST)
						|| Boundary.isIn(c, Boundary.WG_Boundary)) && (item == itemId)) {
		...
		if (itemId == 28585) {
			if (c.getToxicStaffOfTheDeadCharge() <= 0) {
				c.getItems().deleteItem2(28585, 1);
				c.getItems().addItem(28583, 1);
				c.sendMessage("The staff had no charge, but has been reverted to uncharged.");
				return;
			}
			if (c.getItems().freeSlots() <= 0) {
			...
			c.getItems().deleteItem2(28585, 1);
			c.getItems().addItem(28583, 1);
			c.getItems().addItem(28593, c.getToxicStaffOfTheDeadCharge());
			c.setToxicStaffOfTheDeadCharge(0);
			c.sendMessage("You uncharged the warped sceptre");
		}
		...
		if (itemId == 12926 || itemId == 28688) {
			...
			if (charge <= 0) {
				...
				c.getItems().deleteItem2(itemId, 1);
				c.getItems().addItem(itemId == 28688 ? 28687 : 12924, 1);
				return;
			}
