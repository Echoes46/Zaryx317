...
public class PickupItem implements PacketType {
    ...
    public void processPacket(final Player c, int packetType, int packetSize) {
        ...
        if (!c.getMode().isItemScavengingPermitted()) {
            Player owner = PlayerHandler.getPlayerByLoginName(item.getOwnerName());
            GroupWildyManGroup group2 = GroupWildyRepository.getGroupForOnline(c).orElse(null);
            if (owner == null || group2 == null && !c.getLoginNameLower().equalsIgnoreCase(item.getOwnerName()) || group2 != null && c.isApartOfGroupWildyManGroup() && !group2.isGroupMember(owner)) {
                c.sendMessage("Your mode restricts you from picking up items that are not yours.");
                return;
            }
        }
