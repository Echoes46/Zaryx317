...
import io.zaryx.content.LootValue;
...
import io.zaryx.content.commands.all.Claim;
...
import io.zaryx.content.minigames.barrows.RoomLocation;
import io.zaryx.content.minigames.bounty_hunter.BountyHunterEmblem;
...
import io.zaryx.model.entity.player.PrayerScrolls;
...
/*
			...
			/*case 20377:
				...
				c.getPA().closeAllWindows();
				if (c.getLevelForXP(c.playerXP[5])  < 56) {
		...
		case 381:
			...
			break;
			case 7790:
				if (!c.cursePrayers && c.getItems().playerHasItem(719)) {
					c.getItems().deleteItem(719, 1);
					PrayerScrolls.giveCursePrayers(c);
					c.getPA().closeAllWindows();
				} else {
						c.sendMessage("You already have unlocked the Ancient Prayer Book");
						c.getPA().closeAllWindows();
				}
				break;
		case 346:
...
//					3);
	...
	/*		case 956://1 scroll
				...
				if (c.getItems().playerHasItem(956, 1)) {
					Discord.writeServerSyncMessage(""+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $1 scroll.");
