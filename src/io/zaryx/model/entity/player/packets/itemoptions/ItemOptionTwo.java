...
import io.zaryx.model.items.GodBooks;
...
import io.zaryx.content.skills.slayer.DuoMode;
...
public class ItemOptionTwo implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		if(actions != null)
			action = actions[1];
		if(action != null) {
			if(action.canHandle(player, itemId))
...
//		if (Thammaron.clickItem(player, itemId, 2)) {
//			return;
//		}
		...
		case 9762:
			...
			if (player.getPosition().inWild()) {
				...
				break;
		case 10556:
        	player.sendMessage("@red@Attacker Icon gives 10% bonus attack to each combat style");
        	break;
        case 10557:
		...
		case 21347:
			...
			case 13660:
				if(player.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && player.petSummonId != 10533 || !player.getRights().contains(Right.GROUP_WILDYMAN)) {
...
//                TeleportInterface.sendMonsterTab(player);
		...
		case 13136:
			if (player.guthixFaction && player.wildLevel < 50) {
			...
			if (player.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && player.petSummonId != 10533 || !player.getRights().contains(Right.GROUP_WILDYMAN)  && !player.guthixFaction) {
		...
		case 13111:
			...
			case 26314:
				if(player.wildLevel < 100) {
					player.getPA().startTeleport(2095, 6003, 0, "pod", true);
				}
				break;
		case 13226:
		...
		case 5512:
			...
			break;
			case 5514:
				Pouches.check(player, 3);
				break;
			case 5515:
				Pouches.check(player, 4);
				break;
		case 12904:
			player.sendMessage("The toxic staff of the dead has " + player.getToxicStaffOfTheDeadCharge() + " charges remaining.");
			break;
			case 28585:
				player.sendMessage("The Warped Sceptre has " + player.getToxicStaffOfTheDeadCharge() + " charges remaining.");
				break;
		case 13199:
		case 13197:
