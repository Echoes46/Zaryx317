...
import io.zaryx.content.bosses.mimic.MimicCasket;
...
import io.zaryx.content.dialogue.impl.ClaimDonatorScrollDialogue;
...
import io.zaryx.content.skills.SkillPetRateIncreaseScroll;
import io.zaryx.content.skills.hunter.Hunter;
import io.zaryx.content.skills.hunter.trap.impl.BirdSnare;
import io.zaryx.content.skills.hunter.trap.impl.BoxTrap;
...
public class ItemOptionOne implements PacketType {
    ...
    public void processPacket(Player c, int packetType, int packetSize) {
        ...
        if (itemId == 33209) {
            ...
            c.sendMessage("Ho Ho Ho, Merry christmas from the Zaryx staff team!");
            return;
        }
        ...
        if (itemId == 11157) {
            ...
            c.start(new DialogueBuilder(c).statement("The devious souls enter your body granting xp"));
            c.getPA().addSkillXPMultiplied(2500*count, Skill.DEMON_HUNTER.getId(), true);
            c.getItems().deleteItem2(11157, count);
        }

        if (itemId == 20238) {
            int count = c.getItems().getInventoryCount(20238);
            if (count <= 0) {
                return;
            }
        ...
        if (itemId == 27285) {
            c.start(new DialogueBuilder(c).option("Which weapon would you like to upgrade?", new DialogueOption("Tumeken's Shadow (500m UpgradePoints & 25x Corruptors)", p -> {
            ...
            c.start(new DialogueBuilder(c).option("what tool would you like to upgrade?", new DialogueOption("Infernal pickaxe (or). (100m MadPoints & 5 Smoked gems)", p -> {
                ...
                new DialogueOption("Infernal Axe (or). (100m MadPoints & 5 Smoked gems", p -> {
                    ...
                    new DialogueOption("Infernal Harpoon (or). (100m MadPoints & 5 Smoked gems", p -> {
                        ...
                        if (p.getItems().getInventoryCount(28274) < 5) {
                            p.sendMessage("@red@You need 5 Smoke gems to upgrade your Harpoon");
                            p.getPA().closeAllWindows();
                            return;
                        }
                        if (p.foundryPoints < 100_000_000) {
                            p.sendMessage("@red@You require 500m MadPoints to upgrade your Harpoon!");
                            p.getPA().closeAllWindows();
                            return;
                        }
        ...
        if (itemId == 10025) {
            int count = c.getItems().getInventoryCount(10025);
            if (count <= 0) {
                return;
            }
        ...
        if (itemId == 11481) {
            ...
            if (Boundary.isIn(c, Boundary.DUEL_ARENA)) {
                c.sendMessage("You can't use this in the duel arena!");
                return;
            }
            if (Boundary.isIn(c, Boundary.OUTLAST_HUT)) {
                c.sendMessage("You are not allowed to drink potions in the outlast hut.");
        ...
        if (itemId == 11429) {
            if (Boundary.isIn(c, Boundary.DUEL_ARENA)) {
                c.sendMessage("@red@You can't use this in the duel arena!");
                return;
            }
            if (Boundary.isIn(c, Boundary.NEX)) {
                c.sendMessage("@red@You can't use this in Nex!");
                return;
            }
            if (Boundary.isIn(c, Boundary.OUTLAST_HUT)) {
                c.sendMessage("@red@You are not allowed to drink potions in the outlast hut.");
                return;
            }
            if (c.isDead) {
                c.getPotionTimer().reset();
        ...
        if (io.zaryx.content.dwarfleaguecannon.Cannon.clickItem(c, itemId)) {
            return;
        }
        ...
        case 21034:
            ...
            case 6799:
                ...
                break;
            case 6798:
                c.getItems().deleteItem(6798, 1);
                break;
            case 6800:
                c.getItems().deleteItem(6800, 1);
                break;
            case 6801:
                c.getItems().deleteItem(6801, 1);
                break;
            case 6802:
                c.getItems().deleteItem(6802, 1);
                break;
            case 6803:
                c.getItems().deleteItem(6803, 1);
                break;
            case 6804:
                c.getItems().deleteItem(6804, 1);
                break;
            case 6806:
                c.getItems().deleteItem(6806, 1);
                break;
            case 6808:
                c.getItems().deleteItem(6808, 1);
                break;
        ...
        case 23185:
            ...
            case 19564:
                ...
                if (c.wildLevel < 31 && c.getMode().equals(Mode.forType(ModeType.STANDARD))) {
                ...
                else if (c.wildLevel < 31 && c.getMode().equals(Mode.forType(ModeType.ULTIMATE_IRON_MAN))) {
                    c.getPA().startTeleport(3087, 3504, 0, "pod", false);
                    break;
                }
                else if(c.wildLevel < 31 && c.getMode().equals(Mode.forType(ModeType.HC_IRON_MAN))) {
                    c.getPA().startTeleport(3087, 3504, 0, "pod", false);
                    break;
                }
                else if (c.wildLevel < 31 && c.getMode().equals(Mode.forType(ModeType.IRON_MAN))) {
                    c.getPA().startTeleport(3087, 3504, 0, "pod", false);
                    break;
                }
                else if (c.wildLevel < 31 && c.getMode().equals(Mode.forType(ModeType.GROUP_IRONMAN))) {
                    c.getPA().startTeleport(3087, 3504, 0, "pod", false);
                    break;
                }
                else if (c.wildLevel < 150 && c.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
                    c.getPA().startTeleport(3444, 3837, 0, "pod", false);
                    break;
        ...
        /*case 6830:
            ...
            case 22087:
                ...
                else if (c.getItems().playerHasItem(22087)) {
                    c.getDragonToken().openInterface();
                    c.inDonatorBox = true;
                    c.stopMovement();
                    return;
                }
