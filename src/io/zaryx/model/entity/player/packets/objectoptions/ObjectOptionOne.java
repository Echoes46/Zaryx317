...
import io.zaryx.content.minigames.arbograve.instance.ArbograveInstance;
import io.zaryx.content.minigames.arbograve.party.ArbogravePartyWild;
import io.zaryx.content.minigames.raids.WildPartyCox;
...
import io.zaryx.content.WeaponGames.WGPowerup;
import io.zaryx.content.achievement_diary.impl.*;
import io.zaryx.content.afkzone.Afk;
...
import io.zaryx.content.bosses.hydra.AlchemicalHydra;
import io.zaryx.content.bosses.wintertodt.Wintertodt;
import io.zaryx.content.bosses.wintertodt.WintertodtActions;
import io.zaryx.content.combat.Hitmark;
import io.zaryx.content.combat.weapon.WeaponDataConstants;
import io.zaryx.content.dialogue.DialogueBuilder;
import io.zaryx.content.dialogue.DialogueOption;
import io.zaryx.content.dialogue.impl.CrystalCaveEntryDialogue;
import io.zaryx.content.dialogue.impl.FireOfDestructionDialogue;
import io.zaryx.content.dialogue.impl.OutlastLeaderboard;
import io.zaryx.content.dialogue.impl.SkillingPortalDialogue;
...
import io.zaryx.content.minigames.barrows.TunnelDoor;
import io.zaryx.content.minigames.barrows.TunnelDoors;
...
import io.zaryx.content.minigames.raids.CoxParty;
import io.zaryx.content.minigames.raids.Raids;
import io.zaryx.content.skills.FlaxPicking;
import io.zaryx.content.skills.Skill;
import io.zaryx.content.skills.agility.AgilityHandler;
import io.zaryx.content.skills.agility.PyramidPlunder;
import io.zaryx.content.skills.agility.impl.rooftop.RooftopArdougne;
import io.zaryx.content.skills.agility.impl.rooftop.RooftopSeers;
import io.zaryx.content.skills.agility.impl.rooftop.RooftopVarrock;
import io.zaryx.content.skills.crafting.BraceletMaking;
import io.zaryx.content.skills.crafting.JewelryMaking;
import io.zaryx.content.skills.dungeoneering.DungMain;
import io.zaryx.content.skills.dungeoneering.DungParty;
import io.zaryx.content.skills.firemake.Burner;
import io.zaryx.content.skills.hunter.Hunter;
import io.zaryx.content.skills.runecrafting.Runecrafting;
import io.zaryx.content.skills.slayer.NewInterface;
import io.zaryx.content.skills.smithing.CannonballSmelting;
import io.zaryx.content.skills.thieving.Thieving.Stall;
import io.zaryx.content.skills.woodcutting.Tree;
import io.zaryx.content.skills.woodcutting.Woodcutting;
import io.zaryx.content.teamBounties.BountyChest;
import io.zaryx.content.teleportv2.inter.TeleportInterface;
import io.zaryx.content.tournaments.TourneyManager;
import io.zaryx.content.tournaments.ViewingOrb;
...
import io.zaryx.model.entity.player.packets.objectoptions.impl.DarkAltar;
import io.zaryx.model.entity.player.packets.objectoptions.impl.Overseer;
import io.zaryx.model.entity.player.packets.objectoptions.impl.RaidObjects;
import io.zaryx.model.entity.player.packets.objectoptions.impl.TrainCart;
...
import io.zaryx.model.multiplayersession.duel.DuelSessionRules.Rule;
...
import static io.zaryx.content.skills.smithing.Smelting.*;
...
public class ObjectOptionOne {
	...
	static int[] barType = { 2363, 2361, 2359, 2353, 2351, 2349, 2364, 2361, 2360, 2354, 2352, 2350 };

	public static void handleOption(final Player c, int objectType, int obX, int obY) {

//		System.out.println(objectType + " / " + c.getDisplayName());
...
/*		if(actions != null)
		...
		if (c.getPrayer().handleObjects(c, objectType)) {
			return;
		}

		/*if (c.getElonIsland().handleObjects(c, objectType)) {
			return;
		}*/
		...
		if (Burner.handleBonfireClick(c, objectType)) {
			return;
		}
...
/*			if (days < 1) {
		...
		/*if (Burner.handleObjects(c, objectType)) {
			return;
		}*/
        ...
        if (c.getAgilityPyramid().agilityPyramidCourse(c, objectType)) {
            return;
        }
		...
		if ((def != null ? def.name : null) != null && def.name.toLowerCase().contains("bank") && !Boundary.isIn(c, Boundary.OURIANA_ALTAR) && !Boundary.isIn(c, Boundary.ECTOFUNTUS)) {
		...
		if (c.getDungInstance() != null && c.getDungInstance().handleObjectClick(c, objectType, obX, obY)) {
			c.objectDistance = 3;
			return;
		}
...
//			}
			...
			int targetX = obX;
			int targetY = obY;
			//	System.out.println("face: "+object.getFace()+" and rotation: "+door.getRotation());
			if (c.absX == obX) {
		...
		switch (objectType) {
			case 6192:
				if (!c.getItems().hasItemOnOrInventory(1265)) {
					c.getItems().addItem(1265, 1);
					c.sendMessage("@blu@You take a bronze pickaxe from the anvil.");
				} else {
					c.sendMessage("@red@You already have a bronze pickaxe.");
				}
				break;
			case 20720:
			case 20721:
			...
			case 32619:
				if (c.amDonated < 25) {
					c.sendMessage("@red@You must be a donator to use this bell!");
					return;
				} else if (System.currentTimeMillis() - c.lastDropTableSelected < TimeUnit.SECONDS.toMillis(5)) {
					c.sendMessage("You can only do this once every 5 seconds.");
				}
				c.startAnimation(2140);
				new Broadcast(c.getDisplayName() + " is Requesting to challenge players to a safe PvP fight!").submit();
				return;
			case 33396:
			case 41181:
			...
			case 5947:
				if (HesporiSpawner.isSpawned()) {
					c.getPA().spellTeleport(3745, 4361, 0, false);
					c.setHesporiDamageCounter(0);
				} else {
					c.sendMessage("@red@[World Event] @bla@There is currently no world event going on.");
				}
				return;
			case 30176: //wildycave entrance
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2375, 4281, 0, "You crawl through the shortcut",
						"Beware youre still not safe here", 2);
				break;
			case 3610://wildyman exit
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 3481, 3863, 0, "You crawl through the shortcut",
						"you return to the neverending danger.", 2);
				break;
			case 16520: //wildycave entrance
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2390, 4269, 0, "You crawl through the shortcut",
						"nope, youre still not safe", 2);
				break;
			...
			case 4616: //wildycave entrance
				AgilityHandler.delayFade(c, "JUMP", 2595, 3608, 0, "you jump the rotting bridge",
						"Phew i made it", 2);
				break;
			case 12770:
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2020, 5974, 0, "You crawl through the shortcut",
						"you end up by the Mining Area!", 2);
				return;
			case 12771:
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2078, 6001, 0, "You crawl through the shortcut",
						"you end up by the Home Area!", 2);
				return;
			/**
			 Slayer Shortcut
			 */
			...
			case 32637: //wildycave entrance
				AgilityHandler.delayFade(c, "JUMP", 3689, 5407, 1, "",
						"", 2);
				break;
			case 34499: //dz1
				AgilityHandler.delayFade(c, "CRAWL", 3041, 4530, 0, "",
						"", 2);
				break;

			case 29495: //dz1
				AgilityHandler.delayFade(c, "JUMP", Configuration.START_LOCATION_X, Configuration.START_LOCATION_X, 0, "",
						"", 2);
				break;

			case 727:
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2054, 5955, 0, "You crawl through the shortcut",
						"you end up by the Slayer Area!", 2);
				return;
			case 43759:
				c.startAnimation(844, 3);
				AgilityHandler.delayFade(c, "NONE", 2096, 6015, 0, "You crawl through the shortcut",
						"you end up by the Home Area!", 2);
				return;

			case 40441:
				c.startAnimation(734, 3);
				AgilityHandler.delayFade(c, "NONE", 3101, 3517, 0, "You jump into the portal!",
						"you end up near the Edgeville Ditch!", 2);
				return;
			/**
			 *
			 * End
			 */
			case 20149://Six Hour Scrolls
				if (c.twoHourClaimDate == c.getTodayDate()) {
					c.getDH().sendStatement("You have already claimed your scroll. Check back in two hours!");
					c.nextChat = -1;
					return;
				}
				if (!c.twoHourClaimable) {
					long milliseconds = (long) c.playTime2 * 12000;
					long minutes = TimeUnit.MILLISECONDS.toHours(milliseconds);
					int hour = 120;
					long time = minutes;
					c.getDH().sendStatement("You can't take a Scroll that quick!", "Check back in @red@" + (hour - time) + " minutes!");
					return;
				}
				if (c.twoHourClaimable && c.twoHourClaimDate != c.getTodayDate()) {
					SixHourScrolls.searchTable(c);
					c.getPA().closeAllWindows();
					return;
				}
				break;

			case 39160://Corpse Cart
				if (c.corpseCartDate == c.getTodayDate()) {
					c.getDH().sendStatement("You have already claimed your scroll. Check back in two hours!");
					c.nextChat = -1;
					return;
				}
				if (!c.corpseCartClaim) {
					long milliseconds = (long) c.playTime2 * 6000;
					long minutes = TimeUnit.MILLISECONDS.toHours(milliseconds);
					int hour = 60;
					long time = minutes;
					c.getDH().sendStatement("You can't check the cart yet", "Check back in @red@" + (hour - time) + " minutes!");
					return;
				}
				if (c.corpseCartClaim && c.corpseCartDate != c.getTodayDate()) {
					CorpseCart.searchCart(c);
					c.getPA().closeAllWindows();
					return;
				}
				break;
			...
			case 26285:
				if (c.playerLevel[Skill.THIEVING.getId()] < 75) {
					c.sendMessage("You need a thieving level of 75 to steal from this.");
					return;
				} else {
					c.getItems().addItem(565, 3);
				}
				break;

			case 26273://Hourly chest
				if (c.loyaltyClaimDate == c.getTodayDate()) {
					c.getDH().sendStatement("You have already claimed your loyalty reward today. Check back tomorrow!");
					c.nextChat = -1;
					return;
				}
				if (!c.loyaltyChestClaimable) {
					long milliseconds = (long) c.playTime1 * 6000;
					long minutes = TimeUnit.MILLISECONDS.toMinutes(milliseconds);
					int hour = 60;
					long time = minutes;
					c.getDH().sendStatement("The chest is not yet unlocked. Check back in another @red@" + (hour - time) + " minutes!");
					return;
				}
				if (c.loyaltyChestClaimable && c.loyaltyClaimDate != c.getTodayDate()) {
					HourlyRewardBox.searchChest(c);
					c.getPA().closeAllWindows();
					return;
				}
				break;
			case 32655:
				c.start(new DialogueBuilder(c).option("You see a note asking for Help!",
						new DialogueOption("Go Help", p -> {
							c.getPA().startTeleport(3504, 3215, 0, "modern", false);
						}),
						new DialogueOption("Nah im Scared", p -> {
...
//				} else {
					DukeSucellus instance = new DukeSucellus(c, Boundary.DUKE_INSTANCE);
					DukeSucellus.enter(c, instance);
					c.sendMessage("@red@After each kill click on the gate to start another");
				break;
			case 47560:
				if (!c.getItems().hasItemOnOrInventory(233)) {
					c.getInventory().addOrDrop(new ImmutableItem(233, 1));
				}
				break;
			case 47561:
				if (!c.getItems().hasItemOnOrInventory(1267)) {
					c.getInventory().addOrDrop(new ImmutableItem(1267, 1));
				}
				break;
			case 47536:
				if (c.getItems().getInventoryCount(28346) >= 6 && c.getItems().getInventoryCount(28342) >= 6) {
					c.getItems().deleteItem2(28346, 6);
					c.getItems().deleteItem2(28342, 6);
					c.getItems().deleteItem2(28349, 6);
					DukeSucellus.changeVats(object, c);
				}
				break;
			case 47347:
				TourneyManager tourny = TourneyManager.getSingleton();
				tourny.leaveLobby(c, false);
				break;

			case 47539:
				DukeSucellus.changeVats(object, c);
				c.getInventory().addOrDrop(new ImmutableItem(28351, 1));
				break;
			case 47524:
...
//				break;
			case 48234:
				if (Boundary.isIn(object, DonorVault.area) && Boundary.isIn(c, DonorVault.area)) {
					if (c.DonorVaultObjects.size() >= 20) {
						...
						c.sendMessage("@red@You have completed your personal vault! Thank you for supporting Zaryx!");
			...
			case 1533:
				...
				break;
			case 40452:
				if (c.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) && c.totalLevel > 750 || (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.totalLevel > 750)) {
					c.getPA().spellTeleport(2398, 4252, 0, false);
				} else {
					c.sendMessage("You need to be 750 total and be a wild adventurer.");
				}
				return;
			case 40454:
				if (c.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) && c.totalLevel > 1500 || (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.totalLevel > 1500)) {
					c.getPA().spellTeleport(2406, 4242, 0, false);
				} else {
					c.sendMessage("You need to be 1500 total and be a wild adventurer.");
				}
				return;

			case 47077://kratos entrance
				c.getPA().movePlayer(3153, 5237, 0);
				c.sendMessage("@red@You hear the voices calling your name...");
				break;

			case 23074://kratos exit
				c.getPA().movePlayer(3320, 3796, 0);
				c.sendMessage("@red@Things suddenly become silent, what relief..");
				break;

			case 2353://kratos exit
				AgilityHandler.delayEmote(c, "CLIMB", 3321, 3795, 0, 2);
				c.sendMessage("@red@Things suddenly become silent, what relief...");
				break;

			case 22945://kratos door
				if (c.getPosition().inWild() && c.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN)) || (c.getPosition().inWild() && c.getMode().equals(Mode.forType(ModeType.WILDYMAN)))) {
					AgilityHandler.delayEmote(c, "GLASS WALL", 3168, 5214, 0, 2);

					c.sendMessage("@red@Welcome to Seldaeh' Lair, The voices Deafen you.");
				} else {
					c.sendMessage("@red@You need to be a wild adventurer.");
				}
				return;
			...
			case GroupIronmanBank.OBJECT_ID:
				...
				break;
			case GroupWildyManBank.OBJECT_ID:
				Optional<GroupWildyManGroup> group2 = GroupWildyRepository.getGroupForOnline(c);
				group2.ifPresentOrElse(it -> it.getBank().open(c),
						() -> c.sendMessage("@red@You must be in a Group to use the bank!"));
				break;
			case 29064:
			...
			case 42967:
				if (c.usingInfAgro) {
					c.sendMessage("@red@You can not enter nex with infinite aggro on.");
					return;
				}
				...
				break;
//			case 28686:
//				if (c.getItems().playerHasItem(993) && c.getItems().playerHasItem(995, 5_000_000)) {
//					c.getItems().deleteItem(993, 1);
//					c.getItems().deleteItem(995, 5_000_000);
//					AgilityHandler.delayEmote(c, "CRAWL", 2278, 5015, 0, 2);
//				} else {
//					c.sendMessage("@red@ You need 5m and a Key of the forest to enter.");
//				}
//				break;
			...
			case 30283:
				if (c.getItems().playerHasItem(993) && c.getItems().playerHasItem(995, 5_000_000)) {
					...
					AgilityHandler.delayEmote(c, "JUMP", 2278, 5015, 0, 2);
				} else {
					c.sendMessage("@red@ You need 5m and a Key of the forest to enter.");
				}
				break;
			...
			case 8929:
				...
				if (c.getSlayer().getMaster() == 7663) {
					c.sendMessage("@bla@You need a slayer task from @red@Nieve @bla@to use this cave");
					return;
				}
				if (c.getSlayer().getMaster() == 8623) {
					c.sendMessage("@bla@You need a slayer task from @red@Nieve @bla@to use this cave");
					return;
				}
				if (c.getSlayer().getMaster() == 603) {
					c.sendMessage("@bla@You need a slayer task from @red@Nieve @bla@to use this cave");
					return;
				}
				if (c.getSlayer().getMaster() == 8761) {
					c.sendMessage("@bla@You need a slayer task from @red@Nieve @bla@to use this cave");
					return;
				}
			...
			case 29338:
				c.getPA().movePlayer(3347, 9623, 0);
				break;
			...
			case 29778:
				...
				if (Boundary.isIn(c, Boundary.FULL_RAIDS)) {
					LobbyManager.get(LobbyType.WILD_COXLOBBY)
							.ifPresent(lobby -> lobby.attemptLeave(c));
					c.getPA().movePlayer(3182, 3725);
					c.setRaidsInstance(null);
					return;
				}
				break;
			case 31623: //making forocious gloves
				if (c.getItems().playerHasItem(995, 15_000_000) && c.getItems().playerHasItem(22983) && c.getItems().playerHasItem(2347) ||
						c.getItems().playerHasItem(995, 15_000_000) && c.getItems().playerHasItem(22983) && c.getItems().playerHasItem(2949)) {
...
/*				if (!(System.currentTimeMillis() - c.chestDelay > 2000)) {
			...
			case 41304:
				...
				CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.NOTEDFISH, c, new CycleEvent() {
					@Override
					public void execute(CycleEventContainer container) {
						if (c.isDisconnected()) {
							container.stop();
				...
				c.getPA().addSkillXPMultiplied(100, Skill.FISHING.getId(), true);
				...
				c.getItems().addItem(reward.getId(), Misc.random(1, 2), true);
				c.infernoLeaveTimer = System.currentTimeMillis();
				int petRate = c.skillingPetRateScroll ? (int) (2500 * .75) : 2500;
				if (Misc.random(petRate) == 20 && c.getItems().getItemCount(13320, false) == 0 && c.petSummonId != 20663) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] <col=255>" + c.getDisplayName() + "</col> now goes hand in hand with a <col=CC0000>Heron</col> pet!");
					c.getItems().addItemUnderAnyCircumstance(13320, 1);
					c.getCollectionLog().handleDrop(c, 5, 13320, 1);
			...
			case 37989:
				...
				CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.NOTEDFISH, c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						...
						c.getPA().addSkillXPMultiplied(100, Skill.WOODCUTTING.getId(), true);
						List<GameItem> rewards = Lists.newArrayList(
								new GameItem(1512, 1),
								new GameItem(1514, 1),
								new GameItem(1518, 1),
						...
						c.infernoLeaveTimer = System.currentTimeMillis();
						int petRate = c.skillingPetRateScroll ? (int) (2500 * .75) : 2500;
						if (Misc.random(petRate) == 20 && c.getItems().getItemCount(13322, false) == 0 && c.petSummonId != 20663) {
							PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] <col=255>" + c.getDisplayName() + "</col> now goes hand in hand with a <col=CC0000>Beaver</col> pet!");
							c.getItems().addItemUnderAnyCircumstance(13322, 1);
							c.getCollectionLog().handleDrop(c, 5, 13322, 1);
...
/*			case 1521:
			...
			case 14749:
			case 14750:
				io.zaryx.objects.DoorHandler.handleDoor(c, object);
				break;*/
			//case 14735:
			...
			case 36197:
				if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
					c.getPA().spellTeleport(3444, 3837, 0, true);
				} else if (c.getMode().equals(Mode.forType(ModeType.GROUP_WILDYMAN))) {
					c.getPA().spellTeleport(3444, 3837, 0, true);
				} else {
			...
			case 34727:
				...
				break;
			case 10734:
				c.getPA().movePlayer(2723, 4584, 0);
			case 21600:
				if (c.absY == 3802) {
			...
			case 11845:
				if (c.getY() == 5969) {
					c.getPA().movePlayer(2095, 5967, 0);
				}
				if (c.getY() == 5967) {
					c.getPA().movePlayer(2095, 5969, 0);
			...
			case 31558://lower stairs
				...
				break;
			case 16646://lower stairs
				if (c.saradominFaction) {
					c.getPA().movePlayer(3666, 3522, 1);
					}
				else if (!c.getItems().playerHasItem(995, 1000000)) {
					c.sendMessage("@red@You need 1,000,000 coins to go upstairs!");
					return;
				}
				c.getItems().deleteItem(995, 1000000);
				c.getPA().movePlayer(3666, 3522, 1);
				break;
			case 16647://lower stairs
				c.getPA().movePlayer(3666, 3517, 0);
				break;

				case 19051:
					if (c.saradominFaction && Boundary.isIn(c, Boundary.ECTOFUNTUS)) {
						c.getPA().c.itemAssistant.openUpBank();
						c.inBank = true;

					} else c.sendMessage("@red@you must be a follower of saradomin to use this");
				break;
...
//				}
				time2 = 5000;
				if (System.currentTimeMillis() - c.lastHealChest < time2) {
//					if (c.amDonated >= 1000) {
...
//					} else {
					c.sendMessage("You may only use this table every 5 seconds including after login.");
					//}
					return;
				}
				/*for (int skill = 0; skill < c.playerLevel.length; skill++) {
...
//				}
				time1 = 5_000L;
			if (System.currentTimeMillis() - c.lastHealChest < time1) {
//					if (c.amDonated >= 1000) {
//						c.sendMessage("Your rank may only use this chest every 30 seconds.");
//					} else if (c.amDonated >= 500) {
//						c.sendMessage("Your rank may only use this chest every 1 minute.");
...
//					} else {
			...
			case 1206:
				...
				if (c.getItems().getInventoryCount(9017) == 100) {//TODO FINISH BOTH ROCKS FOR HESPORI
					c.sendMessage("You dont need anymore essence, BURN HIM!");
					return;
				}
...
//				AgilityHandler.delayEmote(c, "CLIMB_DOWN", 2548, 9951, 0, 2);
			...
			case 17385:
				if (Boundary.isIn(c, Boundary.EDGE_DUNG_LADDER)) {
					...
					AgilityHandler.delayEmote(c, "CLIMB_UP", 2059, 5964, 0, 2);
			...
			case 31902:
				...
				if (c.getItems().playerHasItem(11864)) {
					c.getItems().deleteItem2(11864, 1);
					c.getItems().addItem(11865, 1);
				}
				else if (c.getItems().playerHasItem(21888)) {
					c.getItems().deleteItem2(21888, 1);
					c.getItems().addItem(21890, 1);
				}
				else if (c.getItems().playerHasItem(21264)) {
					c.getItems().deleteItem2(21264, 1);
					c.getItems().addItem(21266, 1);
				}
				else if (c.getItems().playerHasItem(23073)) {
					c.getItems().deleteItem2(23073, 1);
					c.getItems().addItem(23075, 1);
				}
				else if (c.getItems().playerHasItem(24370)) {

					c.getItems().deleteItem2(24370, 1);
					c.getItems().addItem(24444, 1);
				}
				else if (c.getItems().playerHasItem(19639)) {

					c.getItems().deleteItem2(19639, 1);
					c.getItems().addItem(19641, 1);
				}
				else if (c.getItems().playerHasItem(19643)) {

					c.getItems().deleteItem2(19643, 1);
					c.getItems().addItem(19645, 1);
				}
				else if (c.getItems().playerHasItem(19647)) {

					c.getItems().deleteItem2(19647, 1);
					c.getItems().addItem(19649, 1);

				}
				else if (c.getItems().playerHasItem(25898) && c.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {

					c.getItems().deleteItem2(25898, 1);
					c.getItems().addItem(25900, 1);
				}
				else if (c.getItems().playerHasItem(25904) && c.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {

					c.getItems().deleteItem2(25904, 1);
					c.getItems().addItem(25906, 1);
				}
				else if (c.getItems().playerHasItem(25910) && c.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {

					c.getItems().deleteItem2(25910, 1);
					c.getItems().addItem(25912, 1);
				}
				else if (c.getItems().playerHasItem(23073) && c.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {

					c.getItems().deleteItem2(23073, 1);
					c.getItems().addItem(23075, 1);
				}
				else if (c.getItems().playerHasItem(24370) && c.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {

					c.getItems().deleteItem2(24370, 1);
					c.getItems().addItem(24444, 1);

				} else {
					c.sendMessage("You need a slayer helmet in your inventory to do this.");
					return;
				}
				break;
			...
			case 37410:
				AgilityHandler.delayEmote(c, "CRAWL", 3559, 5408, 0, 2);
				break;
...
//					c.sendMessage("You cannot leave yet, wait a couple of seconds and try again.");
			...
			case 18819:
				if (c.amDonated < 2500) {
					c.sendMessage("You need 2500 or greater total donated.");
					return;
				}
				if (System.currentTimeMillis() - c.infernoLeaveTimer < 1_500L) {
//					c.sendMessage("You cannot leave yet, wait a couple of seconds and try again.");
				...
				c.facePosition(obX, obY);
				c.startAnimation(881);
				c.getPA().addSkillXPMultiplied(100, Skill.FARMING.getId(), true);
				...
				reward = Misc.getRandomItem(rewards);
				c.getItems().addItem(reward.getId(), Misc.random(1, 2), true);
				c.infernoLeaveTimer = System.currentTimeMillis();
				petRate = c.skillingPetRateScroll ? (int) (2500 * .75) : 2500;
				if (Misc.random(petRate) == 20 && c.getItems().getItemCount(20661, false) == 0 && c.petSummonId != 7352) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] <col=255>" + c.getDisplayName() + "</col> now goes hand in hand with a <col=CC0000>Tangleroot</col> pet!");
					c.getItems().addItemUnderAnyCircumstance(20661, 1);
					c.getCollectionLog().handleDrop(c, 5, 20661, 1);
				}
				break;
...
//					c.sendMessage("You cannot leave yet, wait a couple of seconds and try again.");
				...
				c.facePosition(obX, obY);
				c.startAnimation(881);
				c.getPA().addSkillXPMultiplied(100, Skill.HUNTER.getId(), true);
				rewards = Lists.newArrayList(
				...
				c.infernoLeaveTimer = System.currentTimeMillis();
				petRate = c.skillingPetRateScroll ? (int) (2500 * .75) : 2500;
				if (Misc.random(petRate) == 20 && c.getItems().getItemCount(13326, false) == 0 && c.petSummonId != 6721) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] <col=255>" + c.getDisplayName() + "</col> now goes hand in hand with a <col=CC0000>Golden Chinchompa</col> pet!");
					c.getItems().addItemUnderAnyCircumstance(13326, 1);
					c.getCollectionLog().handleDrop(c, 5, 13326, 1);
				}
				break;
			case 6799:
				if (c.amDonated < 2500) {
...
//					c.sendMessage("You cannot leave yet, wait a couple of seconds and try again.");
				...
				c.facePosition(obX, obY);
				c.startAnimation(881);
				c.getPA().addSkillXPMultiplied(100, Skill.CRAFTING.getId(), true);
				rewards = Lists.newArrayList(
						new GameItem(1754, 1),
						new GameItem(1752, 1),
						new GameItem(1750, 1),
				...
				c.infernoLeaveTimer = System.currentTimeMillis();
				petRate = c.skillingPetRateScroll ? (int) (2500 * .75) : 2500;
				if (Misc.random(petRate) == 20 && c.getItems().getItemCount(19497, false) == 0 && c.petSummonId != -1) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>RARE</col>] <col=255>" + c.getDisplayName() + "</col> recieved a <col=CC0000>Zenyte</col> whilst using the crafting bench! ");
					c.getItems().addItemUnderAnyCircumstance(19497, 1);
			...
			case 6455: // Ice ledge (Bottom)
				...
				break;
			case 10048:
				c.getPA().movePlayer(1762, 5490, 0);
				break;
			case 32578:
				c.getPA().movePlayer(1440, 3941, 0);
				break;
			case 677:
				int z = 2;
				if (c.absX == 2974)
...
//			case 5094:
				...
				WildWarning.sendWildWarning(c, plr -> {
					if (plr.getX() == 3134 || plr.getX() == 3135) {
						if (plr.getY() == 3639) {
							...
							PathFinder.getPathFinder().findRoute(plr,plr.getX(),plr.getY()-1,true,1,1,true);
						} else if (plr.getY() == 3617) {
							PathFinder.getPathFinder().findRoute(plr,plr.getX(),plr.getY()-1,true,1,1,true);
						} else if (plr.getY() == 3616) {
							PathFinder.getPathFinder().findRoute(plr,plr.getX(),plr.getY()+1,true,1,1,true);
						}
					}
			...
			case 5100:
				...
				break;
			case 40474:
				c.getPA().movePlayer(3157, 3742, 0);
				break;
			case 21728:
			...
			case 40453:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3178 && c.absY == 3725) {
						c.getPA().movePlayer(3179, 3725, 0);
					}
					if (c.absX == 3179 && c.absY == 3725) {
						c.getPA().movePlayer(3178, 3725, 0);
					}
					if (c.absX == 3178 && c.absY == 3724) {
						c.getPA().movePlayer(3179, 3724, 0);
					}
					if (c.absX == 3179 && c.absY == 3724) {
						c.getPA().movePlayer(3178, 3724, 0);
						break;
					}
				}
				break;
			case 40438:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3178 && c.absY == 3734) {
						c.getPA().movePlayer(3179, 3734, 0);
					}
					if (c.absX == 3179 && c.absY == 3734) {
						c.getPA().movePlayer(3178, 3734, 0);
					}
					if (c.absX == 3178 && c.absY == 3733) {
						c.getPA().movePlayer(3179, 3733, 0);
					}
					if (c.absX == 3179 && c.absY == 3733) {
						c.getPA().movePlayer(3178, 3733, 0);
						break;
					}
				}
				break;
			case 40458:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3178 && c.absY == 3743) {
						c.getPA().movePlayer(3179, 3743, 0);
					}
					if (c.absX == 3179 && c.absY == 3743) {
						c.getPA().movePlayer(3178, 3743, 0);
					}
					if (c.absX == 3178 && c.absY == 3742) {
						c.getPA().movePlayer(3179, 3742, 0);
					}
					if (c.absX == 3179 && c.absY == 3742) {
						c.getPA().movePlayer(3178, 3742, 0);
						break;
					}
				}
				break;
			case 2041:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3168 && c.absY == 3745) {
						c.getPA().movePlayer(3168, 3744, 0);
					}
					if (c.absX == 3168 && c.absY == 3744) {
						c.getPA().movePlayer(3168, 3745, 0);
						break;
					}
				}
				break;
			case 2039:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3168 && c.absY == 3745) {
						c.getPA().movePlayer(3169, 3744, 0);
					}
					if (c.absX == 3168 && c.absY == 3743) {
						c.getPA().movePlayer(3168, 3745, 0);
						break;
					}
				}
			...
			case 2155:
				if (c.getRights().contains(Right.WILDYMAN) || c.getRights().contains(Right.GROUP_WILDYMAN)) {
					if (c.absX == 3159 && c.absY == 3721) {
						c.getPA().movePlayer(3159, 3722, 0);
					}
					if (c.absX == 3159 && c.absY == 3722) {
						c.getPA().movePlayer(3159, 3721, 0);
						break;
					}
				}
				break;
			case 36521:
				c.getPA().movePlayer(3091, 3500, 0);
				break;
			case 5088:
			case 20882:
			...
			case 2030:
				...
				if (DoubleCannonballSmelting.isSmeltingCannonballs(c)) {
					DoubleCannonballSmelting.smelt(c);
				} else {
					c.getSmithing().sendSmelting();
				}
				break;
			case 42824:
				c.barType = Arrays.toString(barType);
				c.smeltAmount = 1;
			...
			case 61:
				...
				break;
			case 38452:
				if (!c.inParty(DungParty.TYPE)) {
					c.sendMessage("You must be in a party to start the dungeon!");
					return;
				}

				if (c.inParty(DungParty.TYPE)) {
					c.getParty().openStartActivityDialogue(c, "Dungeoneering", Boundary.DUNG_LOBBY_ENTRANCE::in, list -> new DungMain().startDung(list, true));
					return;
				}

//				if (Boundary.isIn(c, Boundary.DUNG_LOBBY_ENTRANCE)) {
//					LobbyManager.get(LobbyType.DUNGEONEERING).ifPresent(lobby -> lobby.attemptJoin(c));
//					return;
//				}
//
//				if  (Boundary.isIn(c, Boundary.DUNG_LOBBY)) {
//					LobbyManager.get(LobbyType.DUNGEONEERING)
//							.ifPresent(lobby -> lobby.attemptLeave(c));
//					c.getPA().movePlayer(2399, 5981, 0);
//					return;
//				}
//				LobbyManager.get(LobbyType.DUNGEONEERING)
...
//				break;
			...
			case 4529:
				if (!c.inParty(WildPartyCox.TYPE)) {
					c.sendMessage("You must be in a party to start Chambers of Xeric.");
					return;
				}

				if (c.inParty(WildPartyCox.TYPE)) {
					c.getParty().openStartActivityDialogue(c, "Chambers of Xeric", Boundary.WILD_COXPARTY::in, list -> new Raids().startRaid(list, true));
					return;
				}

				if (Boundary.isIn(c, Boundary.WILD_COXPARTY)) {
					LobbyManager.get(LobbyType.WILD_COXLOBBY).ifPresent(lobby -> lobby.attemptJoin(c));
					return;
				}

				if  (Boundary.isIn(c, Boundary.WILD_COX)) {
					LobbyManager.get(LobbyType.WILD_COXLOBBY)
							.ifPresent(lobby -> lobby.attemptLeave(c));
					c.getPA().movePlayer(1234, 3567, 0);
					return;
			...
			case 31707:
				if (!c.inParty(ArbogravePartyWild.TYPE)) {
					c.sendMessage("You must be in a party to start Raid Of The Damned!");
					return;
				}
				if (c.totalLevel < 1500 &&
						!c.getExpMode().equals(new ExpMode(ExpModeType.OneTimes)) &&
						!c.getExpMode().equals(new ExpMode(ExpModeType.FiveTimes))) {
					c.sendMessage("You need a total level of at least 1500 to join this raid!");
					return;
				...
				c.getParty().openStartActivityDialogue(c, "Raid Of The Damned", ArbograveConstants.WILD_ARBOPARTY::in, list -> new ArbograveInstance(list.size()).start(list));
			break;

			case 30725:
				if (!c.inParty(TobPartyWild.TYPE)) {
					c.sendMessage("You must be in a party to start Theatre of Blood.");
					return;
				}
				...
				if (c.totalLevel < 1000 &&
					...
					return;
				} else if (c.totalLevel < 750 &&
						(c.getExpMode().equals(new ExpMode(ExpModeType.OneTimes)) ||
								c.getExpMode().equals(new ExpMode(ExpModeType.FiveTimes)))) {
					c.sendMessage("You need a total level of at least 750 to join this raid!");
					return;
				}

				c.getParty().openStartActivityDialogue(c, "Theatre of Blood", TobConstants.WILD_TOBPARTY::in, list -> new TobInstance(list.size()).start(list));
				break;
...
/*				if (Raids.isMissingRequirements(c)) {
			...
			case 2873:
				if (!c.saradominFaction) {
					c.sendMessage("You must be a follower of Guthix to acquire this cape.");
					return;
				}
				c.startAnimation(645);
			...
			case 30397:
				if (c.objectY == 4061) {
					if (c.getX() == 3166 && c.getY() == 4061 || c.getX() == 3165 && c.getY() == 4061) {
						c.getPA().movePlayer(3165, 4062, 0);
					} else if (c.getX() == 3165 && c.getY() == 4062 || c.getX() == 3166 && c.getY() == 4062) {
						c.getPA().movePlayer(3166, 4061, 0);
					}
				} else if (c.objectY == 4047) {
					if (c.getX() == 3165 && c.getY() == 4048 || c.getX() == 3166 && c.getY() == 4048) {
						c.getPA().movePlayer(3166, 4047, 0);
					} else if (c.getX() == 3166 && c.getY() == 4047 || c.getX() == 3165 && c.getY() == 4047) {
						c.getPA().movePlayer(3166, 4047, 0);
					}
				}
				break;
			...
			case 40437:
				if (c.objectY == 5977) {
					if (c.getX() == 1943 && c.getY() == 5977) {
						c.getPA().movePlayer(1944, 5977, 0);
					} else if (c.getX() == 1944) {
						c.getPA().movePlayer(1943, 5977, 0);
					}
				}
					break;
