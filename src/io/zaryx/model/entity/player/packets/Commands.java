...
import io.zaryx.content.bosses.hespori.*;
...
import io.zaryx.content.event.eventcalendar.EventCalendarHelper;
...
import io.zaryx.content.skills.farming.Plants;
...
import io.zaryx.content.worldevent.impl.WildernessBossWorldEvent;
...
import io.zaryx.sql.eventcalendar.queries.AddWinnerQuery;
import io.zaryx.util.Misc;
import io.zaryx.util.logging.player.ClanChatLog;
import io.zaryx.util.logging.player.CommandLog;
...
/*            if (playerCommand.startsWith("filter")) {
            ...
            if (playerCommand.toLowerCase().contentEquals("gwstats")) {
                Optional<GroupWildyManGroup> group = GroupWildyRepository.getGroupForOnline(c);
                group.ifPresentOrElse(it -> it.getStatistics().display(c), () -> c.sendMessage("You're not in a Group Wildyman group."));
            }
            ...
            if (playerCommand.toLowerCase().contentEquals("store")) { //extra command not needed for interaces
                c.getPA().sendFrame126("https://zaryx.net/store/", 12000);
            }
...
*/
            ...
            if (playerCommand.startsWith("vote")) {
                ...
                c.getPA().sendFrame126("https://zaryx.net/vote.php", 12000);
                return;
            }

            if (playerCommand.startsWith("reward")) {
                if (args.length < 2) {
                    c.sendMessage("Usage: ::reward <id> [amount]");
                    return;
                }

                final String id     = args[1];
                final String amount = args.length >= 3 ? args[2] : "1";
                final String name   = c.getLoginName();
                    ...
                    @Override public void run() {
                        try {
                            ...
                            if (res == null || res.length == 0) {
                                c.sendMessage("No response from vote API. Please try again shortly.");
                                return;
                            }
                            if (res[0].message != null) {
                                c.sendMessage(res[0].message);
                                return;
                            }

                            int claims = Math.max(0, res[0].give_amount);
                            if (claims == 0) {
                                c.sendMessage("No pending rewards for that id.");
                                return;
                            }
                            ...
                            c.sendMessage("Thanks for voting! (" + claims + (claims == 1 ? " claim" : " claims") + " processed)");
                            c.sendMessage("You now have " + c.votePoints + " vote point" + (c.votePoints == 1 ? "" : "s") + ".");

                        } catch (Exception e) {
                            c.sendMessage("API services are currently offline. Please try again shortly.");
                            e.printStackTrace();
                        }
                    }
                });
            ...
            if (playerCommand.startsWith("/")) {
                ...
                if (playerCommand.startsWith("//")) {
                    ...
                    GroupWildyMan.sendGroupMessage(c, playerCommand);
                } else {
