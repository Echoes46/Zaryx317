...
import io.zaryx.content.skills.TabletCreation;
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (c.inBank) {
			if (c.getBank().withdrawFromSlot(c.xInterfaceId, c.xRemoveId, c.xRemoveSlot, Xamount)) {
				...
				c.xRemoveId = 0;
				return;
			} else if (GroupWildyManBank.processContainerAction(c, action)) {
			c.xRemoveSlot = 0;
			c.xInterfaceId = 0;
			c.xRemoveId = 0;
			return;
		...
		switch (c.xInterfaceId) {
			...
			case 45349:
				...
				c.getPakYak().finishEnterAmount_withdraw(c.xRemoveId,Xamount);
				break;
		...
		case 5064:
			...
			if (c.inpakyak) {
				c.getPakYak().finishEnterAmount_deposit(c.xRemoveId,Xamount);
				return;
			}
			if (c.isBanking) {
