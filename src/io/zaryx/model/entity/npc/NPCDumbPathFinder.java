...
import io.zaryx.model.collisionmap.TileControl;
...
private static final int NORTH = 0, EAST = 1,  SOUTH = 2, WEST = 3;
	...
	public static void follow(NPC npc, Entity following, boolean runTick) {
		...
		if (npc.getNpcId() == Npcs.THE_MAIDEN_OF_SUGADINTI)
			return;
		if (npc.getNpcId() == 12191 || npc.getNpcId() == 12214 || npc.getNpcId() == 12166 ||  npc.getNpcId() == 12215 || npc.getNpcId() == 12219)
			return;
		...
		if (distance > 1) {
			...
			if (moves[NORTH] && moves[EAST]) {
				...
				for (int x = npc.getX(); x < npc.getX() + npcSize; x++) {
					if (npc.getRegionProvider().blockedNorth(x, npc.getY() + (npcSize - 1), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[0] = true;
					}

					if (npc.getRegionProvider().blockedNorthEast(x, npc.getY() + (npcSize - 1), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				for (int y = npc.getY(); y < npc.getY() + npcSize; y++) {
					if (npc.getRegionProvider().blockedEast(npc.getX() + (npcSize - 1), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[1] = true;
					}

					if (npc.getRegionProvider().blockedNorthEast(npc.getX() + (npcSize - 1), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				for (int x = npc.getX(); x < npc.getX() + npcSize; x++) {
					...
					if (npc.getRegionProvider().blockedNorthWest(x, npc.getY() + (npcSize - 1), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				// West Check
				for (int y = npc.getY(); y < npc.getY() + npcSize; y++) {
					if (npc.getRegionProvider().blockedWest(npc.getX(), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[1] = true;
					}

					if (npc.getRegionProvider().blockedNorthWest(npc.getX(), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				// South Check
				for (int x = npc.getX(); x < npc.getX() + npcSize; x++) {
					if (npc.getRegionProvider().blockedSouth(x, npc.getY(), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[0] = true;
					}

					if (npc.getRegionProvider().blockedSouthEast(x, npc.getY(), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				for (int y = npc.getY(); y < npc.getY() + npcSize; y++) {
					...
					if (npc.getRegionProvider().blockedSouthEast(npc.getX() + (npcSize - 1), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				for (int y = npc.getY(); y < npc.getY() + npcSize; y++) {
					...
					if (npc.getRegionProvider().blockedSouthWest(npc.getX(), y, npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
				...
				for (int x = npc.getX(); x < npc.getX() + npcSize; x++) {
					...
					if (npc.getRegionProvider().blockedSouthWest(x, npc.getY(), npc.getHeight(), true) && !Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) {
						blocked[2] = true;
					}
				}
