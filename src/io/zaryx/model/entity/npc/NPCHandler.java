...
import io.zaryx.content.achievement_diary.AchievementDiaryKills;
...
import io.zaryx.content.minigames.fight_cave.Wave;
import io.zaryx.content.minigames.inferno.InfernoWaveData;
import io.zaryx.content.minigames.pest_control.PestControl;
import io.zaryx.content.minigames.xeric.XericWave;
import io.zaryx.content.questing.hftd.DagannothMother;
import io.zaryx.content.skills.hunter.impling.PuroPuro;
...
import io.zaryx.model.entity.npc.actions.LoadSpell;
import io.zaryx.model.entity.npc.data.AttackAnimation;
...
import static io.zaryx.model.entity.player.Boundary.DONATOR_ZONE_BLOODY;

public class NPCHandler {

    private static final Logger logger = LoggerFactory.getLogger(NPCHandler.class);
    public static NPCHandler handler;
    ...
    public void killedBarrow(int i) {
        Player player = PlayerHandler.players[npcs[i].killedBy];
        if (player != null && player.getBarrows() != null) {
            Optional<Brother> brother = player.getBarrows().getBrother(npcs[i].getNpcId());
    ...
    public void stepAway(NPC npc) {
        ...
        if (npc.getNpcId() == 12205 || npc.getNpcId() == 12223 || npc.getNpcId() == 12191 || npc.getNpcId() == 12214 || npc.getNpcId() == 12166 || npc.getNpcId() == 655) {
            return;
        }
    ...
    public static boolean isFightCaveNpc(NPC npc) {
        ...
        switch (npc.getNpcId()) {
        case io.zaryx.content.minigames.fight_cave.Wave.TZ_KEK_SPAWN:
        case io.zaryx.content.minigames.fight_cave.Wave.TZ_KIH:
        case io.zaryx.content.minigames.fight_cave.Wave.TZ_KEK:
        case io.zaryx.content.minigames.fight_cave.Wave.TOK_XIL:
        case io.zaryx.content.minigames.fight_cave.Wave.YT_MEJKOT:
        case io.zaryx.content.minigames.fight_cave.Wave.KET_ZEK:
        case Wave.TZTOK_JAD: 
            return true;
        }
        return false;
    ...
    public int getNpcDelay(NPC npc) {
        ...
        case 498: 
            ...
            case 9400:
                return npc.getAttackType() == CombatType.MAGE ? 5 : 4;
        case 6611: 
        case 6612:
...
//        long start = System.currentTimeMillis();
        Arrays.stream(npcs).filter(Objects::nonNull).forEach(npc -> {
            ...
            try {
                ...
                //lets see how this affect performance
...
//        System.out.println("Took [" + (System.currentTimeMillis() - start) + "] MS to cycle " + (maxNPCs - 1) + " npcs");
    }
            ...
            if (PlayerHandler.players[j] != null) {
                ...
                if (PlayerHandler.players[j].goodDistance(c.absX, c.absY, npc.absX, npc.absY, multiAttackDistance(npc))) {
                    ...
                    if (npc.getAttackType() == CombatType.SPECIAL) {
                        ...
                        if (npc.getNpcId() == 13201) {
                            if (guardianSpellCoordinates.stream().noneMatch(p -> p[0] == c.absX && p[1] == c.absY)) {
                                continue;
                            }
                        }
    ...
    public boolean multiAttacks(NPC npc) {
        ...
        case 7617:
            ...
            case 13201:
            case 1277:
                return npc.getAttackType() == CombatType.RANGE || npc.getAttackType() == CombatType.MAGE;
        case 8609: 
    ...
    void bloodyDeathHandler(Player player, NPC npc) {
        if (npc != null) {
            if (player != null) {
                if (player.getBloody_battle() != null &&
                        Boundary.isIn(player, DONATOR_ZONE_BLOODY)
    ...
    private void killedBloodybattle(Player player, NPC npc) {
        if (player.getBloody_battle() != null && player.getHeight() == npc.getHeight()) {
            player.getBloody_battle().setKillsRemaining(player.getBloody_battle().getKillsRemaining() - 1);
            player.getPA().sendString(35428, "Bloody Wave: " + (player.getBloody_wave()+1)
                    + ", Kills Remaining : " + player.getBloody_wave_kills());
            if (player.getBloody_battle().getKillsRemaining() == 0) {
                player.setBloody_wave(player.getBloody_wave() + 1);
                player.getBloody_battle().spawn(npc);
            }
        }
    }
    ...
    public void resetPlayersInCombat(int i) {
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) if (PlayerHandler.players[j].underAttackByNpc == i) PlayerHandler.players[j].underAttackByNpc = 0;
        }
    }
    ...
    public void followPlayer(NPC npc, int playerId) {
        ...
        if (npc.getNpcId() == 12223 || npc.getNpcId() == 12191 || npc.getNpcId() == 655) {
            return;
        }
    ...
    public int distanceRequired(NPC npc) {
        ...
        case 319: 
            ...
            case 7145:
            case 7146:
                if (npc.getAttackType() == CombatType.MAGE) if (npc.getAttackType() == CombatType.SPECIAL) if (npc.getAttackType() == CombatType.RANGE) return 4;
                if (npc.getAttackType() == CombatType.MELEE) return 2;
        //case 5890:
        case 7544: 
        case 5129: 
    ...
    public boolean prayerProtectionIgnored(NPC npc) {
        ...
        case 6503:
            ...
            return npc.getAttackType() == CombatType.SPECIAL;
            case 9400:
                return npc.getAttackType() == CombatType.MAGE || npc.getAttackType() == CombatType.MELEE;
        case 465: 
    ...
    public int getMaxHit(Player c, NPC npc) {
        ...
        case Skotizo.DARK_ANKOU:
            return 8;
        case 8028:
            int resistance = c.getItems().isWearingItem(1540) || c.getItems().isWearingItem(11283) || c.getItems().isWearingItem(11284) || c.getItems().isWearingItem(33195) || c.getItems().isWearingItem(33196) || c.getItems().isWearingItem(33197)|| (npc.getNpcId() == 465 && c.getItems().isWearingItem(2890)) ? 1 : 0;
            if ((System.currentTimeMillis() - c.lastAntifirePotion) < c.antifireDelay) {
                resistance = resistance + 1;
            }
        ...
        case 2558:
            ...
            case 9400:
                return npc.getAttackType() == CombatType.MAGE ? 60 : 50;
        case 2562:
            return 31;
        case 2215:
    ...
    private final ArrayList<int[]> guardianSpellCoordinates = new ArrayList<>(3);
    ...
    public void groundSpell(NPC npc, Player player, int startGfx, int endGfx, String coords, int time) {
        ...
        switch (coords) {
            case "araphel":
                ...
                break;
            case "guardian":
                player.coordinates = guardianSpellCoordinates;
                break;
        case "hydra":
        ...
        CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
                for (int[] point : player.coordinates) {
                    ...
                    if (Objects.equals(coords, "guardian")) {
                        player.getPA().createPlayersStillGfx(281, x2, y2, player.heightLevel, 5);
                    }
                }
