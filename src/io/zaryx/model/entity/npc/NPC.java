...
import io.zaryx.content.bosses.CorporealBeast;
...
import io.zaryx.model.entity.npc.data.BlockAnimation;
import io.zaryx.model.entity.npc.data.DeathAnimation;
...
public class NPC extends Entity {
	...
	public void process() {
		if(!getRegionProvider().get(getX(), getY()).playersInRegion.isEmpty()) {
			...
			AlchemicalHydra hydraInstance = NPCHandler.getHydraInstance(npc).orElse(null);
						...
						if (!npc.summoner && !npc.ThrallSummoner) {
							...
							if (npc.attackTimer == 0) {
								if (npc.getNpcAutoAttacks().isEmpty()) {
									Server.npcHandler.attackPlayer(c, npc);
								} else {
							...
							if (c != null && c.getPosition().inMulti() && c.npcAttackingIndex > 0) {
								...
								if (c.absX == npc.absX && c.absY == npc.absY) {
									...
									System.out.println("Npc " + npc.getDefinition().getName() + " stepping away 2");
...
//		oldIndex = c.getIndex();
	...
	private int facingDirection; // 0 = NORTH, 1 = EAST, 2 = SOUTH, 3 = WEST
	...
	// Set the facing direction of the NPC
	public void setFacingDirection(int direction) {
		if (direction >= 0 && direction <= 3) {
			this.facingDirection = direction;
		} else {
			throw new IllegalArgumentException("Invalid direction value. Must be 0 (NORTH), 1 (EAST), 2 (SOUTH), or 3 (WEST).");
		}
	}
