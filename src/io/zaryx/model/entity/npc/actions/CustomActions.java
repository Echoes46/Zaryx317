        ...
        NPCAction.register(315, 1, (player, npc) -> {
            if (player.isMember()) {
                ...
                player.start(new DialogueBuilder(player).npc(315, "Welcome, I am Steve. I hold all the passes for the division,", "If you're interested in buying a pass I can sell you one",
                        "For a low low price of 1Bill UpgradePoints.").option("Buy division Premium pass for 1Bill UpgradePoints?",
                        new DialogueOption("Yes", p -> {
                            if (p.foundryPoints <= 1_000_000_000) {
                                p.start(new DialogueBuilder(player).npc(315, "You don't have a enough UpgradePoints to purchase a pass!"));
                                return;
                            }
...
//                            player.getPA().closeAllWindows();
                        ...
                        if (player.foundryPoints < 100_000_000) {
                            player.start(new DialogueBuilder(player).npc(7950,"You need to bring me", "100M UpgradePoints!"));
//                            player.getPA().closeAllWindows();
                            return;
                        }
...
//                        player.getPA().closeAllWindows();
                        ...
                        PlayerHandler.executeGlobalMessage(player.getDisplayName() + " has just obtained Juan gloves!");
                    }),
                    new DialogueOption("Ghommal's Hilt 5", p -> {
                        ...
                        if (player.foundryPoints < 50_000_000) {
                            player.start(new DialogueBuilder(player).npc(7950,"You need to bring me", "50M UpgradePoints!"));
//                            player.getPA().closeAllWindows();
                            return;
                        }
                    ...
                    new DialogueOption("Ghommal's Hilt 6", p -> {
                        ...
                        if (player.foundryPoints < 150_000_000) {
                            player.start(new DialogueBuilder(player).npc(7950,"You need to bring me", "150M MadPoints!"));
//                            player.getPA().closeAllWindows();
                            return;
                        }
