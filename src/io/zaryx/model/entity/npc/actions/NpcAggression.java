    ...
    public static void doAggression(NPC npc, NPCHandler npcHandler) {
        if ((isAggressive(npc) || npc.getBehaviour().isAggressive())
            ...
            if (closestPlayer != null) {
                ...
                Boundary.isIn(npc, Boundary.CATACOMBS) && npc.getHeight() >= 4)) {
            ...
            if (System.currentTimeMillis() - npc.lastRandomlySelectedPlayer > 10000) {
                ...
                if (player > 0) {
                    ...
                    if (PlayerHandler.players[player].underAttackByPlayer > 0 && PlayerHandler.players[player].underAttackByNpc <= 0 || PlayerHandler.players[player].playerAttackingIndex > 0 && PlayerHandler.players[player].underAttackByNpc <= 0) {
//                        System.out.println("Player is under attack by a player or npc ignore them");
            ...
            if (!npc.isDead
                ...
                if (System.currentTimeMillis() - npc.lastRandomlySelectedPlayer > 10000) {
                    ...
                    if (player > 0 && PlayerHandler.players[player].usingInfAgro) {
                        npc.setPlayerAttackingIndex(player);
                        PlayerHandler.players[player].underAttackByPlayer = npc.getIndex();
                        PlayerHandler.players[player].underAttackByNpc = npc.getIndex();
                        npc.lastRandomlySelectedPlayer = System.currentTimeMillis();
    ...
    public static int getCloseRandomPlayer(NPC npc) {
        ...
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
                ...
                if (PlayerHandler.players[j].underAttackByPlayer > 0 && PlayerHandler.players[j].getPosition().inWild() && !PlayerHandler.players[j].getPosition().inMulti()) {
                    continue;
                }
                if (PlayerHandler.players[j].playerAttackingIndex > 0 && PlayerHandler.players[j].getPosition().inWild() && !PlayerHandler.players[j].getPosition().inMulti()) {
                    continue;
                }
    ...
    public static boolean isAggressive(NPC npc) {
        ...
        if (Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS)
                ...
                || Boundary.isIn(npc, Boundary.DUNGROOMS)
                ...
                || (Boundary.isIn(npc, DonorVault.area))
