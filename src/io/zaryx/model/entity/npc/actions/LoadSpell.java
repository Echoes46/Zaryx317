package io.zaryx.model.entity.npc.actions;

import io.zaryx.Server;
import io.zaryx.content.activityboss.Groot;
import io.zaryx.content.bosses.Skotizo;
import io.zaryx.content.bosses.Tekton;
import io.zaryx.content.bosses.leviathan.TheLeviathan;
...
import io.zaryx.content.seasons.HalloweenBoss;
...
import static io.zaryx.content.bosses.DonorBoss3.burnGFX;
import static io.zaryx.model.entity.npc.NPCHandler.*;
    ...
    public static void loadSpell(Player player, NPC npc) {
        ...
        switch (npc.getNpcId()) {
            ...
            case 8888:
                ...
                npc.endGfx = -1;
                npc.hitDelayTimer = 8;
                handler().groundSpell(npc, null, -1, 317, "KRATOS_WILD", 4);
    ...
    /**
         ...
         */
            ...
            case 6768:
                ...
                break;
            /*case 13201:
                chance = Misc.random(100); //comeback to this
                String[] shout2 = {"Leave My Forest at once!", "I will rid of you Evildoer!", "This is my Home!!", "RRRRRAAAaaaaaRRRgghhHHH"};
                String randomShout2 = (shout2[new Random().nextInt(shout2.length)]);
                int distanceToGuardian = player.distanceToPoint(npc.absX, npc.absY);
                int damage1 = 0;
                damage1 = Misc.random(handler().getMaxHit(player, npc));
                int hit1 = damage1;
                int hp1 = npc.getHealth().getCurrentHealth();
                int maxHp1 = npc.getHealth().getMaximumHealth();
                if (distanceToGuardian < 3) {
                    if (chance < 25) {
                        npc.forceChat(randomShout2);
                        npc.setAttackType(CombatType.RANGE);
                        npc.maxHit = 65;
                        npc.hitDelayTimer = 3;
                        ...
                        if (Misc.random(10) == 5) {
                            player.getHealth().proposeStatus(HealthStatus.POISON, 5, Optional.of(npc));
                        }
                    } else if (chance > 90 && System.currentTimeMillis() - npc.lastSpecialAttack > 15000) {
                        npc.forceChat("Give me your Life!");
                        npc.setAttackType(CombatType.SPECIAL);
                        npc.attackTimer = 5;
                        npc.hitDelayTimer = 2;
                        npc.lastSpecialAttack = System.currentTimeMillis();
                        ...
                        if (hp1 >= maxHp1) {
                            return;
                        } else if (npc != null && hp1 < maxHp1) {
                            npc.getHealth().increase(hit1 / 2);
                            player.sendMessage("You feel the Guardian leeching your life force.");
                        }
                    } else {
                        npc.setAttackType(CombatType.MAGE);
                        npc.maxHit = 65;
                    ...
                    if (chance < 71) {
                        ...
                        npc.attackTimer = 7;
                        npc.hitDelayTimer = 4;
                        handler().groundSpell(npc, player, 280, 281, "guardian", 4);
                    } else if (System.currentTimeMillis() - npc.lastSpecialAttack > 15000) {
                        npc.forceChat(randomShout2);
                        npc.setAttackType(CombatType.SPECIAL);
                        handler().groundAttack(npc, player, -1, 1284, -1, 1);
                        npc.hitDelayTimer = 5;
                        npc.endGfx = -1;
                        npc.projectileId = -1;
                        if (Misc.random(10) == 5) {
                            player.getHealth().proposeStatus(HealthStatus.POISON, 25, Optional.of(npc));
                        }
                    } else {
                        npc.forceChat(randomShout2);
                        npc.setAttackType(CombatType.RANGE);
                        ...
                        npc.endGfx = 140;
                        if (Misc.random(10) == 5) {
                            player.getHealth().proposeStatus(HealthStatus.BLEED, 20, Optional.of(npc));
                        }
                    }
                }
                break;*/
            ...
            case 1277:
                // Shades
                int randomHit75 = Misc.random(10);
                if (randomHit75 > 2 && randomHit75 < 7) {
                    npc.setAttackType(CombatType.MAGE);
                    npc.attackTimer = 4;
                    npc.projectileId = 146;
                    npc.endGfx = 147;
                    npc.maxHit = 25;
                    if (Misc.random(10) == 5) {
                        player.getHealth().proposeStatus(HealthStatus.BLEED, 10, Optional.of(npc));
                    }
                } else if (randomHit75 > 6){
                    npc.setAttackType(CombatType.RANGE);
                    npc.projectileId = 1259;
                    npc.endGfx = 140;
                    npc.maxHit = 25;
                ...
                break;
            case 9400:
                int randomType = Misc.random(10);
                int meleeDistance = player.distanceToPoint(npc.absX, npc.absY);
                if (player.distanceToPoint(npc.absX, npc.absY) < 10) {
                    if (randomType > 2 && randomType < 7) {
                        npc.setAttackType(CombatType.MAGE);
                        npc.projectileId = 374;
                        npc.endGfx = 377;
                        npc.maxHit = 50;
                        npc.attackTimer = 5;
                        player.getHealth().proposeStatus(HealthStatus.BLEED, 25, Optional.of(npc));
                    } else if (meleeDistance > 3 && randomType == 2) {
                        npc.setAttackType(CombatType.MELEE);
                        npc.teleport(player.getPosition());
                        npc.maxHit = 50;
                        npc.projectileId = -1;
                ...
                break;
            case 13201:
                int randomAttack4 = Misc.random(10);
                if (player.distanceToPoint(npc.absX, npc.absY) < 2) {
                    npc.forceChat("Leave My Forest at once!");
                    if (randomAttack4 > 2 && randomAttack4 < 7) {
                        npc.setAttackType(CombatType.MAGE);
            ...
            case 7273:
                ...
                break;
            case 12223:
                Vardorvis.handleAttacks(npc);
                break;
            case 12214:
                TheLeviathan.handleAttacks(npc);
                break;
            case 12191:
            ...
            case 8781:
                ...
                if (r10 <= 9) {
                    ...
                    npc.startAnimation(-1);
                    npc.endGfx = 555;
                    player.gfx0(390); // also changed the gfx to suit the attack better
                    player.lastSpear = System.currentTimeMillis();
                    player.getPA().getSpeared(npc.absX, npc.absY, 3);
                    player.freezeTimer = 5;
                    player.sendMessage("The Queen's Ice has stunned you in place");
                }
                break;
            case 10532:
