...
import io.zaryx.content.bosses.*;
import io.zaryx.content.bosses.hespori.Hespori;
import io.zaryx.content.bosses.hydra.AlchemicalHydra;
import io.zaryx.content.bosses.maledictus.Maledictus;
import io.zaryx.content.bosses.nex.NexNPC;
...
import io.zaryx.content.globalboss.*;
import io.zaryx.content.minigames.inferno.AncestralGlyph;
import io.zaryx.content.minigames.inferno.InfernoWaveData;
import io.zaryx.content.minigames.rfd.DisposeTypes;
...
import io.zaryx.model.entity.npc.actions.NPCHitPlayer;
import io.zaryx.model.entity.npc.actions.NpcAggression;
import io.zaryx.model.entity.npc.data.RespawnTime;
                        ...
                        if (!npc.summoner && !npc.ThrallSummoner) {
                            ...
                            if (npc.attackTimer == 0) {
                                if (npc.getNpcId() == 11246) {
                                    Maledictus.handleAttack(npc);
                                } else if (npc.getNpcAutoAttacks().isEmpty()) {
        ...
        if (npc.isDead()) {
            ...
            if (npc.actionTimer == 0 && !npc.applyDead && !npc.needRespawn) {
                ...
                if (npc.getNpcId() == 12191) {
                    ...
                    Server.getDropManager().create(playerOwner, npc, new Location3D(playerOwner.getX(), playerOwner.getY(), playerOwner.getHeight()), 1, 12191);
                    player.getPA().movePlayer(3039, 6431, 0);
                }
...
//                        ChristmasBoss.handleKCIncrease(npc, target);
                            ...
                            if (npc.getNpcId() != 239 || npc.getNpcId() != 965 || npc.getNpcId() != 8713 || npc.getNpcId() != 11278 || npc.getNpcId() != 8999) {
...
//                            }
                    ...
                    if (npc.getDungInstance() != null) {
                        Optional<Player> plrOpt = PlayerHandler.getOptionalPlayerByIndex(npc.killedBy);
                        npc.getDungInstance().handleMobDeath(plrOpt.orElse(null), type);
                    }
...
//                            new Broadcast("<img=58> [WILDYPURSUIT]@blu@ The wildy boss [@red@Revenant Maledictus@blu@] has been defeated!");
//                            npc.getBehaviour().setRespawn(false);
//                            break;
//
//                        case TheUnbearable.NPC_ID:
//                            new Broadcast("<img=58> [WILDYPURSUIT]@blu@ The wildy boss [@red@Unbearable@blu@] has been defeated!");
//                            TheUnbearable.rewardPlayers();
//                            break;
//                        case FragmentOfSeren.NPC_ID:
//                            new Broadcast("<img=58> [WILDYPURSUIT]@blu@ The wildy boss [@red@Seren@blu@] has been defeated!");
