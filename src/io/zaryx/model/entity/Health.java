...
import io.zaryx.model.cycleevent.impl.*;
...
public final class Health {
    ...
    private void updateStatus() {
        if (entity instanceof Player) {
            ...
            if (player.getHealth().getStatus().isNormal()) player.getPA().sendConfig(102, 0);
        }
    }
    ...
    public int getMaximumHealth() {
        if (entity.isPlayer()) {
            maximumHealth = entity.asPlayer().getLevelForXP(entity.asPlayer().playerXP[3]);
        }
    ...
    public void increase(int amount) {
        if (entity instanceof Player) {
            ...
            maximumHealth = player.getPA().getLevelForXP(player.playerXP[Player.playerHitpoints]);

            // Only set max health if not already set (i.e., it's 0)
            if (maximumHealth == 0) {
                maximumHealth = player.getPA().getLevelForXP(player.playerXP[Player.playerHitpoints]);
            }

            player.sendMessage("Current HP: [" + player.getHealth().getCurrentHealth() + "] and Max HP: [" + player.getHealth().getMaximumHealth() + "]");
        }
