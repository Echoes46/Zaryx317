...
public class DupeWarden {
    ...
    private static final int[] specialItemChecker = {
            ...
            13073, 24185, 27226, 27229, 27232, 27235, 27238, 27241, 24664, 24666,
            24668, 33149, 33239
    };
        ...
        for (int itemId : specialItemChecker) {
            ...
            if (increase > maxAllowableIncrease) {
                ...
                for (Player player1 : PlayerHandler.getPlayers()) {
                    ...
                    // Convert display name to lower case once
                    String name = player1.getDisplayNameLower();

                    // Skip if player is in YOUTUBERS
                    boolean isYoutuber = Arrays.stream(Configuration.YOUTUBERS)
                            .map(String::toLowerCase)
                            .anyMatch(name::contains);
                    if (isYoutuber) continue;

                    // Skip if player is staff
                    if (player1.getRights().isOrInherits(Right.HELPER)) continue;
    ...
    private int getMaxAllowableIncrease(int itemId) {
        int foe_price = FireOfExchangeBurnPrice.getBurnPrice(null, itemId, false);
